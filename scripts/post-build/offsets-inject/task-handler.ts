...
const OFFSET_INJECTION_REVIEW_JS_URL = "https://unpkg.com/offsets-injection-review@latest/dist/offsets-injection-review.umd.cjs"
...
export const taskHandler = new (class implements TaskHandler<GlobalData> {
    async globalInitialize(siteDir: string): Promise<GlobalData> {
        ...
        log(`Injecting api endpoint to offsets-injection-review content script ${OFFSET_INJECTION_REVIEW_CONTENT_SCRIPT_TARGET_FILE}...`);
        const contentScriptFile = path.join(siteDir, OFFSET_INJECTION_REVIEW_CONTENT_SCRIPT_TARGET_FILE)
        await fs.promises.writeFile(contentScriptFile, (await fs.promises.readFile(contentScriptFile, "utf-8")).replace("{apiEndpoint}", API_ENDPOINT), "utf-8");

        await updateCommitHash();

        return {
            commitHash,
            jsFileMd5: createHash('md5').update(await jsContent).digest('hex'),
            cssFileMd5: createHash('md5').update(await cssContent).digest('hex'),
        }
    }
    ...
    async process(document: HTMLElement, filePath: string) {
        document.querySelector("body").insertAdjacentHTML("beforeend", `<script>sessionStorage.setItem("commitHash", "${this.globalData.commitHash}")</script>`);
        ...
        document.querySelector("head").insertAdjacentHTML("beforeend", `<link rel="stylesheet" href="${cssFilePathToHtml}?${this.globalData.cssFileMd5}">`);
        document.querySelector("head").insertAdjacentHTML("beforeend", `<script src="${jsFilePathToHtml}?${this.globalData.jsFileMd5}"></script>`);
    }
})