...
const MATHJAX_SOURCE = path.dirname(
  module.createRequire(import.meta.url).resolve("mathjax-full/package.json")
);
...
export class MathRenderer {
  ...
  async initialize() {
    ...
    await Promise.all(
      MATHJAX_PACKAGES.map(async (packageName) => {
        ...
        const packageConfigurationFile = packageDirFiles.find((filename) =>
          filename.endsWith("Configuration.js")
        );
    ...
    const outputJax = new CHTML<LiteElement, unknown, LiteDocument>({
      fontURL: path.relative(
        path.dirname(MATHJAX_TARGET_CSS_FILE),
        MATHJAX_TARGET_FONTS_DIR
      ),
      adaptiveCSS: false,
    });
  ...
  render(math: string, isDisplay: boolean) {
    const element = this.document.convert(math, {
      display: isDisplay,
    }) as LiteElement;
...
export const taskHandler = new (class implements TaskHandler<void> {
    ...
    await Promise.all(
      fontFilesToCopy.map((filename) =>
        fs.promises.copyFile(
          path.join(fontsSourceDir, filename),
          path.join(fontsDestDir, filename)
        )
      )
    );
  ...
  async process(document: HTMLElement, filePath: string) {
    const mathElements = document.querySelectorAll(
      "div.arithmatex, span.arithmatex"
    );
    ...
    if (mathElements.length > 0) {
      ...
      const cssFilePathToHtml = path.relative(
        htmlFilePathToRoot,
        MATHJAX_TARGET_CSS_FILE
      );
      document
        .querySelector("head")
        .insertAdjacentHTML(
          "beforeend",
          `<link rel="stylesheet" href="${cssFilePathToHtml}">`
        );
    }
    ...
    document
      ...
      .filter((element) =>
        element.getAttribute("src")?.endsWith(MATH_CSR_SCRIPT_SUFFIX)
      )
