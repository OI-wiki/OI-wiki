...
struct Query {
	u32 L, R, time, res;
	Query(const u32 _L, const u32 _R, const u32 t) : L(_L), R(_R), time(t), res(0) {}
};
...
int main() {
	...
	// Part II: color_cnt < B
	std::sort(querys.begin(), querys.end(), [](const Query &lhs, const Query &rhs) {
		return lhs.R < rhs.R;
	});
	...
	for(u32 idx = 1; idx <= N; ++idx) {
		if(color_cnt[arr[idx]] < B) {
			for(auto lit = low_index[arr[idx]].begin(); lit != low_index[arr[idx]].end(); ++lit) {
				update(*lit);
			}
