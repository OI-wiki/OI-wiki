...
void hopcroft() {
    const auto rebuild = [&](int u) {
        vector<int> tmp;
        for (auto i : Q[u]) if (belong[i] == u) tmp.push_back(i);
        swap(Q[u], tmp);
    };
    queue<int> que;
    que.push(1);
        ...
        for (int c = 0; c <= 1; c++) {
            ...
            for (auto i : td) {
                if (S[i].size() < sz[i]) {
                    ac[++cnt] = ac[i];
                    if (S[i].size() * 2 <= sz[i]) {
                        // 为了复杂度正确，这里没有从 Q[i] 中删除 S[i]，而是改变它们的 belong
                        // 注意实现时，如果需要取用 Q 中的内容，必须先 rebuild
                        // 以及注意 sz 中的值是真正的等价类的大小，sz[i] 不一定等于 Q[i].size()
                        ...
                        vector<int> tmp;
                        for (auto j : Q[i]) {
                            if (tag[j]) tmp.push_back(j);
                            else Q[belong[j] = cnt].push_back(j);
                        }
                        swap(Q[i], tmp);
                        sz[i] = Q[i].size();
                        sz[cnt] = Q[cnt].size();
    ...
    for (int i = 0; i < (1 << k); i++) for (int c = 0; c <= 1; c++) trans[belong[i]][c] = belong[nxt[i][c]];
    for (int i = 1; i <= cnt; i++) for (int c = 0; c <= 1; c++) pre_trans[trans[i][c]][c].push_back(i);
}
// --8<-- [end:core]
signed main() {
    ...
    for (int i = 1; i <= n; i++) for (int j = 1; j <= cnt; j++) {
        for (int c = 0; c <= 1; c++) {
            if (a[i] == '?' || (a[i] - '0' == c)) (f[i][trans[j][c]] += f[i - 1][j]) %= P;
        }
    }
    for (int i = 1; i <= cnt; i++) if (ac[i]) g[n + 1][i]++;
    for (int i = n; i >= 1; i--) for (int j = 1; j <= cnt; j++) {
        for (int c = 0; c <= 1; c++) {
            if (a[i] == '?' || (a[i] - '0' == c)) for (int k : pre_trans[j][c]) {
                (g[i][k] += g[i + 1][j]) %= P;
            }
        }
    }
    for (int i = 1; i <= n; i++) if (a[i] == '1') printf("0\n");
    else {
        int ans = 0;
        for (int j = 1; j <= cnt; j++) {
