...
void solve(int l, int r, int type) {
    ...
    if (type <= 1) {
        for (int i = l, j = r; i <= j; i += 2, j -= 2) {
            for (int k = 7; k >= 0; k--) {
                if (op[k] == type && d[k >> 2].check(l, i) && d[(k & 3) + 2].check(i + 1, r)) {
                    putchar('('); solve(l, i, k >> 2); solve(i + 1, r, (k & 3) + 2); putchar(')');
                    return;
                }
                if (op[k] == type && d[(k >> 1) + 2].check(l, j - 1) && d[k & 1].check(j, r)) {
                    putchar('('); solve(l, j - 1, (k >> 1) + 2); solve(j, r, k & 1); putchar(')');
                    return;
                }
            }
        }
        ...
        for (int i = l, j = r; i <= j; i += 2, j -= 2) {
            if (d[k >> 1].check(l, i) && d[k & 1].check(i + 1, r)) {
                solve(l, i, k >> 1); solve(i + 1, r, k & 1);
                return;
            }
            if (d[k >> 1].check(l, j - 1) && d[k & 1].check(j, r)) {
                solve(l, j - 1, k >> 1); solve(j, r, k & 1);
                return;
            }
        }
    }
