...
SampleIt sample_fwd(PopIt first, PopIt last, SampleIt out, Diff n, Adapter& rng) {
    Diff k = 0;
    // 先填满前 n 个
...
template <class PopIt, class SampleIt, class Diff, class Adapter>
SampleIt sample_ra(PopIt first, PopIt last, SampleIt out, Diff n, Adapter& rng) {
    Diff N = last - first;
    if (n >= N) {
...
SampleIt sample_impl(PopIt first, PopIt last, SampleIt out, Size n,
    ...
    return sample_fwd(first, last, out, static_cast<decltype(last - first)>(n), adapter);
}
...
SampleIt sample_impl(PopIt first, PopIt last, SampleIt out, Size n,
    ...
    return sample_ra(first, last, out, static_cast<decltype(last - first)>(n), adapter);
}
