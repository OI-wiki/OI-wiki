...
??? note "代码示例"
    ```cpp
    --8<-- "docs/misc/code/random/random_1.cpp"
    ```
...
```cpp
--8<-- "docs/misc/code/random/random_5.cpp"
```
...
播种方式是直接给 $x$ 或 $y$ 赋值为对应的种子，生成方式是直接反复调用 `xorshift32()` 或 `xorshift64()`．这两个函数在 $x$ 和 $y$ 不为 $0$ 时分别具有 $2^{32} - 1$ 和 $2^{64} - 1$ 的周期，因其实现极其简单，常将其和种子下发给选手生成大范围的数据以减少 I/O 开销．
...
??? note "参考实现"
    ```cpp
    --8<-- "docs/misc/code/random/random_9.cpp"
    ```
...
```cpp
--8<-- "docs/misc/code/random/random_10.cpp"
```

下面是用 `mt19937` 及 `shuffle()` 编写的同一个数据生成器．

```cpp
--8<-- "docs/misc/code/random/random_11.cpp"
```

下面是随机排列前十个正整数的一个实现．

```cpp
--8<-- "docs/misc/code/random/random_12.cpp"
```
...
```cpp
--8<-- "docs/misc/code/random/random_13.cpp"
```
