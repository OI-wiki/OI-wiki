...
在计算机程序中，我们需要用有限位数对实数做近似表示，如今的大多数计算机都使用 [IEEE-754](https://en.wikipedia.org/wiki/IEEE_754) 规定的浮点数来作为这个近似表示。对于 $\frac{1}{3}$，由于我们不能在有限位数内对它进行精准表示，因此在使用IEEE754表示法时，必须四舍五入一部分数值（truncate)。这种 **舍入误差**（Rounding off error）是浮点计算的一个特征。
