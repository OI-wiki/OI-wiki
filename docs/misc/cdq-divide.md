...
???+ note "[三维偏序](https://www.luogu.com.cn/problem/P3810)"
    ...
    ??? 解题思路
        ...
        稍微思考一下就会发现，那个 $a_{i} \leq a_{j}$ 的限制条件没啥用了：已经将序列按 $a$ 排序，则 $a_{i} \leq a_{j}$ 可转化为 $i \leq j$。既然 $i$ 比 $mid$ 小，$j$ 比 $mid$ 大，那 $i$ 肯定比 $j$ 要小。现在还剩下两个限制条件：$b_{i} \leq b_{j}$ 与 $c_{i} \leq c_{j}$, 根据这个限制条件我们就可以枚举 $j$, 求出有多少个满足条件的 $i$。
