...
???+ note "简要题意"
    给定一张点、边都有非负权值的无向图，找到一个大小 $\leqslant K$ 的边集合 $S$，以最大化与 $S$ 相连的点的权值和减去 $S$ 的边权和。一个点的权值只被计算一次。
...
-   假设最优解中每个菊花的结点数分别为 $a_1,\cdots,a_l$，则一定有 $(a_1-1)+\cdots+(a_l-1)\leqslant K$，其中 $K$ 表示最多能够选出的边数。
-   从而所有菊花都被染对颜色的概率是 $2^{1-a_1}\cdots 2^{1-a_l}\geqslant 2^{-K}$。
...
-   观察：记 $S'$ 表示所有选取了 $e'$ 的方案中的最优解，则 $|S'|\leqslant |S|+1$。
-   从而单次复杂度 $O(|S'|)=O(|S|)$。
...
推论：等概率随机选取 $(s,t)$，满足前述要求的概率 $\geqslant \dfrac {(n-1)(m-1)}{nm}$。

-   注意到这个结论严格强于先前给出的存在性结论。

推论：等概率独立随机地连续选取 $\dfrac {\min(n,m)}2$ 对不含公共元素的 $(s,t)$，并对它们 **依次** 操作（即连边 $t\to s$），则这些操作全部满足目标 I 的概率 $\geqslant \dfrac 14$。
...
不难想到折半搜索。把点集均匀分成左右两半 $V_L,V_R$（大小都为 $\dfrac n2$），计算数组 $f_{L,k}$ 表示点集 $L\subseteq V_L$ 中的所有 $\geqslant k$ 元团的最大权值和。接着我们枚举右半边的每个团 $C_R$，算出左半边有哪些点与 $C_R$ 中的所有点相连（这个点集记作 $N_L$），并用 $f_{N_L,\frac 23 n-|C_R|}+\textit{value}(C_R)$ 更新答案。
...
-   有了这一性质，我们就可以直接钦定左侧团 $L$、右侧团 $C_R$ 的大小都 $\geqslant \dfrac n3$。这会对复杂度带来两处改进：
    -   $f$ 可以省掉记录大小的维度。
    -   因为只需考虑大小 $\geqslant \dfrac n3$ 的团，所以需要考虑的左侧团 $L$ 和 右侧团 $C_R$ 的数量也大大减少至约 $1.8\cdot 10^6$。
...
???+ note "(*)Schwartz–Zippel 引理"
    ...
    推论：若 $z_1,\cdots,z_k$ 都在 $S$ 中等概率独立随机选取，则 $\mathrm{Pr}\big[f(z_1,\cdots,z_k)=0\big]\leqslant \dfrac d{|S|}$。

记 $F$ 为模 $Q$ 的剩余系所对应的域，则对于一个 $L\leqslant n_1+n_2$，$\sum\limits_i f_{0,i,L}$ 和 $\sum\limits_i f_{1,i,L}$ 就分别对应着一个 $F$ 上关于变元集合 $\{x_{*,*}\}$ 的 $L$ 次多元多项式，不妨将这两个多项式记为 $P_0,P_1$。
...
-   观察：对于任意的 $A\neq B; A,B\leqslant N$ 和随机选取的质数 $Q\leqslant Q_{\max}$，一定有：

$$
\mathrm{Pr}\big[A\equiv B\pmod {Q}\big]=O\Big(\dfrac{\log N \log Q_{max}}{Q_{max}}\Big)
$$

-   这是因为：使 $A\equiv B$ 成立的 $Q$ 一定满足 $Q\big|(A-B)$，这样的 $Q$ 有 $\omega(A-B)\leqslant \log_2 N$ 个；而由质数定理，$Q_{\max}$ 以内不同的质数又有 $\Theta\Big(\dfrac {Q_{\max}}{\log Q_{\max}}\Big)$ 个。将两者相除即可得到上式。
-   在上述观察中取 $A,B$（满足 $A\neq B$）为某一特定项在 $P_0,P_1$ 中的系数（也就等于该项对应的串在 $G_0,G_1$ 中的出现次数），则易见 $A,B\leqslant (m_1+m_2)^{L}$，得到：
...
???+ note "问题"
    ...
    $n\cdot m\leqslant 2\cdot10^5;q\leqslant 10^6;\epsilon=0.5,\delta=0.2$
...
???+ note "期望复杂度的证明"
    将证，对于任意的堆 $A$，从根节点开始每次随机选左或者右走下去（直到无路可走），路径长度（即路径上的结点数）的期望值 $h(A)\leqslant\log_2 (|A|+1)$。
    ...
    \begin{align} h(A)
    &=1+\frac{h(L)+h(R)}2
    \\&\leqslant1+\frac{\log_2(|L|+1)+\log_2(|R|+1)}2
    \\&=\log_2{2\sqrt{(|L|+1)(|R|+1)}}
    \\&\leqslant\log_2{\frac{2\big((|L|+1)+(|R|+1)\big)}2}
    \\&=\log_2{(|A|+1)} \end{align}
    $$
...
**自然常数的使用**：$\Big(1-\dfrac{1}{n}\Big)^n\leqslant \dfrac{1}{\mathrm{e}},\forall n\geqslant1$

-   左式关于 $n\geqslant 1$ 单调递增且在 $+\infty$ 处的极限是 $\dfrac{1}{\mathrm{e}}$，因此有这个结论。
...
???+ note "问题"
    对于 $n \in \mathbf{N}^*; p,q\in [0,1]$ 且 $q\leqslant p$，求证：随机图 $G_1(n,p)$ 的连通分量个数的期望值不超过随机图 $G_2(n,q)$ 的连通分量个数的期望值。这里 $G(n,\alpha)$ 表示一张 $n$ 个结点的简单无向图 $G$，其中 $\dfrac {n(n-1)}2$ 条可能的边中的每一条都有 $\alpha$ 的概率出现，且这些概率互相独立。
