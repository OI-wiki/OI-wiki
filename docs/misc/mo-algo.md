...
证：令每一块中L的最大值为 $max1,max2,max3,...,maxceil(\\sqrt{n})$。

由第一次排序可知，$max1&lt;=max2&lt;=...&lt;=maxceil(\\sqrt{n})$。
...
& = O(\\sqrt{n}_(max1-1+max2-max1+max3-max2+...+maxceil(\\sqrt{n}-1)-maxceil(\\sqrt{n}-2)+maxceil(\\sqrt{n})-maxceil(\\sqrt{n}-1))) \\\\
& = O(\\sqrt{n}_(maxceil(\\sqrt{n})-1))
\\end{aligned}
$$

(裂项求和)

由题可知 $maxceil(\\sqrt{n})$ 最大为 n，所以 L 的总时间复杂度最坏情况下为 $O(n\\sqrt{n})$。
...
下面的代码中mot表示答案的分母(mother)，sub表示分子，sqn表示块的大小：$\\sqrt{n}$(sqrt(n))，arr是输入的数组，node是存储询问的结构体，tab是询问序列（排序后的），col同上所述。
