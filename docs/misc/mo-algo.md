...
???+ note "证明"
    ...
    由第一次排序可知，$\max_1 \leqslant \max_2 \leqslant \cdots \leqslant \max_{\lceil\sqrt{n}\rceil}$。
...
假设 $n, m$ 同阶且 $n$ 是完全平方数。我们考虑形如 $[a \sqrt n, b \sqrt n](1 \leqslant a, b \leqslant \sqrt n)$ 的区间，这样的区间一共有 $n$ 个。如果把所有的区间看成平面上的点，则两点之间的曼哈顿距离恰好为两区间的转移代价，并且任意两个区间之间的最小曼哈顿距离为 $\sqrt n$，所以处理所有询问的时间复杂度最小为 $O(n \sqrt n)$。其它情况的数据构造方法与之类似。
...
??? note "关于四个循环位置的讨论"
    ...
    -   对于 $l\leqslant r$ 的情况，$[1,l-1]$ 的元素相当于被加入了一次又被删除了一次，$[l,r]$ 的元素被加入一次，$[r+1,+\infty)$ 的元素没有被加入。这个区间是合法区间。
    ...
    这 4 种正确写法的共同特点是，前两步先扩大区间（`l--` 或 `r++`），后两步再缩小区间（`l++` 或 `r--`）。这样写，前两步是扩大区间，可以保持 $l\leqslant r+1$；执行完前两步后，$l\leqslant l'\leqslant r'\leqslant r$ 一定成立，再执行后两步只会把区间缩小到 $[l',r']$，依然有 $l\leqslant r+1$，因此这样写是正确的。
