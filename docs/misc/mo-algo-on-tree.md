...
做法：用栈维护当前结点作为父结点访问它的子结点，当从栈顶到父结点的距离大于希望块的大小时，弹出这部分元素分为一块，最后剩余的一块单独作为一块。
...
但是，这样有一个问题，若指针一开始都在 x 上，显然 x 被标记，当两个指针向同一子结点移动（还有许多情况）时，x 应该不被标记，但实际情况是 x 被标记，因为两个指针分别标记了一次，抵消了。
...
-   对于 x 指针，由于每个块中结点的距离在 $unit$ 左右，每个块中 x 指针移动 $unit^2$ 次（$unit\times dis_{\max}$），共计 $n\times unit$ 次（$unit^2 \times (\frac{n}{unit})$）；
