...
分裂等价类的时候，我们可以使用启发式分裂。即如果需要分裂的一部分没有达到整个等价类的一半，我们就只枚举需要分裂的那些状态；而如果超过了一半，我们就直接将整个等价类暴力分裂即可。这么做的话，算法时间复杂度为 $O(n|\Sigma|\log n)$。
...
注意代码中的 `Q`  **并非是真正的等价类**，它的内容没有可靠性，它的作用只是 **在转移时提供一个临时变量**。我们只通过 `belong` 数组维护每个状态 **真正** 属于哪个等价类，以及维护 `sz` 数组表示每个等价类的 **真实** 大小。只有执行完 `rebuild` 函数后，`Q` 中对应等价类的状态集合才是正确的。
...
那么此时如果分为的左右两个部分为 $type_1$ 与 $type_2$，则判断 $[l,mid]$ 能否生成 $type_1$ 和 $[mid+1,r]$ 能否生成 $type_2$，如果能则直接分治下去。

如果使用 $O(1)$ 猫树判定，这么启发式分裂的复杂度是 $O(n\log n)$。
