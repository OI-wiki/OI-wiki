...
???+ abstract "NFA" 
    下面我们令 $\mathcal{P}(Q)$ 表示 $Q$ 的幂集（所有子集的集合），令 $\Sigma_{\epsilon}=\Sigma\cup\{\epsilon\}$（$\epsilon$ 表示空串，即允许空字符的存在，空字符可以走或者不走）。NFA 是一个五元组 $(Q,\Sigma,\delta,q_0,F)$，其中：
...
正则表达式是另一种常用的正则语言表达，我们可以在许多现代语言（例如 Python）中看到它的身影。下面给出它的形式化定义：
