...
双指针顾名思义，就是同时使用两个指针，在序列、链表结构上指向的是位置，在树、图结构中指向的是结点，通过或同向移动，或相向移动来维护、统计信息。
...
???+ note " 例题 2 [luogu P3066 Running Away From the Barn G](https://www.luogu.com.cn/problem/P3066)"
    给定一棵 $n$ 个点的有根树，边有边权，结点从 1 至 $n$ 编号，1 号结点是这棵树的根。再给出一个参数 $t$，对于树上的每个结点 $u$，请求出 $u$ 的子树中有多少结点满足该结点到 $u$ 的距离不大于 $t$。数据范围：$1\leq n \leq 2\times 10^5,1 \leq t \leq 10^{18},1 \leq p_i \lt i,1 \leq w_i \leq 10^{12}$

#### 过程

从根开始用 dfs 遍历整棵树，使用一个栈来记录根到当前结点的树链，设一个指针 $u$ 指向当前结点，另一个指针 $p$ 指向与 $u$ 距离不大于 $t$ 的结点中深度最小的结点。记录到根的距离，每次二分查找确定 $p$。此时 $u$ 对 $p$ 到 $u$ 路径上的所有结点都有一个贡献，可以用树上差分来记录。  
