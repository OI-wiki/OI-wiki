...
???+ note "例题"
    ...
    数据范围：$1\leqslant n\leqslant 10^6$，$1\leqslant m\leqslant 10^{18}$，$1\leqslant k\leqslant n$，$0\leqslant a_i\leqslant 10^9$。
...
??? note "解题思路"
    ...
    这题的 $m\leqslant 10^{18}$，虽然看似恐怖，但是实际上只需要预处理出 $65$ 以内的 $i$，就可以轻松解决，比起暴力枚举快了很多。用行话讲，这个做法的 [时间复杂度](./complexity.md) 是预处理 $\Theta(n\log m)$，查询每次 $\Theta(\log m)$。
