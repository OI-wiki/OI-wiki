...
??? note "解题思路"
    ...
    请注意, 如果直接使用该公式当做比较条件，在某些条件下会违反比较的传递性(即如果a > b, b > c，则a > c)
    
    举一个例子:
    
    现有如下数据, 每行分别代表左右手, 为了方便称呼，分别用a, b, c代表每组数字
    ...
    很明显，出现这种问题的锅大概率是由于外层套上了max函数，所以我们仍然需要将其继续化简，将max函数去掉!
    ...
    此时, 我们只需要找到B和D的关系就可以了(也可以找其它的组，但不一定找的出来)
