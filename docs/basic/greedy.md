...
??? note "解题思路"
    ...
    请注意，如果直接使用该公式当做比较条件，在某些条件下会违反比较的传递性（即如果 a > b, b > c，则 a > c)
    
    举一个例子：
    
    现有如下数据，每行分别代表左右手，为了方便称呼，分别用 a, b, c 代表每组数字
    ...
    如果使用上述公式可以得到 a &lt; c, b = a, b = c, 这三者明显无法组合到一起，在某些条件下会发生意料之外的错误！
    
    很明显，出现这种问题的锅大概率是由于外层套上了 max 函数，所以我们仍然需要将其继续化简，将 max 函数去掉！
    ...
    为了方便表述从左往右，分别给其中的数标识 $A$,$B$,$C$,$D$
    
    可以很明显的发现，$A <= D$,$B >= C$ 恒成立
    
    即：
    
    $$
    \frac{1}{b_{i}} <= \frac{a_{i + 1}}{b_{i}}        \dfrac{a_i} {b_{i+1}} >= \dfrac{1} {b_{i+1}}
    $$
    
    此时，我们只需要找到 B 和 D 的关系就可以了（也可以找其它的组，但不一定找的出来）
    
    此时我们假设 $B >= D$ 恒成立，因为 $D >= A$ 所以 $B >= A$, 不等式左边恒为 $B$, 但因为 $B >= C$, 此时不管右边取哪个值，左边永远都比右边大或者等，假设不成立
    
    因此 $B < D$ 恒成立，式子化简可得
    
    $a_{i}\cdot b_{i} < a_{i + 1} \cdot b_{i + 1}$
    
    实现的时候我们将输入的两个数用一个结构体来保存并重载运算符，然后将输入的数据排序，最终遍历得到最大值：
    
    ```cpp
    struct uv {
      int a, b;
      bool operator<(const uv &x) const { return a * b < x.a * x.b; }
    };
    ```
