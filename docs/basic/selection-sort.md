...
倘若使用链表实现，由于链表的任意位置插入和删除均为 $O(1)$，故无需使用 swap（交换两个元素）操作：每次从未排序部分选择最小元素（若有多个，选取第一个）后，将其插入到未排序部分的第一个元素之前，这样就能够保证稳定性．

数组实现一般会使用 `swap` 将最小元素移到已排序部分，以此减少移动次数，所以不稳定．
