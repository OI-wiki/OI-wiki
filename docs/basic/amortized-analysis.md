...
2.  **非负性**：在任意状态 $S$ 下，势能 $\Phi(S) \geqslant 0$。
...
由于 $\Phi(S) \geqslant \Phi(S_0)$，总时间花销的上界为：

$$
\sum_{i=1}^m p_i \geqslant \sum_{i=1}^m c_i
$$
...
由于总操作次数 $n = n_{push} + n_{pop} + n_{multi-pop} \leqslant 2 \times n_{push}$，所以总成本为 $O(n_{push}) = O(n)$，每次操作的均摊成本为 $O(n)/n = O(1)$。
