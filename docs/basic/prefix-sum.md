...
??? note "解题思路"
    递推：`B[0] = A[0]`，对于 $i \geqslant 1$ 则 `B[i] = B[i-1] + A[i]`。
...
$$
S_{i_1,\cdots,i_k} = \sum_{i'_1\leqslant i_1}\cdots\sum_{i'_k\leqslant i_k} A_{i'_1,\cdots,i'_k}.
$$
...
$$
T\subseteq S \iff \forall i(t_i \leqslant s_i). 
$$
...
???+ note "[洛谷 3128 最大流](https://www.luogu.com.cn/problem/P3128)"
    FJ 给他的牛棚的 $N(2 \leqslant N \leqslant 50,000)$ 个隔间之间安装了 $N-1$ 根管道，隔间编号从 $1$ 到 $N$。所有隔间都被管道连通了。
    
    FJ 有 $K(1 \leqslant K \leqslant 100,000)$ 条运输牛奶的路线，第 $i$ 条路线从隔间 $s_i$ 运输到隔间 $t_i$。一条运输路线会给它的两个端点处的隔间以及中间途径的所有隔间带来一个单位的运输压力，你需要计算压力最大的隔间的压力是多少。
