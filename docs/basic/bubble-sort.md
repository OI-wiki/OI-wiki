...
冒泡排序（英语：Bubble Sort）是一种简单的排序算法。这个算法的名字由来是较小的元素会经由交换慢慢“浮”到数列的顶端。
...
在序列完全有序时，冒泡排序只需遍历一遍数组，不用执行任何交换操作，时间复杂度为 $O(n)$ 。在最坏情况下，冒泡排序要执行 $\frac{(n-1)n}{2}$ 次交换操作，时间复杂度为 $O(n^2)$ 。在平均情况下，冒泡排序的时间复杂度也是 $O(n^2)$ 。
