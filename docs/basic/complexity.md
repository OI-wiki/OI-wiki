...
例如 $T(n) = 3 T\left(\frac{n}{2}\right)＋2n$，那么 $a=3, b=2, 1<\log_23<2$，那么 $\epsilon$ 可以取值为 $1$，从而满足第一种情况，所以 $T(n) = O(n^2)$。

又例如 $T(n) = T\left(\frac{n}{2}\right) + 1$, 那么 $a=1, b=2, log_21=0$，那么 $\epsilon$ 可以取值为 $1$，从而满足第二种情况，所以 $T(n) = \Theta(n^2)$。

再例如 $T(n) = T\left(\frac{n}{2}\right) + logn$，那么 $a=1, b=2, log_21=0$，那么 $k$ 可以取值为 $1$，从而满足第三种情况，所以 $T(n) = \Theta(logn)$。
