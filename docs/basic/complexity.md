...
需要注意的是，这里的第二种情况还需要满足 regularity condition, 即 $a f(n/b) <= c f(n)$，for some constant $k < 1$ and sufficiently large $n$。
...
??? note "证明过程"
    ...
    层层递推，我们可以写出类推树如下：![](./images/master-theorem-proof.png)
    ...
    $$
    case 2: 首先 g(n) = \Omega(f(n))，又因为 a f(\dfrac{n}{b}) <= c f(n)，只要 c 的取值是一个足够小的正数，且 n 的取值足够大，因此可以推导出：g(n) = O(f(n))。两侧夹逼可以得出，g(n) = \Theta(f(n))。
    $$
