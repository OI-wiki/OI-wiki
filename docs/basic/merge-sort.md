...
从左往右依次合并两个长度为 $1$ 的有序段，得到一系列长度 $\leqslant 2$ 的有序段；

从左往右依次合并两个长度 $\leqslant 2$ 的有序段，得到一系列长度 $\leqslant 4$ 的有序段；

从左往右依次合并两个长度 $\leqslant 4$ 的有序段，得到一系列长度 $\leqslant 8$ 的有序段；

……

重复上述过程直至数组只剩一个有序段，该段就是排好序的原数组。

???+ note " 为什么是 $\leqslant n$ 而不是 $= n$"
