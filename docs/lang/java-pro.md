...
|    min(BigInteger val)    |     返回 this 与 val 的较小值      auto     |
...
如果是明确了类型如 `List<Integer>` ，此时编译器会检查放入的数据类型，只能放入整数的数据。以及声明集合变量时只能使用包装类型 `ist<Integer>` 或者自定义的 `Class`，而不能是基本类型  `List<int>`。
