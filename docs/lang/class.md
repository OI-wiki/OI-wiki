...
Ps：C++ 中 **struct 关键字** 定义的也是类，上文中的 **结构体** 的定义来自 C。因为某些历史原因，C++ 保留并拓展了 **struct** 。
...
该例定义了一个名为_Edge_的类。该类拥有四个成员元素，其中_u,v_的数据类型都为_int_，_nxt_是_int_型的指针，_w_的类型为_long long_。

并在 **"}"** 后定义了一个数据类型为_Edge_的数组_e_。当然，对于某种已经存在的类型，也可以使用第十三行后中的方法进行定义 [常量](./var.md) 、 [变量](./var.md) 、 [指针](./pointer.md) 。

### 访问符

不同于 [struct](./struct.md) 中的举例，本例中出现了_public_，这属于访问符。
...
对于_struct_，它的所有成员都是默认 **public** 。对于 **class** ，它的所有成员都是默认 **private** 。
...
该类有一个打印 Edge 成员元素的函数，以及更改成员元素_w_的函数。

和 [函数](./func.md) 类似，对于成员函数，也可以先声明，在定义，如第十八行（声明处）以及二十一行后（定义处）。

如果想要调用_var_的_print_成员函数，可以使用 **"var.print()"** 进行调用。
...
该例 **重载** 了_Edge_的默认构造函数，该函数能够在我们实例化_Edge_类型变量时，将所有的成员元素初始化为 0 或者 NULL。
...
若要在为一个类变量赋值非同等类型值（常为 [内置类型](./var.md) ）时进行报错，请使用_explicit_关键字。
