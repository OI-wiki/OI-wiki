...
-  `emplace_back()` 直接在末尾构造元素，相较 `push_back()` 省去了拷贝的过程，需要 C++11。
-  `pop_back()` 删除末尾元素，常数复杂度。
-  `swap()` 与另一个容器进行交换，此操作是 **常数复杂度** 而非线性的。

???+ warning
    使用 `emplace_back()` , `push_back()` , `insert()` 等函数插入元素时，若新的 `size()` 大于 `capacity()` ，则所有迭代器和引用（包含尾后迭代器）都被非法化。否则仅位于插入点之后的迭代器被非法化。
    
    同样，当使用 `erase()` 函数删除元素时，位于擦除点或之后的迭代器，包含 `end()` 迭代器都会被非法化。
