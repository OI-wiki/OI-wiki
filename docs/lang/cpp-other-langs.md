本文介绍 C++ 与其他常用语言的区别，重点介绍 C 与 C++ 之间重要的或者容易忽略的区别。尽管 C++ 几乎是 C 的超集，C/C++ 代码混用一般也没什么问题，但是了解 C/C++ 间比较重要的区别可以避免碰到一些奇怪的 bug。如果你是以 C 为主力语言的 OIer，那么本文也能让你更顺利地上手 C++。C++ 相比 C 增加的独特特性可以阅读 [C++ 进阶](./class.md) 部分的教程。此外，本文也简要介绍了 Python, Java, Rust 和 C++ 的区别。
...
同样地，Rust 也是面向对象的语言，但是其面向对象的方式并不同。C++ 支持抽象类、多重继承等，但 Rust 是通过结构（`struct`）与特征（`trait`）来实现类似的继承与多态的。特征规定实现它的结构需要实现的行为，并通过特征对象来动态地使用实现了同一个特征的不同结构对象。此外，Rust 还同时支持过程式和函数式编程，且函数式编程会进行极大的优化，使得其与传统过程式代码无太大差别（甚至更优）。
...
```rust
fn main() {
    let mut a = 1;  // mut 关键字用于标注可变变量
    let ptr = &a as *mut i32;  // 获取可变指针
    print!("{} ", a);
    inc(ptr);
    println!("{}", a);
    // Final Output: 1 2
...
/// 这个函数需要删除，否则无法通过编译
fn inc_err(number: *mut i32) {
    *number += 1;  // Compile Error: 不允许直接操作裸指针
}

fn inc(number: *mut i32) {
    unsafe {  // 强制非安全代码，安全性由程序员自行保证
        *number += 1;
    }
}
```

所以，Rust 推荐使用编译器能够保证安全的借用，并由 Rust 借用检查器（Rust Borrow Checker）保证其安全性：
...
fn main() {
    let mut a = 1;
    print!("{} ", a);
    inc(&mut a);  // 传递可变借用。在同一时刻，多个可变借用不能同时存在，借用时原变量不能改变，不可变借用和可变借用不能同时存在
    println!("{}"， a);
    // Final Output: 1 2
}
...
正因为 Rust 的安全性，Rust 在底层编程、大型项目、硬件编程、操作系统、单片机等领域具有很大的竞争潜力，很多相关领域的组织表示考虑逐步使用 Rust 重构它们原来用 C/C++ 编写的项目。
