...
为什么 `a` 和 `b` 没有交换呢，这是因为当我们把 `a` 和 `b` 传到函数 `change` 的时候，参数 `x` 和 `y` 相当于只是把参数的值复制了一遍，修改 `x` 和 `y` 并没有改变 `a` 和 `b` 的值。
所以这里我们就可以用指针。我们可以把参数 `x` 和 `y` 改成指向 `a` 和 `b` 的指针。这样的话，当我们在函数 `change` 里面改变 `*x` 和 `*y` 就相当于改变函数外的变量 `a` 和 `b` 的值。
代码如下：
...
由于数组是在内存中连续存储的，而数组名 `a` 可以视作一个指针，所以当我们访问 `a[3]` 时，其实就是在访问这个指针后移三位的位置，等价于访问 `a+3` 指向的内容。由于 a+3=3+a，所以 `3[a]` 也是访问同一个内容。
综上所述，访问数组也可以这么写：
