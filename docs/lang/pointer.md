...
所以这里我们就需要用到指针。我们可以把参数x和y改成a和b的指针。这样的话，当我们在函数`change`里面改变`*x`和`*y`的值后，相当于我们就已经把a和b的值改变了。
代码如下:
### 代码2
...
不知道大家刚才有没有注意到,我们在将`a`和`b`传递到函数change的时候，在前面加了一个`&`符号，这个符号的作用是取出变量的地址，所以它叫取址符。
```cpp
void change(int &x, int &y){
	int t=x;
	x=y;
...
离奇的RE错误可能由指针引起，其中申请/释放部分是最容易出错。
