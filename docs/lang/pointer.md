...
数组占据一块连续的内存空间，因此若数组 `a[0]` 的编号是 `8` ，那么 `a[1]` 的编号就是 `12` ， `a[3]` 的编号就是 `20` 。 `&(a[3])=&(a[0])+3` 。
...
区别在于，malloc 是 `stdlib.h` （ `cstdlib` ）中的函数，功能是接受一个 `size_t` 类型表示分配的字节数，返回一个 类型为 `void*` 的指针，指向分配的字节，而 `new` 是一个运算符。
