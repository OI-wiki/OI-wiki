...
if...else 语句和 if 语句类似，else 不需要再写条件。两者区别在于当 if 语句的条件不满足时，会直接执行 else 里的语句。同样，当主体只有一条语句的话，可以省略花括号。
...
else if 语句是 if 和 else 的组合，对多个条件进行判断并选择不同的语句分支。在最后一条的 else 语句则不需要再写条件。例如，若条件 1 为真，执行主体 1，条件 3 为真而前面的都为假，执行主体 2，所有的布尔表达式都为假才执行主体 4。
...
switch 语句执行时，求值选择句，然后根据选择句的值选择相应的标签处开始执行。其中，选择句必须是一个整数类型表达式，而标签都必须是整数类型的常量。例如：
...
??? note: "如何理解 switch"
    在上文中，用了大量“case 分句”，“case 子句”等用语，实际上，在底层实现中，switch 相当于一组跳转语句。

    底层在求值 switch 的时候，将其中的选择句的值求出来，然后利用 je 指令跳转到对应的 case 处。也就是说，switch 可以看做是 if+goto。这也是为什么 switch 中的 case 分句需要加 break。

    也因此，有 Duff's Device 这种奇技淫巧，希望了解的人可以自行学习。
