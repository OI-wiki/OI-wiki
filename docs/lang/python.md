...
- 传入的参数有2个 maxsize 和 typed，如果不传则 maxsize 的默认值为128，typed 的默认值为 False。
- 其中 maxsize 参数表示是的被装饰的方法最大可缓存结果数量，如果是默认值128则表示被装饰方法最多可缓存128个返回结果，如果 maxsize 传入为 None 则表示可以缓存无限个结果。
