...
'OI-Wiki 感谢你的阅读\n欢迎参与贡献!'
```
在最新的Python 3版本中，字符串是以Unicode编码的，也就是说，Python的字符串支持多语言。[^ref2]在 Python 中，可以对一个 Unicode 字符使用内置函数 `ord()` 将其转换为对应的 Unicode 编码，逆向的转换使用内置函数 `chr()`。C/C++ 中 `char` 类型也可以和 对应的 ASCII 码互转。
