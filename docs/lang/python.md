...
之所以出现以上的运行结果，是因为默认参数的值仅仅在函数定义的时候赋值一次，对于可变对象（如列表、字典、集合），所有调用会共享同一个对象，lst1和 lst2实际上都指向内存中同一个默认列表对象。因此，第二次调用后，这个共享列表的内容被修改为 [12, 42]。所以函数的默认参数的值应该设为不可变对象，使用 None 占位是一种最佳实践：
