...
一个打包的 `Java` 程序（名称一般是 `*.jar` ）中可以有很多个类似的函数，但是当双击运行这个程序的时候，只有其中一个函数会被运行，这是定义在 `Jar` 的 `Manifest` 文件中的，在OI比赛中一般用不到关于它的知识。
...
如果想要导入某包下面所有的类，只需要把这个语句最后的分号前的类名换成星号 `*` 。
...
for关键字有两种使用方法，其中第一种是普通的for循环，形式如下：
