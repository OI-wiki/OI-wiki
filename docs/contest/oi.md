...
**CSP**（英文：Certified Software Professional Junior/Senior）是 NOIP 在 2019 年被取消之后，CCF 开设的非专业级软件能力认证测试，面向全年龄段。

CSP 分为入门级（Junior，简写为 CSP-J）与提高级（Senior，简写为 CSP-S）两组，赛程分为初试（一般在每年 9 月）和复试（一般在每年 10 月）两场。初试为笔试，考察计算机理论和操作常识和基本的算法与数学知识；复试为上机考试，入门组与提高组都为 4 题，其中入门组考试时间 3.5 个小时，提高组 4 个小时（CSP-S 2019 除外，该场比赛使用旧 NOIP 提高组赛制，赛程分为两天，一天 3 题 3.5 小时）。初试面向社会全体学生报名，经过一定的排名筛选后成绩优秀者有机会参加复试。

报名参加初试和复试、复试后进行题目申诉等都需要向 CCF 缴费。

初试与复试都会以省为单位按照排名对选手成绩进行评级认证，分为一、二、三等。
...
WC 的内容包括若干天的培训和一天的考试。这项考试主要用于从国家集训队（50 人）选拔国家候选队（15 人），但是前一年 NOIP 与 CSP-S 复试取得较好成绩的选手也可以参加（不参与选拔）。
