...
```latex
输入的第一行包含三个正整数 $n$, $m$, $k$ ($1\leqslant n,m\leqslant 2\cdot 10^5$, $1\leqslant k\leqslant 100$) — $n$ 表示数列的长度，$m$ 表示操作个数，$k$ 的意义见题目描述。
```

```latex
输入的第二行包含 $n$ 个非负整数 $a_1,a_2,\ldots,a_n$ ($1\leqslant a_i\leqslant 10^9$) — 题目给出的数列。
```

```latex
接下来的 $m$ 行中的第 $i$ 行包含两个正整数 $l_i$ 和 $r_i$ ($1\leqslant l_i\leqslant r_i\leqslant n$)，表示第 $i$ 次操作在区间 $[l_i,r_i]$ 上进行。
```

```latex
接下来的 $n-1$ 行，每行包含两个正整数 $u$ 和 $v$ ($1\leqslant u,v\leqslant n$)，表示 $u$ 和 $v$ 之间由一条边相连。
...
```latex
输入的第二行包含一个小数点后不超过三位的实数 $x$ ($-10^6\leqslant x\leqslant 10^6$)，意义见题目描述。
```
...
不推荐使用「百分之 XX 的数据满足 XX」的说法，尤其是数据范围有多个变量时。例如，「$30\%$ 的数据满足 $n \leqslant 1000$」和「$40\%$ 的数据满足 $m \leqslant 100$」可能描述了 $70\%$ 的数据的性质，也可能只描述了 $40\%$ 数据的性质。一般来说，subtask 或数据范围表格是更好的选择。
