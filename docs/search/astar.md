...
上述条件下，如果 $h$  **满足三角形不等式，则 A\*算法不会将重复节点加入队列** 。
...
题目大意：按顺序求一个有向图上从节点 $s$ 到节点 $t$ 的所有路径最小的前任意多（不妨设为 $k$ ）个。

很容易发现，这个问题很容易转化成用 A\*算法解决问题的标准程式。

初始状态为处于节点 $s$ ，最终状态为处于节点 $t$ ，距离函数为从 $s$ 到当前节点已经走过的距离，估价函数为从当前节点到节点 $t$ 至少要走过的距离，也就是当前节点到节点 $t$ 的最短路。

就这样，我们在预处理的时候反向建图，计算出节点 $t$ 到所有点的最短路，然后将初始状态塞入优先队列，每次取出 $f(x)=g(x)+h(x)$ 最小的一项，计算出其所连节点的信息并将其也塞入队列。当你第 $k$ 次走到节点 $t$ 时，也就算出了节点 $s$ 到节点 $t$ 的 $k$ 短路。

由于设计的距离函数和估价函数，每个状态需要存储两个参数，当前节点 $x$ 和已经走过的距离 $v$ 。

我们可以在此基础上加一点小优化：由于只需要求出第 $k$ 短路，所以当我们第 $k+1$ 次或以上走到该节点时，直接跳过该状态。因为前面的 $k$ 次走到这个点的时候肯定能因此构造出 $k$ 条路径，所以之后在加边更无必要。
