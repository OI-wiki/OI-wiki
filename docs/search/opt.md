...
???+ note "工作分配问题"
    **题目描述**
    ...
    有 $n$ 份工作要分配给 $n$ 个人来完成，每个人完成一份。第 $i$ 个人完成第 $k$ 份工作所用的时间为一个正整数 $t_{i,k}$，其中 $1 \leqslant i, k \leqslant n$。试确定一个分配方案，使得完成这 $n$ 份工作的时间总和最小。
    ...
    **数据范围**
    ...
    $1 \leqslant t_{i,k} \leqslant 10^4$
