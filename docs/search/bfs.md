...
在基于优先队列的 BFS 中，我们每次从队首取出代价最小的节点进行进一步搜索。容易证明这个贪心思想是正确的，因为从这个节点开始扩展的搜索，一定不会更新原来那些代价更高的节点。换句话说，其余那些代价更高的节点，我们不回去考虑更新它。

当然，每个节点可能会被入队多次，只是每次入队的代价不同。当该节点第一次从优先队列中取出，以后便无需再在该节点进行搜索，直接忽略即可。所以，优先队列的 BFS 当中，每个节点只会被处理一次。

相对于普通队列的 BFS，时间复杂度多了一个 $\log$ ，毕竟要维护这个优先队列嘛。不过普通 BFS 有可能每个节点入队、出队多次，时间复杂度会达到 $O(n^2)$ ，不是 $O(n)$ 。所以优先队列 BFS 通常还是快的。
