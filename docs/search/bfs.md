...
具体来说，我们用一个队列 Q 来记录要处理的结点，然后开一个 $vis[]$ 布尔数组来标记某个结点是否已经访问过了。

开始的时候，我们把起点 s 以外的结点的 vis 值设为 0，意思是没有访问过。然后把起点 s 放入队列 Q 中。
...
在实现 BFS 的时候，我们把未被访问过的结点放在一个称为 open 的容器中，而把已经访问过了的结点放在 closed 容器中。
