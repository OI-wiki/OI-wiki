...
打开 ST 后键入<kbd>Crtl</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd>唤出命令框，输入 `Install` 后回车（完整命令是 `Install Package Control`，不区分大小写），此时应该会看到左下角有一个 `=` 在不停的左右移动。Package Control 安装完成后会有弹窗提示，具体的加载时间取决于网络。
...
Vim 作为一款编辑器，上手之后效率极高。但 Vim 的界面并不太友好，而且部分功能对于现代编辑器确实有些薄弱。

如果是希望使用 Vim 但又需要快速提升效率，ST 可以作为一个中间过渡。

#### 开启方式

ST 的 Vim 模式由自带的插件 Vintage 提供，它默认是禁用的。可以通过 `首选项->设置` 在用户设置中，将 `"ignored_packages"` 一项中的 Vintage 删除并保存（不要整个删除，只删除 Vintage），ST 的状态栏左边就会出现 `INSERT MODE`，此时 Vintage 插件已开启。
...
ST 的 Vintage 插件与 Vim 有一定不同，部分列于此处：
...
[^ref1]: [NOI Linux 2.0发布](https://www.noi.cn/gynoi/jsgz/2021-07-16/732450.shtml)
