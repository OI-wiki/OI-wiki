...
打开 ST3 后键入<kbd>Crtl</kbd>+<kbd>Shift</kbd>+<kbd>P</kbd>唤出命令框，输入 `Install` 后回车（完整命令是 `Install Package Control`，不区分大小写），此时应该会看到左下角有一个 `=` 在不停的左右移动。Package Control 安装完成后会有弹窗提示，具体的加载时间取决于网络。
...
输入 `Chinese` 选择 ChineseLocalizationgs 并回车，等待安装完毕，完成后界面会自动切换为中文（如是 ST4，因为汉化插件未更新，会少一些新增的菜单项，但一般不影响）。

### 调整字体

进入 `首选项->设置`，在右边的用户设置中的花括号中添加一行 `"font_face": "字体名",`，ST3 的设置使用 JSON 格式储存。修改完成后保存，如果系统安装了对应字体会自动切换。
...
例如，这里给出 BracketHighlighter 的一种比较实用的设置，在 `首选项->Package Setting->BracketHighlighter->Bracket Setting` 中修改：
...
例如，如果要修改自动补全，可以打开安装目录下（注意是 ST3 自身的安装目录，非插件目录）`\Package\C++` 插件中的 `Snippets\*.sublime-snippet` 文件修改，如要增添自动补全，可以安装 C++ Snippets 并在其中修改或添加文件（或新建一个插件，但不能直接添加进自带的 C++ 插件，否则无法被识别）。保存任何修改时记得关闭 ST3。
...
按右下角的 `Plain Text` 然后修改为需要的语言即可。在 `视图->语法` 中也可以设置。

![](images/sublime3-2.png)

### 快捷键

ST3 有复合快捷键，如<kbd>Ctrl</kbd>+<kbd>K</kbd>&<kbd>Ctrl</kbd>+<kbd>Backspace</kbd>表示先按<kbd>Ctrl</kbd>+<kbd>K</kbd>再按<kbd>Ctrl</kbd>+<kbd>Backspace</kbd>。
...
五个查找选项分别为正则表达式（开启后可以直接输入正则表达式）、大小写敏感、全字匹配、循环查找、在选段中查找。
...
尝试在 ST3 中键入一些内容，并直接把整个 ST3 关闭，ST3 会直接关闭且没有任何提示。这时再打开 ST3 不会丢失任何内容，即使重启电脑也不会。
...
ST3 默认的编译选项为 `g++ "${file}" -o "${file_path}/${file_base_name}"`，如果要修改编译选项，可以新建一个编译系统。
...
如果编译时选择 `C++ Single File - Run`（即编译后运行），那么在下方弹出的编译信息窗口应该不会有任何显示（除非编译错误），因为 ST3 的编译信息窗口同时也是一个 CMD 终端，此时它在等待输入数据。

运行结束后会提示程序的运行时间。

??? note "为什么感觉时间多了？"
    注意这个时间是从按下<kbd>Ctrl</kbd>+<kbd>B</kbd>到全部 CMD 命令运行结束的时间，也就是说包括编译的时间和输入的时间，以及如果在外部 CMD 运行还包括 CMD 开启关闭的时间。
