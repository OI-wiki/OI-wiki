...
???+note "注意"
    内置终端依赖了KDE的 Konsole[^ref1]，而 Konsole 为 *nix 独有包。也就是说，Windows 下该特性不可用。
...
[^ref1]: [Arch Linux中对该包的描述](https://archlinux.org/packages/extra/x86_64/kate/)中，其可选依赖了`konsole`，描述为`open a terminal in Kate`(在Kate中打开一个终端)。