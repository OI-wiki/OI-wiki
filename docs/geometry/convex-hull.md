...
2.  两个凸集的闵可夫斯基和的凸包是对原来两个集合的凸包的各边重排序。

    ??? note "证明"
        设两个凸包（及其所围成的凸集） $A = \{a_1,a_2,\cdots,a_n\},B = \{b_1,b_2,\cdots,b_m\}$。

        则 $A+B=\{a_i+b_j | 1\le i\le n,1\le j\le m\}$。

        对于 $A$ 的每条边 $(a_i,a_{i+1})$ 和 $B$ 的每条边 $(b_j,b_{j+1})$，都有 $(a_i+b_j,a_{i+1}+b_{j+1})\in A+B$。
