...
点集 $A$ 和点集 $B$ 的闵可夫斯基和 $A+B$ 定义为 $A+B=\{a+b|a\in A,b\in B\}$，即把点集 $B$ 中的每个点看做一个向量，将点集 $A$ 中每个点沿这些向量平移，最终得到的结果的集合就是点集 $A+B$。此处仅讨论 **凸包** 的闵可夫斯基和。
...
1.  若点集合 $A$，$B$ 为凸集，则其闵可夫斯基和 $A+B$ 也是凸集。

    ??? note "证明"
        考虑凸集的性质，若 $a,b\in A$，则 $c=xa+(1-x)b,c \in A(0\le x \le 1)$。

        设 $a_1,a_2 \in A,b_1,b_2 \in B$，根据定义，$a_1+b_1,a_2+b_2 \in A+B$。

        对于任意的 $x$ 我们有：

        $$
        \begin{aligned} &x(a_1+b_1) + (1-x)(a_2+b_2) \\ =& x(xa_1+(1-x)a_2) + (1-x)(xb_1+(1-x)b_2) \\ \end{aligned}
        $$
...
2.  若点集 $A$，$B$ 为凸集，则其闵可夫斯基和 $A+B$ 的边集是由凸集 $A$，$B$ 的每条边组成的。

    ??? note "证明"
        形象化的表示闵可夫斯基和，是把凸集 $B$ 中的一个点限制在 $A$ 的边界上运动一圈后形成的，下面以 **逆时针** 转动为例。

        因为 $B$ 是做逆时针运动，那么 $B$ 中最外侧的点（即从 $B$ 当前所在的 $A$ 上的边与 $B$ 相切的点） 也是逆时针变换的。

        当它在 $A$ 的 **边** 上运动时，最外侧的点扫过的路径就对应 $A$ 的一条边，保留在了 $A+B$ 中。

        当它在 $A$ 的 **点** 上时，若最外侧的点从 $B_i$ 变到了 $B_{i+1}$ 那么 $B$ 的一条边 $[B_i,B_{i+1}]$ 就保留在了 $A+B$ 中。

        由于 $B$ 会逆时针绕 $A$ 转动一圈，最外侧的点会在 $B$ 上变动一圈后回到最开始的点。

        证毕。

### 实现

这个算法的时间复杂度是 $O(n^2)$ 的，但在应用时，我们往往只关注 **凸包** 上的点。

所以，我们可以根据性质 2，将凸集 $A,B$ 极角排序，得到它们在 $A+B$ 上的出现顺序，把 $a_1+b_1$ 看做 $A+B$ 的起点，然后用类似归并的做法依次放边即可。
