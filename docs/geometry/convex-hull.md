...
1.  若点集合 $P$，$Q$ 为凸集，则其闵可夫斯基和 $P+Q$ 也是凸集。
    ...
    ??? note "证明"
        设 $e,f\in P+Q$，有 $a,b \in P$，$c,d\in Q$ 且 $e=a+c,f=b+d$，则有：
...
2.  若点集 $P$，$Q$ 为凸集，则其闵可夫斯基和 $P+Q$ 的边集是由凸集 $P$，$Q$ 的边按极角排序后连接的结果。
    ...
    ??? note "证明"
        将坐标系进行旋转，使得 $P$ 上的一条边 $XY$ 与 $x$ 轴平行且在最下方。
        
        设此时 $Q$ 中最低的点 $U$，$P+Q$ 的 **最低** 且 **靠左** 的点 $A$。

        可知 $\vec{A} = \vec{X} + \vec{U}$，所以 $A$ 必然在 $S$ 的边界上。

        同理，$A+B$ 中 **最低** 且 **靠右** 的点 $\vec{B} = \vec{Y} + \vec{U}$，也必然在 $S$ 的边界上。

        因此，由 $\vec{AB} = \vec{XY} + \vec{U}$。

        若按顺序进行旋转，则结果连续的构成了 $A+B$ 中的每条边。

        证毕。

### 实现

我们可以根据性质 2，将凸集 $P,Q$ 极角排序，得到它们在 $P+Q$ 上的出现顺序，把 $a_1+b_1$ 看做 $P+Q$ 的起点，然后用类似 **归并** 的做法依次放边即可。
