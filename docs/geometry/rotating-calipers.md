...
求出凸包后的数组自然地是按照逆时针旋转的顺序排列，不过要记得提前将最左下角的 1 结点补到数组最后，这样在挨个枚举边 $(i,i+1)$ 时，才能把所有边都枚举到。
...
???+ note "核心代码"
    === "C++"
        ```cpp
        int sta[N], top;  // 将凸包上的结点编号存在栈里，第一个和最后一个结点编号相同
    ...
    === "Python"
        ```python
        sta = [0] * N
        top = 0  # 将凸包上的结点编号存在栈里，第一个和最后一个结点编号相同
