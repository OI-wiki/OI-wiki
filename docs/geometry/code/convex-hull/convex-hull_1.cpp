...
vector<Point<T>> convex_hull(vector<Point<T>> p) {
  assert(!p.empty());
  sort(p.begin(), p.end(), [](const auto &a, const auto &b) { return a.x < b.x; });
...
vector<Point<T>> minkowski_sum(vector<Point<T>> a, vector<Point<T>> b) {
  ...
  merge(a.begin(), a.end(), b.begin(), b.end(), c.begin() + 1, [](const auto &a, const auto &b) { return (a ^ b) < 0; });
...
int main() {
  ...
  while (q--) {
    ...
    auto it = upper_bound(a.begin() + 1, a.end(), v, [](const auto &a, const auto &b) { return (a ^ b) < 0; });
