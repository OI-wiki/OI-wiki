...
$$
\text{hash}(s)=\text{hash}(t)
$$

否则这两个字符串无法比较。

注意，不必保持相反的方向。如果哈希值相等，字符串也不一定相等。我们把这种条件称为是单侧错误，这是因为两个完全不同的字符串可能具有相同的哈希值。
...
若进行 $n$ 次比较，每次率 $\frac 1 M$ ，那么总冲突率是 $1-\left(1-\frac 1 M\right)^n$ 。在随机数据下，若 $M=10^9 + 7$ , $n=10^6$ ，冲突率约为 $\frac 1{1000}$ ，并不是能够完全忽略不计的。
...
为了解决这个问题，我们遍历了所有子串的长度 $l=1,\dots ,n$ 。对于每个长度为 $l$ ，我们将其哈希值乘以相同的幂 $b$ ，并存入一个数组中……数组中不同元素的数量等于字符串中长度不同的子串的数量，并此数字将添加到最终答案中。
...
字符串个数不超过 $10 ^ 5 $ ，总长不超过 $10 ^ 6 $ 。
    ...
    ? ? ? mdui - shadow -
              6 "题解" 每次需要求最长的、是原答案串的后缀、也是第 $i$
                  个串的前缀的字符串。枚举这个串的长度，哈希比较即可。
