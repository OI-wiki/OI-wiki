...
通过哈希同样可以 $O(n)$ 解决这个问题，具体方法就是记 $R[i]$ 表示以 $i$ 作为结尾的最长回文的长度，那么答案就是 $\max_{i=1}^nR[i]$。考虑到 $R[i]\leq R[i-1]+2$，因此我们只需要暴力从 $R[i-1]+2$ 开始递减，直到找到第一个回文即可。记变量 $z$ 表示当前枚举的 $R[i]$，初始时为 $0$，则 $z$ 在每次 $i$ 增大的时候都会增大 $2$，之后每次暴力循环都会减少 $1$，故暴力循环最多发生 $2n$ 次，总的时间复杂度为 $O(n)$。
