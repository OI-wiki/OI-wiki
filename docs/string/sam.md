...
**本文版权协议为经过修改的 CC-BY-SA 4.0，与原协议不同之处在于，这个协议禁止任
何人（包括译者自己）转载文章到洛谷（即[洛谷](https://www.luogu.org/)以及其直接
控制的地方，包括但不限于洛谷博客、讨论区、题解等），而其它网站不受此限制。**
...
转移的个数是线性规模的，和通常情况下线性级别的算法运行时间还不清楚，这些会在证
明正确性之后证明。
...
最后一步我们将一些到 $q$ 转移重定向到 $clone$。我们需要修改哪些转移呢？只重定向
...
每个状态 $v$ 我们想要找到第一次出现这个状态的末端的位置 $furstpos[v]$。换句话
...
动机呢？对应于以 $P$ 为后缀所有字符串。换句话说我们要找到所有可以通过后缀链接到
达状态 $t$ 的所有状态。
...
个状态记录标记 `is_cloned`，我们就可以简单地忽略掉被复制的状态，只输出其它所有
状态的 $firstpos$ 的值。
...
缀链接后我们处在的最长字符串对应于一个子串。
 - 如果仍然没有使用被要求的转移，我们继续重复经过后缀链接并减小 $l$，直到我们找
到一个转移或到达虚拟状态 $-1$（这意味着字符 $T[i]$ 根本没有在 $S$ 中出现过，所
所我们设置 $v=l=0$）。
