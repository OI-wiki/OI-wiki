...
直观上，字符串的 SAM 可以理解为给定字符串的 **所有子串** 的压缩形式。值得注意的事实是，SAM 将所有的这些信息以高度压缩的形式储存。对于一个长度为 $n$ 的字符串，它的空间复杂度仅为 $O(n)$。而且，构造 SAM 的时间复杂度也仅为 $O(n)$。准确地说，一个 SAM 最多有 $2n-1$ 个节点和 $3n-4$ 条转移边。
...
-   每个 **转移** 都标有一些字符。从一个结点出发的所有转移均 **不同**。
-   存在一个或多个 **终止状态**。如果我们从初始状态 $t_0$ 出发，最终转移到了一个终止状态，则路径上的所有转移的标号连接起来一定是字符串 $s$ 的一个后缀。$s$ 的每个后缀均可用一条从 $t_0$ 到某个终止状态的路径构成。
-   在所有满足上述条件的自动机中，SAM 的结点数是最少的。

### 子串和路径

SAM 最简单、也最重要的性质是，它包含关于字符串 $s$ 的所有子串的信息。任意从初始状态 $t_0$ 开始的路径，如果我们将路径上的所有转移的标号写下来，都会形成 $s$ 的一个 **子串**。反之，每个 $s$ 的子串对应从 $t_0$ 开始的某条路径。

为了简化表达，我们称子串 **对应** 一条（从 $t_0$ 出发且它上面所有转移的标号构成这个子串的）路径。反过来，我们说任意一条路径 **对应** 它的标号构成的字符串。
...
一个事实是，每个这样的等价类都对应 SAM 的一个状态[^state-endpos]。也就是说，SAM 中的每个非初始状态都对应一个或多个 $\operatorname{endpos}$ 相同的非空子串。换句话说，SAM 中的状态数等于所有非空子串的等价类的个数，再加上初始状态。
...
???+ note "引理 1"
    字符串 $s$ 的两个非空子串 $u$ 和 $w$（假设 $\left|u\right|\le \left|w\right|$）的 $\operatorname{endpos}$ 相同，当且仅当字符串 $u$ 在 $s$ 中的每次出现，都是以 $w$ 后缀的形式存在的。
...
考虑 SAM 中某个不是初始状态 $t_0$ 的状态 $v$。我们已经知道，状态 $v$ 对应于具有相同 $\operatorname{endpos}$ 的子串等价类。我们如果定义 $w$ 为这些字符串中最长的一个，则所有其它的字符串都是 $w$ 的后缀。
...
???+ note "引理 5"
    通过 $\operatorname{endpos}$ 集合构造的树（规定每个子节点的 $\operatorname{endpos}$ 集合都包含在父节点的 $\operatorname{endpos}$ 集合中）与通过后缀链接 $\operatorname{link}$ 构造的树相同。
...
在讨论算法本身前，我们总结一下之前学过的知识，并引入一些辅助记号。

-   $s$ 的子串可以根据它们的结束位置集合 $\operatorname{endpos}$ 划分为多个等价类；

-   SAM 由初始状态 $t_0$ 和与每一个（非空子串的）$\operatorname{endpos}$ 等价类对应的每个状态组成；

-   对于每一个状态 $v$，一个或多个子串与之匹配。我们记 $\operatorname{longest}(v)$ 为其中最长的一个字符串，记 $\operatorname{len}(v)$ 为它的长度。类似地，记 $\operatorname{shortest}(v)$ 为最短的子串，它的长度为 $\operatorname{minlen}(v)$。那么对应这个状态的所有字符串都是字符串 $\operatorname{longest}(v)$ 的不同的后缀，且所有字符串的长度恰好取遍区间 $[\operatorname{minlen}(v),\operatorname{len}(v)]$ 中的每一个整数。
...
-   现在我们进行如下流程：从状态 $\textit{last}$ 开始，如果还没有标号为字符 $c$ 的转移，我们就添加一个到状态 $\textit{cur}$ 的转移，并沿后缀链接移动。如果过程中某个状态已经存在到字符 $c$ 的转移，我们就停下来，并将这个状态标记为 $p$。
...
为了进一步理解算法的操作，可以从 $\operatorname{endpos}$ 集合的角度理解状态和转移的变化。

-   设字符串 $s$ 长度为 $i=\operatorname{len}(last)$，在添加新的字符 $c$ 后，$\operatorname{endpos}$ 集合可能发生的变化就只有增加了新位置 $i$ 这一种可能。（下标从 $0$ 开始）

    而且，这些发生变化的状态一定对应 $t+c$ 的形式的字符串，其中的 $t$ 是 $s$ 的某个后缀。所以，只需要令 $p$ 初始为 $last$，沿着后缀链接移动，就能找到 $s$ 的所有后缀对应的状态，再考察状态 $p$ 沿着字符 $c$ 的转移即可。这样一定能处理到所有可能发生变化的状态。

-   如果字符串 $t+c$ 原来的 $\operatorname{endpos}$ 集合为空集，即不存在这样的状态，则该集合变成 $\{i\}$ 就相当于新建了状态 $\textit{cur}$。状态 $cur$ 对应的字符串 $t+c$ 只在位置 $i$ 处出现过，这就意味着字符串 $t$ 对应的状态，没有经由字符 $c$ 的转移。

    这就是算法最开始的操作：将 $p$ 沿着后缀链接移动，如果没有经由 $c$ 的转移，就建立它，并指向新的状态 $cur$。

-   如果字符串 $t+c$ 原来的 $\operatorname{endpos}$ 集合不为空集，就说明 $t+c$ 之前已经在 $s$ 中出现过。设字符串 $t+c$ 在 $s$ 的 SAM 中对应的状态是 $q$。因为 $q$ 可能对应着多个字符串，而添加了字符 $c$ 后，未必每个字符串的结束位置都增加了新位置 $i$。也就是说，同一个旧的状态 $q$ 中对应的字符串的 $\operatorname{endpos}$ 集合的变化未必是一致的。

    当然，变化只有两种可能，即增加新位置 $i$ 和不增加新位置 $i$，因此单个旧状态至多分裂成两个新状态。

    而且，只要字符串 $t+c$ 可以结束在新位置 $i$，那么对于所有 $t$ 的后缀 $u$，字符串 $u+c$ 都一定可以结束在新的位置 $i$。这说明，从字符串 $t+c$ 对应的状态出发，沿着后缀链接移动找到的所有状态，它们对应的所有字符串的 $\operatorname{endpos}$ 集合都同样地增加了一个新位置 $i$。因此，这些状态都不会分裂。

    前面已经说明，发生变化的字符串 $t+c$ 一定都是 $s+c$ 的后缀，因此都可以通过后缀链接找到。如果过程中，有某个状态分裂，就说明它对应的一部分字符串的结束位置增加，也就说明继续沿着后缀链接移动，经过的状态都将不再分裂。因此，分裂至多发生一次。

-   算法之前将 $p$ 沿着后缀链接移动，直到找到当前状态经由 $c$ 的转移为止。如果没有这样的状态，就是算法中的 **情况一**，最为简单。

    否则，转移指向的状态 $q$ 就是可能分裂的旧状态。因为 $p$ 经过的状态对应的字符串 $t$ 都是 $s$ 的后缀，所以由 $p$ 经 $c$ 转移到的状态 $q$ 一定对应着 $t+c$，而字符串 $t+c$ 的 $\operatorname{endpos}$ 集合一定会新增位置 $i$。这就说明，$q$ 对应的字符串一定有一部分，结束位置变多了。

    因此，要么它不分裂，所有字符串结束位置都变多；要么它分裂，一部分字符串结束位置变多，一部分字符串结束位置不变。而且，无论它分不分裂，之后再沿着后缀链接移动，找到的状态 $p$ 经由 $c$ 转移到的状态，就都不用分裂了。

-   那怎么判断是否需要分裂呢？因为对于 $p$ 对应的字符串 $t$，字符串 $t+c$ 结束位置都增加了。所以，只需要看 $q$ 中有没有字符串不具有 $t+c$ 的形式即可，其中，$t$ 必须对应状态 $p$。怎么看呢？因为结束位置相同，所以只需要比较长度即可。状态 $p$ 对应的字符串 $t$ 最长为 $\operatorname{len}(p)$，故而它们相应的 $t+c$ 最长就是 $\operatorname{len}(p)+1$；同时，状态 $q$ 对应的字符串最长为 $\operatorname{len}(q)$。所以，状态 $q$ 存在结束位置不会增加的字符串，当且仅当 $\operatorname{len}(q)>\operatorname{len}(p)+1$。

    如果 $\operatorname{len}(q)=\operatorname{len}(p)+1$，则状态 $q$ 不会分裂，此即 **情况二**；否则，状态 $q$ 会分裂，需要为结束位置没有增加的元素和结束位置增加的元素分别创建一个节点，此即 **情况三**。

    情况二容易处理，因为无需调整经由 $c$ 指向 $q$ 的转移；情况三则稍显复杂。因为，我们为那些结束位置增加的字符串复制了新状态 $clone$。设 $t$ 为 $\operatorname{longest}(p)$，也就是说，$t+c$ 是 $q$ 对应的字符串中结束位置增加的最长的那个。现在，根据上面的讨论，需要把状态 $q$ 中 $t+c$ 的后缀都挪到新状态 $clone$ 中。为此，只需要继续将 $p$ 沿着后缀链接移动，就能找到所有这样的 $t$ 的后缀，再将那些原本指向 $q$ 的调整到 $clone$ 即可。

### 线性时间复杂度

首先我们假设字符集大小为 **常数**。如果字符集大小不是常数，SAM 的时间复杂度就不是线性的。从一个结点出发的转移存储在支持快速查询和插入的平衡树中。因此如果我们记 $\Sigma$ 为字符集，$\left|\Sigma\right|$ 为字符集大小，则算法的渐进时间复杂度为 $O(n\log\left|\Sigma\right|)$，空间复杂度为 $O(n)$。然而如果字符集足够小，可以不写平衡树，以空间换时间将每个结点的转移存储为一个长度为 $\left|\Sigma\right|$ 的数组（用于快速查询）和一个动态列表（用于快速遍历所有可用关键字）。这样算法的时间复杂度[^time-complexity]为 $O(n)$，空间复杂度为 $O(n\left|\Sigma\right|)$。

所以我们将认为字符集的大小为常数，即每次对一个字符搜索转移、添加转移、查找下一个转移这些操作的时间复杂度都为 $O(1)$。
...
正如之前提到的一样，如果你用内存换时间（空间复杂度为 $O(n\left|\Sigma\right|)$，其中 $\left|\Sigma\right|$ 为字符集大小），你可以在 $O(n)$ 的时间内构造字符集大小任意的 SAM。但是这样你需要为每一个状态储存一个大小为 $\left|\Sigma\right|$ 的数组（用于快速根据字符找到相应的转移）以及一个包含所有可用转移的列表（用于快速遍历所有可用的转移）。
...
引理 4 中提及，所有状态和所有后缀链接构成根为 $t_0$ 的根向树，这个树也称为 **后缀链接树**。
