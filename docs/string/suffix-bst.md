...
后缀之间的大小由字典序定义，后缀平衡树就是一个维护这些后缀顺序的平衡树，即字符串 $T$ 的后缀平衡树是 $T$ 所有后缀的有序集合。后缀平衡树上的一个结点相当于原字符串的一个后缀。
...
记后缀平衡树维护的集合为 $X$，当前添加的后缀为 $S$，则添加下一个后缀就是向 $X$ 中加入 $\texttt{c}S$（亦可理解为后缀平衡树维护的字符串为 $S$，下一步往 $S$ 前加入一个字符 $\texttt{c}$）。这一操作其实就是向平衡树中插入结点。
...
根据做法 2，如果能够 $O(1)$ 判断平衡树中两个结点之间的大小关系，那么就可以在 $O(n \log n)$ 的时间内完成后缀平衡树的构造。

记 $val_i$ 表示结点 $i$ 的值。如果在建平衡树时，每个结点多维护一个标记 $tag_i$，使得若 $tag_i > tag_j \iff val_i > val_j$，那么就可以根据 $tag_i$ 的大小 $O(1)$ 判断平衡树中两个结点的大小。

不妨令平衡树中每个结点对应一个实数区间，令根结点对应 $(0, 1)$。对于结点 $i$，记其对应的实数区间为 $(l, r)$，则 $tag_i = \frac{l + r}{2}$，其左子树对应实数区间 $(l, tag_i)$，其右子树对应实数区间 $(tag_i, r)$。易证 $tag_i$ 满足上述要求。
...
类似于插入操作，借助平衡树的删除结点操作可以完成删除 $\texttt{c}S$ 的操作。
