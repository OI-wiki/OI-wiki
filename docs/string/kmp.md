author: Ir1d, LeoJacob, Xeonacid, greyqz, StudyingFather, Marcythm, minghu6, Backl1ght, SingerCoder
...
换一个角度来讲，如果你对 AC 自动机的 fail 树有一定了解，不难发现这里的答案其实就是子树大小。
...
构造字符串 $t = s + c$ 并将其反转得到字符串 $t^{\sim}$。现在我们的任务变为计算有多少 $t^{\sim}$ 的前缀未在 $t^{\sim}$ 的其余任何地方出现。利用上一部分的算法，在 $O(n)$ 时间复杂度内计算出每个前缀在字符串中的出现次数，其中出现次数为 1 的前缀即为新增加的本质不同子串。
