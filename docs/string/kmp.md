...
如果我们找到了这样的长度 $j$，那么仅需要再次比较 $s[i + 1]$ 和 $s[j]$。如果它们相等，那么就有 $\pi[i + 1] = j + 1$。

观察上图可以发现，因为 $s[0\dots \pi[i]-1] = s[i-\pi[i]+1\dots i]$，所以对于 $s[0\dots i]$ 的第二长度 $j$，有这样的性质：

$$
\overbrace{\underbrace{s_0 ~ s_1}_j ~ \dots ~\underbrace{s_{\pi[i]-j} ~ s_{\pi[i]-1}}_j}^{\pi[i]}
$$
...
也就是说 $j$ 的取值就是子串 $s[\pi[i]-1]$ 的“最长相等真前缀与真后缀”的长度，即子串 $s[\pi[i]-1]$ 的前缀函数值，$j = \pi[\pi[i]-1]$。

继续比较，当 $s[i + 1] \neq s[j]$ 时，需要找到次于 $j$ 的第二长度 $j^{(2)}$。显然，$j^{(2)}$ 的取值等价于 $s[j-1]$ 的前缀函数值，$j^{(2)}=\pi[j-1]$。由此我们可以得到一个关于 $j$ 的状态转移方程：$j^{(n)}=\pi[j^{(n-1)}-1], \ \ (j^{(n-1)}>0)$
