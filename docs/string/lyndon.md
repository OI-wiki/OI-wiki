...
Lyndon 串：对于字符串 $s$ ，如果 $s$ 的字典序严格小于 $s$ 的所有后缀的字典序，我们称 $s$ 是简单串，或者 **Lyndon 串** 。举一些例子， $\texttt{a}$ , $\texttt{b}$ , $\texttt{ab}$ , $\texttt{aab}$ , $\texttt{abb}$ , $\texttt{ababb}$ , $\texttt{abcd}$ 都是 Lyndon 串。如果 $s$ 是 Lyndon 串，当且仅当 $s$ 的字典序严格小于它的所有非平凡的循环同构串。
