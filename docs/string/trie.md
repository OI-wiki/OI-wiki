...
可以发现，这棵字典树用边来代表字母，而从根节点到树上某一节点的路径就代表了一个字符串。举个例子， $1\to4\to 8\to 12$ 表示的就是字符串 `caa` 。

Trie 的结构非常好懂，我们用一个二维数组 $tr[i,j]$ 表示节点 i 的 j 字符指向的下一个节点，或着说是节点 i 代表的字符串后面添加一个字符 j 形成的字符串的节点。（j 的取值和字符集大小有关，不一定是 $0\sim 26$ ）
...
struct trie {
  ...
  void insert(char *s, int l) {  // 插入字符串
    ...
    for (int i = 0; i < l; i++) {
      int c = s[i] - 'a';
      if (!nex[p][c]) nex[p][c] = ++cnt;  // 如果没有，就添加节点
      p = nex[p][c];
    }
    exist[p] = 1;
  }
