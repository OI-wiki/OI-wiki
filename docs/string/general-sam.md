...
由于在普通后缀自动机上，其前一个结点的 `len` 值为固定值，即为 `last` 结点的 `len`。但是在广义后缀自动机中，插入的队列是一个不严格递增的数列。所以对于每一个值，对于它的 `last` 应该是已知而且固定的，在字典树上，即为其父亲结点。

由于在字典树中，已经建立了一个近似的后缀自动机，所以只需要对整个字典树的结构进行一定的处理即可转化为广义后缀自动机。我们可以按照前面提出的队列顺序来对整个字典树上的每一个结点进行更新操作。最终我们可以得到广义后缀自动机。
...
1.  将所有字符串插入到字典树中
2.  从字典树的根结点开始进行 BFS，记录下顺序以及每个结点的父亲结点
3.  将得到的 BFS 序列按照顺序，对每个结点在原字典树上进行构建，注意不能将 `len` 小于当前 `len` 的数据进行操作

### 对操作次数为线性的证明

由于仅处理 BFS 得到的序列，可以保证字典树上所有结点仅经过一次。

对于最坏情况，考虑字典树本身结点个数最多的情况，即任意两个字符串没有相同的前缀，则结点个数为 $\sum_{i=1}^{k}|S_i|$，即所有的字符串长度之和。
...
-   由于整个 BFS 的过程得到的顺序，其父结点始终在变化，所以并不需要保存 `last` 指针。
-   插入操作中，`int cur = next[last][c];` 与正常后缀自动机的 `int cur = tot++;` 有差异，因为我们插入的结点已经在树型结构中完成了，所以只需要直接获取即可
-   在 `clone` 后的数据拷贝中，有这样的判断 `next[clone][i] = len[next[q][i]] != 0 ? next[q][i] : 0;` 这与正常的后缀自动机的直接赋值 `next[clone][i] = next[q][i];` 有一定差异，此次是为了避免更新了 `len` 大于当前结点的值。由于数组中 `len` 当且仅当这个值被 BFS 遍历并插入到后缀自动机后才会被赋值
...
可以根据后缀自动机的性质得到，以点 $i$ 为结束结点的子串个数等于 $len[i] - len[link[i]]$
...
我们需要对每个结点建立一个长度为 $k$ 的数组 `flag`（对于本题而言，可以仅为标记数组，若需要求出此子串的个数，则需要改成计数数组）

在字典树插入字符串时，对所有结点进行计数，保存在当前字符串所在的数组

然后按照 `len` 递减的顺序遍历，通过后缀链接将当前结点的 `flag` 与其他结点的合并

遍历所有的结点，找到一个 `len` 最大且满足对于所有的 `k`，其 `flag` 的值均为非 $0$ 的结点，此结点的 $len$ 即为解
