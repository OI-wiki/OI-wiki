...
注意到 $f$ 的转移与 $i$ 无关，可以认为一次转移是乘上了一个矩阵，即 $f{i+1}=f_iM$。由于 $M$ 的最小零化多项式次数不超过 $V$，所以 $f$ 的最短递推式长度也不超过 $V$，故 $\Pr[t>i]=\sum_{j=1}^{n-1}f(i,j)$ 的最短递推式长度也不超过 $V$。我们可以在 $O(VE)$ 的时间求出 $\Pr[t>0],\Pr[t>1],\cdots,\Pr[t>3V]$，然后使用*Berlekamp–Massey*算法，在 $O(V^2)$ 的时间内求解出 $\Pr[t > i]$ 的最短递推式。
...
这样，我们就在 $O(VE+V^2)$ 的时间复杂度内解决了本题。如果图 $G$ 的点数与边数同阶，本题中时间复杂度可以认为是 $O(V^2)$。
...
所以，通过引入稳态分布，我们可以在 $O(V^3)$ 的时间内求解 $G$。
