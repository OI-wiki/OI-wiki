...
$$
\sum_{k \geqslant 0} k \times\left(P^k\right)_{s, t}
$$
...
???+ note " 例题 1 [Circles of Waiting](https://codeforces.com/problemset/problem/963/E)"
    ...
    求期望经过多少时间它会移动到一个离原点的欧几里得距离大于 $R$ 的位置。$0 \leqslant R \leqslant 50$，$p_1, p_2, p_3, p_4>0$，答案对 $10^9+7$ 取模。
...
f(i, j)=
\begin{cases}
p_1 f(i-1, j) + p_2 f(i, j-1) + p_3 f(i+1, j) + p_4 f(i, j+1) + 1 & i^2 + j^2 \leqslant R^2 \\
0 & i^2 + j^2 > R^2
\end{cases}
$$
...
但主元法还可以做类似于网格图的转移方程的消元，例如 $f(i, j)=p_1 f(i+1, j)+p_2 f(i, j+1)+p_3 f(\operatorname{pre}(i, j))+1$，其中 $\operatorname{pre}(i, j)=(x, y)(x \leqslant i, y \leqslant j)$ 是问题给定的值，而直接消元法的复杂度分析在这种模型中并不适用。
...
???+ note "例题 2 Expected Value"
    给定一张简单无向连通稀疏图 $G=(V, E)$，有一枚棋子起始被放在 $v_1$，每秒棋子会从与当前点相连的边中等概率选择一条走到出边指向的点，求到达 $v_n$ 的期望时间。$n \leqslant 2000$，答案对 $p$ 取模，$p$ 是在区间 $\left[10^9, 1.01 \times 10^9\right]$ 内随机生成的一个质数。
...
注意到，期望走的时间 $E(t)=\sum_{i\geqslant0}\Pr[t>i]$，如果我们能求出走了 $i$ 步还没有结束的概率，对所有 $i ≥ 0$ 求和即为答案。
...
回到原问题，由于我们能求出 $\Pr[t > i]$ 的最短递推式，则我们可以求出 $C(x)$ 和 $A_0(x)$（定义与上一段相同），移项得 $A(x)=\frac{A_0(x)}{1-C(x)}$。我们要求的是 $\sum_{i\geqslant0}[x^i]A(x)$，不难发现这个值就等于 $A(1)$，将 $x = 1$ 带入原问题求解即可。由于模数是随机质数，可以认为分母不会为 $0$。
...
$$
f_{i,j}=1+\sum_{1\leqslant k\leqslant n}p_{i,k}f_{k,j}
$$

当 $i = j$ 时，记 $g_i$ 表示从 $i$ 开始随机游走，第一次回到 $i$ 的期望时间，那么：

$$
f_{i,i}=1-g_i+\sum_{1\leqslant k\leqslant n}p_{i,k}f_{k,i}
$$
