...
将可平面图的问题转化到对偶图上，有时更容易解决．一个典型的例子是，可平面图 [最小割](./flow/min-cut.md) 问题可以转化为对偶图 [最短路](./shortest-path.md) 问题．设 $G$ 是带边权的可平面图，$s,t$ 是它的两个顶点，需要求最小的 $s$-$t$ 割．一个常见的误区是直接宣称平面图最小割等于对偶图最短路．事实上，这种转化只适用于存在 $G$ 的平面嵌入使得 $s,t$ 共面的情况，只是在考察此知识点的算法竞赛题目中，给出的图往往具有并且附带这样的平面嵌入．下面先介绍这种转化．
...
然而，如果 $G$ 并不具有这样的平面嵌入，上述转化就不适用，反例如下图．
...
可以发现，反例图中添加边 $(s,t)$ 之后得到非平面图 $K_5$，故而不具有这样的平面嵌入．
