...
将可平面图的问题转化到对偶图上，有时更容易解决．一个典型的例子是，可平面图 [最小割](./flow/min-cut.md) 问题可以转化为对偶图 [最短路](./shortest-path.md) 问题．设 $G$ 是带边权的可平面图，$s,t$ 是它的两个顶点，需要求最小的 $s$-$t$ 割．如果存在 $G$ 的平面嵌入使得 $s,t$ 处于同一个面上，那么就可以简单转化．在算法竞赛题目中，给出的图往往附带这样的平面嵌入，不过出于理论完备的考量，这里给出一个可以用于判定这样的平面嵌入存在性的定理．
