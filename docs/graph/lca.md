...
### Tarjan 算法
 ...
 `Tarjan 算法` 是一种 `离线算法` ，需要使用 `并查集` 记录某个节点的祖先节点。做法如下：

1.  首先接受输入（邻接链表）、查询（存储在另一个邻接链表内）。查询边其实是虚拟加上去的边，为了方便，每次输入查询边的时候，将这个边及其反向边都加入到 `queryEdge` 数组里。
2.  然后对其进行一次 DFS 遍历，同时使用 `visited` 数组进行记录某个节点是否被访问过、 `parent` 记录当前节点的父亲节点。
3.  其中涉及到了 `回溯思想` ，我们每次遍历到某个节点的时候，认为这个节点的根节点就是它本身。让以这个节点为根节点的 DFS 全部遍历完毕了以后，再将 `这个节点的根节点` 设置为 `这个节点的父一级节点` 。
4.  回溯的时候，如果以该节点为起点， `queryEdge` 查询边的另一个节点也恰好访问过了，则直接更新查询边的 LCA 结果。
5.  最后输出结果。
