...
通过肉眼观察可以得出结论——$1$ 号结点的右子树（虽然实际上可能有多个子树，但这里只有两个子树，所以暂时这么称呼了）一个红色结点都没有，**所以没必要去 DP 它**。

观察题目给出的条件，红色点（关键点）的总数是与 $n$ 同阶的，也就是说实际上一次询问中红色的点对于整棵树来说是很稀疏的，所以如果我们能让复杂度由红色点的总数来决定就好了。

因此我们需要 **浓缩信息，把一整棵大树浓缩成一棵小树**。
...
在提出方案之前，我们先确认一个事实——在虚树里，只要保证祖先后代的关系没有改变，就可以随意添加结点。
...
因为多个结点的 LCA 可能是同一个，所以我们不能多次将它加入虚树。
...
??? note "证明"
    ...
    另外第一个点没有被一个结点连接会不会有影响呢？因为第一个点一定是这棵树的根，所以不会有影响，所以总边数就是 $m-1$ 条。

因为至少要两个实点才能够召唤出来一个虚点，再加上一个根结点，所以虚树的点数就是实点数量的两倍。
...
-   取序列中第一个作为当前结点，也就是 $4$。再取栈顶元素，为 $1$。求 $1$ 和 $4$ 的 LCA：$LCA(1,4)=1$。
...
-   发现 $LCA(6,4)\neq$ 栈顶元素，进入判断阶段。
-   判断阶段：发现栈顶结点 $4$ 的 DFS 序是大于 $LCA(6,4)$ 的，但是次大结点（栈顶结点下面的那个结点）$1$ 的 DFS 序是等于 LCA 的（其实 DFS 序相等说明结点也相等），说明 LCA 已经入栈了，所以直接连接 $1\to4$ 的边，也就是 LCA 到栈顶元素的边。并把 $4$ 从栈中弹出。
...
-   发现 $LCA(7,6)\neq$ 栈顶元素，进入判断阶段。
-   判断阶段：发现栈顶结点 $6$ 的 DFS 序是大于 $LCA(7,6)$ 的，但是次大结点（栈顶结点下面的那个结点）$1$ 的 DFS 序是小于 LCA 的，说明 LCA 还没有入过栈，所以直接连接 $3\to6$ 的边，也就是 LCA 到栈顶元素的边。把 $6$ 从栈中弹出，并且把 $LCA(6,7)$ 入栈。
-   结束了判断阶段，将 $7$ 入栈，当前栈为 $1,3,7$。

![vtree-17](./images/vtree-construction6.svg)

-   发现序列里的 3 个结点已经全部加入过栈了，退出循环。
-   此时栈中还有 3 个结点：$1,3,7$，很明显它们是一条链上的，所以直接链接：$1\to3$ 和 $3\to7$ 的边。
-   虚树就建完啦！
...
???+ note "代码实现"
    ...
    void build() {
      ...
      // 1 号结点入栈，清空 1 号结点对应的邻接表，设置邻接表边数为 1
      for (int i = 1, l; i <= k; ++i)
        if (h[i] != 1) {
          // 如果 1 号结点是关键结点就不要重复添加
          l = lca(h[i], sta[top]);
          // 计算当前结点与栈顶结点的 LCA
          if (l != sta[top]) {
            // 如果 LCA 和栈顶元素不同，则说明当前结点不再当前栈所存的链上
            while (id[l] < id[sta[top - 1]])
              // 当次大结点的 Dfs 序大于 LCA 的 Dfs 序
              g.push(sta[top - 1], sta[top]), top--;
            // 把与当前结点所在的链不重合的链连接掉并且弹出
            if (id[l] > id[sta[top - 1]])
              // 如果 LCA 不等于次大结点（这里的大于其实和不等于没有区别）
              g.head[l] = -1, g.push(l, sta[top]), sta[top] = l;
            // 说明 LCA 是第一次入栈，清空其邻接表，连边后弹出栈顶元素，并将 LCA
            // 入栈
            else
              g.push(l, sta[top--]);
            // 说明 LCA 就是次大结点，直接弹出栈顶元素
          }
          g.head[h[i]] = -1, sta[++top] = h[i];
          // 当前结点必然是第一次入栈，清空邻接表并入栈
        }
      for (int i = 1; i < top; ++i)
...
对于消耗战这题，直接在虚树上跑最开始讲的那个 DP 就行了，我们等于利用了虚树排除了那些没用的非关键结点！仍然考虑 $i$ 的所有儿子 $v$：
