...
最后，在序列 $A$ 上，枚举 **相邻** 的两个 **点编号**  $x,y$，求得它们的 LCA 并且连接 $\text{LCA}(x,y),y$，虚树就构造完成了。

为什么连接 $\text{LCA}(x,y)$ 和 $y$ 可以做到不重不漏呢？

??? note "证明"
    如果 $x$ 是 $y$ 的祖先，那么 $x$ 直接到 $y$ 连边。因为 dfs 序保证了 $x$ 和 $y$ 的 DFS 序是相邻的，所以 $x$ 到 $y$ 的路径上面没有关键点。
    
    如果 $x$ 不是 $y$ 的祖先，那么就把 $\text{LCA}(x,y)$ 当作 $y$ 的的祖先，根据上一种情况也可以证明 $\text{LCA}(x,y)$ 到 $y$ 点的路径上不会有关键点。
    
    所以连接 $\text{LCA}(x,y)$ 和 $y$，不会遗漏，也不会重复。
