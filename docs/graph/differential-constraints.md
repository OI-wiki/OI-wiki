...
差分约束系统中的每个约束条件 $x_i-x_j\leq c_k$ 都可以变形成 $x_i\leq x_j+c_k$ ，这与单源最短路中的三角形不等式 $dist[y]\leq dist[x]+z$ 非常相似。因此，我们可以把每个变量 $x_i$ 看做图中的一个节点，对于每个约束条件 $x_i-x_j\leq c_k$ ，从节点 $j$ 向节点 $i$ 连一条长度为 $c_k$ 的有向边。
