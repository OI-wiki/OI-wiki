...
每次将一个结点与其在 $T$ 上的父亲合并，时间复杂度为 $O(n\log n+m\log m+k\log k)$，空间复杂度为 $O(m+n\log m+k)$。这样在求出一个结点对应的堆时，无需复制结点且之后其父亲结点对应的堆仍然可以正常访问。

注意的是，如上文所言，最终询问时不需要可并堆的合并操作。  
询问时使用优先队列维护可并堆的根，对于可并堆堆顶的删除，直接将其左右儿子加入二叉堆中，  
就只需要 $O(k)$ 而非 $O(k\log n)$ 的空间。
