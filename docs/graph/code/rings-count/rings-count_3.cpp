...
long long solve1() {
    ...
    ans[1] = 0;
    for (int a = 1; a <= n; a++)
        ans[1] += 1ll * deg[a] * (deg[a] - 1) % mod * (deg[a] - 2) % mod * (deg[a] - 3) % mod * power(24) % mod;
    return ans[1];
}
...
long long solve4() {
    ...
    for (int i = 1; i <= n; i++)
        for (int j = head[i]; j; j = edge[j].nxt) {
            int x = edge[j].to;
            ans[4] += (deg[i] - 1ll) * (deg[x]-1) % mod * (deg[x] - 2) % mod * power(2) % mod;
        }
    ans[4] -= 2 * solve3();
...
int main() {
    ...
    while (T--) {
        ...
        cerr<<solve1() <<' '<< solve2() <<' '<< solve3() <<' '<< solve4() <<' '<< solve5()<<endl;
        printf("%lld\n", ((solve1() + solve2() + solve3() + solve4() + solve5()) % mod + mod) % mod);
    }
    return 0;
}