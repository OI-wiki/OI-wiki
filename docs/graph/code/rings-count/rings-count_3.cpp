...
long long solve1() {
    ...
    for (int i = 1; i <= n; i++) {
        int x = E[i].size();
        ans[1] += 1ll * x * (x - 1) % mod * (x - 2) % mod * (x - 3) % mod * power24 % mod;
    }
    return ans[1] %= mod;
}
long long solve2() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j : E1[i])
            for (int k : E1[j])
                ans[2] += (1ll * cnt1[k] * (cnt1[k] - 1) / 2 + 1ll * cnt1[k] * cnt2[k]) % mod, cnt1[k] = 0;
        for (int j : E2[i])
            for (int k : E1[j])
                if (k != i) ans[2] += 1ll * cnt2[k] * (cnt2[k] - 1) / 2 % mod * power2 % mod, cnt2[k] = 0;
    }
    return ans[2];
}
long long solve3() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        for (int j : E1[i])
            for (int k : E1[j])
                if (vis[k]) ans[3] = (1ll * ans[3] + E[i].size() + E[j].size() + E[k].size() - 6) % mod, ans[0]++;
...
long long solve4() {
    ...
    for (int i = 1; i <= n; i++)
        for (int j : E[i])
            (ans[4] += 1ll * (E[j].size() - 1) * (E[j].size() - 2) / 2 * (E[i].size() - 1) % mod) %= mod;
...
long long solve5() {
    ...
    solve3();
    return ans[5] = (ans[5] % mod - 2 * solve3() - 4 * solve2() - 3 * ans[0]) % mod;
}
...
int main() {
    ...
    while (T--) {
        ...
        printf("%lld\n", ((solve5() + solve1() + solve2() + solve4() + solve3()) % mod + mod) % mod);
    }
    return 0;
}