...
int t, n, m, u, v, d[kN * 2 + 10], fa[100010], dfn[200020], low[200020], dcnt,
    bel[200020], scnt;
...
void DFS(int x, int pa, int cd) {
  ...
  if (x <= n) {
    ...
    fa[x - n] =
        findfa(x - n + 1);  // 注意要在这里赋值，否则可能两次走到同一个节点
    modify(x - n, dfn[x]);
    for (auto i : to[x]) {
      if (!dfn[i]) {
        DFS(i, x, cd);
  ...
  if (low[x] == dfn[x]) {
    ...
    for (; stk.size() && dfn[stk.back()] >= dfn[x]; stk.pop_back()) {
      bel[stk.back()] = scnt;
      if (stk.back() > n)
        modify(stk.back() - n, kN);  // 注意，由于第二类情况中有一个 !vis[i]
                                     // 的限制，在这里需要把 i 的记录删除
    }
  }
}
