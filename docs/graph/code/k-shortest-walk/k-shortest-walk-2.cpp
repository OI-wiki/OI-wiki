...
void find_k_shortest_walks() {
    ...
    while (!pq.empty() && cnt < k) {
        ...
        pq.pop();
        ans[cnt++] = cost;
        if (heaps.lc[cur]) pq.emplace(cost - heaps.va[cur] + heaps.va[heaps.lc[cur]], heaps.lc[cur]);
        if (heaps.rc[cur]) pq.emplace(cost - heaps.va[cur] + heaps.va[heaps.rc[cur]], heaps.rc[cur]);
