...
void modify_subtree(int u, int w) {
	...
	} else T.upd(1, 1, n, dfn[u], dfn[u] + siz[u] - 1, w);
}
...
int main() {
	...
	for (std::cin >> m; m; --m) {
		int op, u, v, w; std::cin >> op >> u;
		if (op == 1) root = u; // 换根 
		else if (op == 2) std::cin >> v >> w, modify_path(u, v, w); // 修改路径 
		else if (op == 3) std::cin >> w, modify_subtree(u, w); // 修改子树 
		else if (op == 4) std::cin >> v, std::cout << query_path(u, v) << "\n"; // 查询路径 
		else std::cout << query_subtree(u) << "\n"; // 查询子树 
	}
	std::cout.flush();
	return 0;
}
