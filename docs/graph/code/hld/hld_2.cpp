...
long long n, q, num[30010], dfn[30010], rep[30010], top[30010], son[30010],
    dep[30010], siz[30010], fa[30010], cnt, tree_sum[120010], tree_max[120010];
...
void change_sum(long long now, long long l, long long r, long long x,
                long long k) {
  if (r < x || l > x) return;
  if (l == r && l == x) {
    tree_sum[now] = k;
  ...
  long long mid = (l + r) >> 1;
  if (x <= mid)
    change_sum(now << 1, l, mid, x, k);
  else
    change_sum(now << 1 | 1, mid + 1, r, x, k);
  tree_sum[now] = tree_sum[now << 1] + tree_sum[now << 1 | 1];
}

long long find_sum(long long now, long long l, long long r, long long x,
                   long long y) {
  if (r < x || l > y) return 0;
...
void change_max(long long now, long long l, long long r, long long x,
                long long k) {
  if (r < x || l > x) return;
  if (l == r && l == x) {
    tree_max[now] = k;
  ...
  long long mid = (l + r) >> 1;
  if (x <= mid)
    change_max(now << 1, l, mid, x, k);
  else
    change_max(now << 1 | 1, mid + 1, r, x, k);
  tree_max[now] = max(tree_max[now << 1], tree_max[now << 1 | 1]);
}

long long find_max(long long now, long long l, long long r, long long x,
                   long long y) {
