...
我们可以使用倍增来维护，预处理出每个结点的 $2^i$ 级祖先及到达其 $2^i$ 级祖先路径上最大的边权，这样在倍增求 LCA 的过程中可以直接求得。
...
首先新建 $n$ 个集合，每个集合恰有一个结点，点权为 $0$。

每一次加边会合并两个集合，我们可以新建一个点，点权为加入边的边权，同时将两个集合的根结点分别设为新建点的左儿子和右儿子。然后我们将两个集合和新建点合并成一个集合。将新建点设为根。

不难发现，在进行 $n-1$ 轮之后我们得到了一棵恰有 $n$ 个叶子的二叉树，同时每个非叶子结点恰好有两个儿子。这棵树就叫 Kruskal 重构树。
...
也就是说，到点 $x$ 的简单路径上最大边权的最小值 $\leq val$ 的所有点 $y$ 均在 Kruskal 重构树上的某一棵子树内，且恰好为该子树的所有叶子结点。

我们在 Kruskal 重构树上找到 $x$ 到根的路径上权值 $\leq val$ 的最浅的结点。显然这就是所有满足条件的结点所在的子树的根结点。
...
??? note "[NOI 2018 归程](https://uoj.ac/problem/393)"
    ...
    我们构造出来根据海拔的最大生成树。显然每次询问可以到达的结点是在最大生成树中和询问点的路径上最小边权 $> p$ 的结点。
    
    根据 Kruskal 重构树的性质，这些结点满足均在一棵子树内同时为其所有叶子结点。
    
    也就是说，我们只需要求出 Kruskal 重构树上每一棵子树叶子的权值 min 就可以支持子树询问。
    
    询问的根结点可以使用 Kruskal 重构树上倍增的方式求出。
