...
具体来说，维护一个森林，查询两个节点是否在同一棵树中，连接两棵树。
...
是另一种常见并且好写的最小生成树算法。
基本思想是从一个节点开始，不断加点（而不是 Kruskal 算法的加边）。

### 证明

从任意一个节点开始，将节点分成两类：已加入的，未加入的。

每次从未加入的节点中，找一个与已加入的节点之间边权最小值最小的节点。

然后将这个节点加入，并连上那条边权最小的边。
...
具体来说，每次要选择距离最小的一个节点，以及用新的边更新其他节点的距离。
