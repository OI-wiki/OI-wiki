...
1.  将所有边的容量 $c(u,v)$ 更新为 $c(u,v)(|E| + 1) + 1$，在新图上求最小割，就得到原图的最少割边最小割．新图最小割的容量除以 $|E| + 1$ 下取整即原图最小割，余数即割边数量．
    ...
    注意，这一做法的正确性建立在所有边容量都是整数的前提下：只有这样，才能保证 $k\le|E|<|E|+1$ 不会进位到高位．若容量是有理数，可先乘以公分母化为整数；若容量是任意实数，或整型会溢出，则直接将容量取为二元组 $(c(u,v),1)$，加法按分量进行，比较用字典序即可，此时只依赖容量加减与比较的增广类算法（如 Dinic 算法等）都可以原样运行．
