...
MPM 算法的整体结构和 Dinic 算法类似，也是分阶段运行的。在每个阶段，计算当前流量的残量网络和分层网络 $L$ ，如果汇点 $t$ 没有出现在 $L$ 中，算法结束。 否则，删除所有不在分层图中从源点 $s$ 到汇点 $t$ 的路径上的顶点。重复以上步骤直到找到阻塞流。并且，在每个阶段我们都会在 $G$ 的残量网络的分层网络中找到阻塞流。
 
MPM 算法与 Dinic 算法的主要区别在于寻找阻塞流的方式不同：MPM 算法中寻找阻塞流的部分的复杂度只花了 $O(n^2\log n)$ （用斐波那契堆的话则为 $O(n^2)$ ），要优于 Dinic 算法。
...
这样的定义同样适用于表示减去非零流量获得的残量。具体步骤如下：

1. 找到最小容量 $d$ 的顶点 $v$ 。 如果 $d = 0$ ，执行步骤二。如果 $d \neq 0$，执行步骤三。
2. 删除 $v$ 和所有相邻边并更新相邻顶点的容量。回到步骤一 。
3. 将 $d$ 个单位的流从 $v$ 推到汇点，并将 $d$ 个单位的流从源点加到 $v$，使得通过 $v$ 的流量增加 $d$ 。
    - $v$ 的输出边按顺序饱和，最多留下一个非饱和边。在此过程中删除变得饱和的所有边。然后，在 $v$ 中的边饱和期间接收流的每个顶点上重复此过程，一直到汇点 $t$ 。总是有可能将所有 $d$ 个单位的流一直推到 $t$ ，因为每个顶点的容量至少为 $d$ 。
    - $v$ 的传入边按顺序饱和，最多留下一个非饱和边。在此过程中删除变得饱和的所有边。然后，在边缘饱和到 $v$ 期间从中获取流的每个顶点上重复此过程，依此类推，一直到源点 $s$ 。 总是可以从 $s$ 一路取出所有 $d$ 个单位的流量，因为每个顶点的容量至少为 $d$ 。
    - 这样， $v$ 的所有传入边或 $v$ 的所有传出边都饱和并被删除，因此可以从分层图中删除 $v$ 及其所有剩余的关联边，并更新邻居的容量。之后跳转到步骤一。
    - 以上步骤可以用BFS直接模拟。

#### 时间复杂度分析

使用 BFS 计算当前流和分层图的残量网络需要 $O(m)$ 。使用基于堆的优先队列或斐波那契堆，需要 $O(n \log n)$ 才能找到并删除最小容量的顶点， 需要 $O(m)$ 在所有循环中删除完全饱和边，需要 $O(n^2)$ 在所有循环中执行部分饱和，因为在步骤三中，对于步骤一中每个 $v$ 的选择，在每个顶点处最多执行一次。

这样，我们唯一没有考虑到的就是当推动流通过一个顶点时，它的容量必须减少，进而可能会改变它在优先队列中的优先级。更新优先队列顶点的优先级需要 $O(log n)$ ， 而更新斐波那契堆的只需要 $O(1)$ 。我们可能需要对每个完全饱和边执行一次，对每个部分饱和边执行一次。 在整个阶段，由于饱和边被删除，最多有 $m$ 个完全饱和边，并且最多有 $n^2$ 个部分饱和边。 因此，在整个阶段，减少优先级所需的时间对于基于堆的优先队列，时间复杂度为 $O(n^2 \log n)$ ，对于斐波那契堆， 时间复杂度为 $O(n^2)$ 。
