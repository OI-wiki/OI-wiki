...
MPM 算法的整体结构和 Dinic 算法类似，也是分阶段运行的。在每个阶段，计算当前流量的残差网络和分层网络 $L_G$ ，如果 $t$ 没有出现在 $L_G$ 中，算法结束。 否则，删除所有不在分层图中从 $s$ 到 $t$ 的路径上的顶点。重复以上步骤直到找到阻塞流。并且，在每个阶段我们都会在 $G$ 的残差网络的分层网络中找到阻塞流。
