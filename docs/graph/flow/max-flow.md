...
MPM 算法需要考虑顶点而不是边的容量。在分层网络 $L$ 中，如果定义顶点 $v$ 的容量 $c(v)$ 为其传入边总容量和传出边总容量中的最小值，则有：

$c(v) = min \sum\limits_{u \in V} c(u, v), \sum\limits_{w \in V} c(v, w).$

那么这个公式可以直观地表示可以通过该顶点的最大流量。这样的定义同样适用于表示减去非零流量获得的残量。具体步骤如下：
...
使用 BFS 计算当前流和分层图的残量网络需要 $O(m)$ 。使用基于堆的优先队列或斐波那契堆，需要 $O(n \log n)$ 才能找到并删除最小容量的顶点， 需要 $O(m)$ 在所有循环中删除完全饱和边，需要 $O(n^2)$ 在所有循环中执行部分饱和，因为在步骤 $3$ 中，对于步骤 $1$ 中每个 $v$ 的选择，在每个顶点处最多执行一次。
