...
该方法在求解过程中忽略流守恒性，并每次对一个节点更新信息，以求解最大流

有 HLPP 的主流算法

推送 - 重贴标签算法通过对单个节点的更新操作，直到没有节点需要更新来求解最大流

算法过程维护的流函数不一定保持流守恒性，对于一个节点，我们允许进入节点的流超过流出节点的流，超过的部分被称为节点 $u(u\in V-\{s,t\})$ 的 **超额流**  $e(u)$ ：
...
推送 - 重贴标签算法维护每个节点的高度 $h(u)$ ，并且规定溢出的节点 $u$ 如果要推送超额流，只能向高度小于 $u$ 的节点推送；如果 $u$ 没有相邻的高度小于 $u$ 的节点，就修改 $u$ 的高度（重贴标签）。
...
引理 1：设 $G_f$ 上的高度函数为 $h$ ，对于任意两个节点 $u,v\in V$ ，如果 $h(u)>h(v)+1$ ，则 $(u,v)$ 不是 $G_f$ 中的边。

算法只会在 $h(u)=h(v)+1$ 的边执行推送。

#### 推送 -Push

适用条件：节点 $u$ 溢出，且存在节点 $v((u,v)\in V_f,c(u,v)-f(u,v)>0,h(u)=h(v)+1)$ ，则 push 操作适用于 $(u,v)$ 。
...
适用条件：如果节点 $u$ 溢出，且 $\forall (u,v)\in E_f,h(u)\leq h(v)$ ，则 relabel 操作适用于 $u$ 。
...
-   只要存在节点 $u$ 满足 push 或 relabel 的条件，就执行对应的操作。

如图，每个节点中间表示编号，左下表示高度值 $h(u)$ ，右下表示超额流 $e(u)$ ，节点颜色的深度也表示节点的高度；边权表示 $c(u,v)-f(u,v)$ ，绿色的边表示满足 $h(u)=h(v)+1$ 的边 $(u,v)$ （即残存网络的边 $E_f$ ）：

![p1](./images/2148.png)

整个算法我们大致浏览一下过程，这里笔者使用的是一个暴力算法，即暴力扫描是否有溢出的节点，有就更新
...
可以发现，最后的超额流一部分回到了 $s$ ，且除了源点汇点，其他节点都没有溢出；这时的流函数 $f$ 满足流守恒性，为最大流，即 $e(t)$ 。
...
最高标号预流推进算法（High Level Preflow Push）是基于推送 - 重贴标签算法的优先队列实现，该算法优先推送高度高的溢出的节点，算法算法复杂度 $O(n^2\sqrt m)$ 。

具体地说，HLPP 维护以下过程：

1.  初始化（基于推送 - 重贴标签算法）；
2.  选择溢出节点（除 $s,t$ ）中高度最高的节点 $u$ ，并对它所有可以推送的边进行推送；
...
HLPP 推送的条件是 $h(u)=h(v)+1$ ，而如果在算法的某一时刻， $h(u)=t$ 的节点个数为 $0$ ，那么对于 $h(u)>t$ 的节点就永远无法推送超额流到 $t$ ，因此只能送回 $s$ ，那么我们就在这时直接让他们的高度变成 $n+1$ ，以尽快推送回 $s$ ，减少重贴标签的操作。
...
bool bfs_init() {
  ...
  queue<int> q;
  q.push(t), ht[t] = 0;
  while (q.size()) {  // 反向 BFS, 遇到没有访问过的节点就入队
    int u = q.front();
    q.pop();
    for (int i = h[u]; i; i = e[i].nex) {
      const int &v = e[i].t;
...
priority_queue<int, vector<int>, cmp> pq;  // 将需要推送的节点以高度高的优先
bool vis[N];                               // 是否在优先队列中
