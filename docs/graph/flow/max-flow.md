...
MPM 算法的整体结构和 Dinic 算法类似，也是分阶段运行的。在每个阶段，计算当前流量的残差网络和分层网络 $L_G$ ，如果汇点 $t$ 没有出现在 $L_G$ 中，算法结束。 否则，删除所有不在分层图中从源点 $s$ 到汇点 $t$ 的路径上的顶点。重复以上步骤直到找到阻塞流。并且，在每个阶段我们都会在 $G$ 的残差网络的分层网络中找到阻塞流。
 
MPM 算法与 Dinic 算法的主要区别在于寻找阻塞流的方式不同，MPM 算法中寻找阻塞流的部分的复杂度只花了 $O(n^2\log n)$ （如果用斐波那契堆的话是 $O(n^2)$ ），要优于 Dinic 算法。

MPM 算法需要考虑顶点而不是边的容量。如果定义顶点 $v$ 的容量 $c(v)$ 为其传入边总容量和传出边总容量中的最小值，则有：

$c(v) = min \sum\limits_{u \in V} c(u, v), \sum\limits_{w \in V} c(v, w).$

那么这个公式可以直观地表示可以通过该顶点的最大流量。这样的定义同样适用于表示减去非零流量获得的剩余容量。具体步骤如下：
