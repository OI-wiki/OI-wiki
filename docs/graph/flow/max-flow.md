...
**MPM** (Malhotra, Pramodh-Kumar and Maheshwari) 算法得到最大流的方式有两种：使用基于堆的优先队列，时间复杂度为 $O(n^3\log n)$ ；常用BFS解法，时间复杂度为 $O(n^3)$ 。注意，本章节只专注于分析更优也更简洁的 $O(n^3)$ 算法。
...
那么最小参考节点 $r$ 的定义为 $p(r) = min {p(v)}$ 。 对于一个参考节点 $r$ ， 流可以通过增加 $p(r)$ 使 $p(r)$ 变为 $0$ 。这是因为 $L$ 是非环状的，所以我们可以通过传出边将流从 $r$ 推出，并且到达 $t$ 。 类似地，我们可以从 $s$ 将流汇出。 增广路的构建就是基于这个原理。在每次迭代中，我们找到一个参考节点并将流通过 $r$ 从源点 $s$ 推送到汇点 $t$ 。 这个过程可以用 BFS 模拟。 所有满流边都可以从 $L$ 中删除，因为它们任何情况下都不会在此阶段后被使用。 同样，所有与 $s$ 和 $t$ 不同且没有出边或入边的节点都可以删除。

具体步骤如下：

1. 找到最小容量 $r$ 的顶点 $v$ 。 如果 $r = 0$ ，执行步骤二。如果 $r \neq 0$，执行步骤三。
2. 删除 $v$ 和所有相邻边并更新相邻顶点的容量。回到步骤一 。
3. 将 $r$ 个单位的流从 $v$ 推到汇点，并将 $r$ 个单位的流从源点加到 $v$，使得通过 $v$ 的流量增加 $r$ 。
    - $v$ 的出边按顺序满流，最多留下一个非满流边。在此过程中删除满流边。然后，在 $v$ 中的边满流期间接收流的每个顶点上重复此过程，一直到汇点 $t$ 。我们可以确定总能将所有 $r$ 个单位的流一直汇到汇点 $t$ ，因为每个顶点的容量至少为 $r$ 。
    - $v$ 的传入边按顺序满流，最多留下一个非满流边。在此过程中删除变得满流的所有边。然后，在边满流到 $v$ 期间从中获取流的每个顶点上重复此过程，依此类推，一直到源点 $s$ 。 总是可以从 $s$ 一路取出所有 $r$ 个单位的流量，因为每个顶点的容量至少为 $r$ 。
    - 这样， $v$ 的所有传入边或 $v$ 的所有传出边都满流并被删除，因此可以从分层图中删除 $v$ 及其所有剩余的关联边，并更新邻居的容量。之后跳转到步骤一。
...
- **引理1**： 每次迭代后，从 $s$ 到每个点的距离不会减少， 也就是说， $level_{i+1}[v] \ge level_{i}[v]$ 。
