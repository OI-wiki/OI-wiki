...
对于网络 $G = (V, E)$，如果对于源点或汇点，所有与其关联的边容量均为 $1$，则 Dinic 算法的时间复杂度为 $O(|V||E|)$。

???+ info "证明"
    我们考虑单次增广的复杂度。类似一般图上的分析，复杂度分两部分：成功的增广路中饱和边的贡献和没有成功增广的代价。
    
    第一部分中，我们不妨选取每条增广路中的饱和边为与源点（或汇点）关联的那条，这样所有可能的饱和边只有 $O(|V|)$ 条，并且如果一条与源点（或汇点）关联的边在某次增广中成为了饱和边，之后这条边上不可能发生退流，即整个算法过程中这样的边只能成为一次饱和边。而根据之前的分析，一条饱和边对复杂度的贡献是 $O(|V|)$，所以这部分的总时间复杂度为 $O(|V|^2)$。
...
-   如果对于源点或汇点，所有与其关联的边容量均为 $1$，则 Dinic 算法的时间复杂度为 $O(|V||E|)$。
