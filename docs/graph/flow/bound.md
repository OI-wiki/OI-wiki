...
由于最大流需要满足初始流量平衡条件（最大流可以看成是下界为 $0$ 的上下界最大流），但是构造出来的初始流很有可能不满足初始流量平衡。假设一个点初始流出流量减初始流入流量为 $M$ 。
...
则我们可以加入一条 $T$ 到 $S$ 的上界为 $\infin$ ，下界为 $0$ 的边转化为无源汇上下界可行流问题。
...
??? note "[AHOI 2014 支线剧情](https://loj.ac/problem/2226)"
    ...
    对于每条 $x$ 到 $y$ 花费 $v$ 的剧情边设上界为 $\infin$ , 下界为 $1$。
