...
我们先随意选择一个结点作为根结点 $rt$ ，所有完全位于其子树中的路径可以分为两种，一种是经过当前根结点的路径，一种是不经过当前根结点的路径。对于经过当前根结点的路径，又可以分为两种，一种是以根结点为一个端点的路径，另一种是两个端点都不为根结点的路径。而后者又可以由两条属于前者链合并得到。所以，对于枚举的根结点 $rt$ ，我们先计算在其子树中且经过该结点的路径对答案的贡献，再递归其子树对不经过该结点的路径进行求解。

在本题中，对于经过根结点 $rt$ 的路径，我们先枚举其所有子结点 $ch$ ，以 $ch$ 为根计算 $ch$ 子树中所有结点到 $rt$ 的距离。记结点 $i$ 到当前根结点 $rt$ 的距离为 $dist_i$ ， $tf_{d}$ 表示之前处理过的子树中是否存在一个结点 $v$ 使得 $dist_v=d$ 。若一个询问的 $k$ 满足 $tf_{k-dist_i}=true$ ，则存在一条长度为 $k$ 的路径。在计算完 $ch$ 子树中所连的边能否成为答案后，我们将这些新的距离加入 $tf$ 数组中。
...
若我们 **每次选择子树的重心作为根结点** ，可以保证递归层数最少，时间复杂度为 $O(n\log_2 n)$ 。
