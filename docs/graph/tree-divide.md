...
2. $(u, v)$ 路径上没有出现过的颜色 $j$，它们的贡献来自于 $u$ 除了 $d$ 以外的其他所有子树的 $cnt_j$，这部分答案为 $\sum_{j \notin (u, v)} cnt_j$。
