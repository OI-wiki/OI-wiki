...
设 $s_i$ 表示第 $i$ 个连通块内点的数量。我们考虑对 $k$ 个连通块构造 Prüfer 序列。由于两个连通块之间的连接方法很多，这并不是普通的 Prüfer 序列。于是不妨假设设 $d_i$ 为第 $i$ 个连通块的度数。由于度数之和是边数的两倍，于是 $\sum_{i=1}^kd_i=2k-2$。则对于给定的 $d$ 序列构造 Prüfer 序列的方案数是
