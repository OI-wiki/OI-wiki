...
??? note " 一道很不简单的例题 [uoj 444 二分图](https://uoj.ac/problem/444)"
    ...
    若 $degree \bmod k \neq 0$, 我们将其拆为 $degree/k$ 个度数为 k 的结点和一个度数为 $degree \bmod k$ 的结点。
    
    若 $degree \bmod k = 0$, 我们将其拆为 $degree/k$ 个度数为 k 的结点。
