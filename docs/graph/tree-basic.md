...
按照BFS的过程遍历二叉树。下图的二叉树BFS 遍历的结果是`[3,9,2,4,1,7]`。

![tree-basic-levelOrder](images/tree-basic-levelOrder.svg)

#### 二叉树层序遍历

层序遍历是BFS最重要应用之一。层序遍历是指二叉树按照从根节点到叶子节点的层次关系，一层一层的横向遍历各个节点。

下图的二叉树层序遍历的结果是`[[3],[9,2],[4,1,7]]`（每一层从左向右）。可以直接用 BFS 得出层序遍历的结果。与BFS 是不同的是，层序遍历要求区分每一层，也就是返回一个二维数组。
