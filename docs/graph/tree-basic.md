...
二叉树节点数为 N，遍历二叉树，要求时间复杂度为 O(N)，额外空间复杂度 O(1)。

遍历二叉树的递归方法和非递归方法都使用了栈结构，来实现从下层到上层的移动，无法达到空间复杂度 O(1）的限制。Morris 遍历的实质是避免使用栈，利用底层节点空闲的 right 指针指回上层的某个节点，从而完成下层到上层的移动。
