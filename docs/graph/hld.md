...
保证 $1\leqslant n\leqslant 30000$，$0\leqslant q\leqslant 200000$。
...
\begin{cases}
0&n=1\\
T\left(\left\lfloor\frac{n-1}2\right\rfloor\right)+1&n\geqslant2
\end{cases}
$$

$2999+\sum_{i=1}^{2999}T(i)\leqslant 29940$，事实上这个上界是可以通过构造数据达到的，然而只要进行一些随机扰动（如对深度进行排序时使用不稳定的排序算法），询问次数很难超过 $21000$ 次。
...
现在我们假设我们找到了询问节点的 $2^i$ 级祖先满足 $2^i \leqslant k < 2^{i+1}$。

我们考虑求出其所在重链的节点并且按照深度列入表格。假设重链长度为 $d$。

同时我们在预处理的时候找到每条重链的根节点的 $1$ 到 $d$ 级祖先，同样放入表格。

根据长链剖分的性质，$k-2^i \leqslant 2^i \leqslant d$, 也就是说，我们可以 $O(1)$ 在这条重链的表格上求出的这个节点的 $k$ 级祖先。
