...
-   $u$ 是 $\textit{root}$ 的祖先，即 $u$ 位于 $1$ 到 $\textit{root}$ 的简单路径上。
    ...
    -   若 $\textit{dep}(\textit{top}(v))<\textit{dep}(u)+1$，说明 $u,v$ 处在同一条重链上，根据同一条重链上 DFS 序连续的性质，$\textit{dfn}(v)=\textit{dfn}(\textit{top}(v))+\textit{dep}(u)+1-\textit{dep}(\textit{top}(v))$。然后用 $\textit{rnk}$ 就可以找到 $v$ 了。
