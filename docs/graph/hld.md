...
???+ note "[Acwing 2524. 树链剖分 II](https://www.acwing.com/problem/content/description/2526)"
    给定一颗 $n$ 个结点的树（初始根结点为 $1$），要求支持如下的 $m$ 次操作：
    
    -   换根，将节点 $u$ 设为新的树根。
    -   修改路径上节点权值，将节点 $u$ 和节点 $v$ 之间路径上的所有节点（包括这两个节点）的权值增加 $w$。
    -   修改子树上节点权值，将以节点 $u$ 为根的子树上的所有节点的权值增加 $w$。
    -   询问路径，询问节点 $u$ 和节点 $v$ 之间路径上的所有节点（包括这两个节点）的权值和。
    -   询问子树，询问以节点 $u$ 为根的子树上的所有节点的权值和。
    
    $1 \le n,m \le 10^5$。

树链剖分维护的信息是静态的，不支持动态修改。同时，不可能每次换根后重新预处理信息，复杂度过高。那么，需要充分利用之前得到的信息来帮助解决换根操作。

先以 $1$ 作为根结点跑 `dfs`，预处理出树链剖分所必需的信息。

对于换根操作，我们直接令 $root\gets u$。对于路径修改操作，由于树上两点之间的简单路径唯一，所以不会发生变化，与正常的处理方式相同，此处不过多赘述。

下面来考虑查询子树的问题。我们对 $u$ 和 $root$ 的分布来分类讨论：
