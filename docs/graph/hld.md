...
第二个 DFS 记录所在链的链顶（top，应初始化为结点本身）、重边优先遍历时的 DFS 序（dfn）、DFN 对应的节点编号（rank）。
...
在剖分时 **重边优先遍历** ，最后树的 DFS 序上，重链内的 DFN 是连续的。按 DFN 排序后的序列即为剖分后的链。
