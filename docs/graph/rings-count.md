...
??? note "解题思路"
    考虑状态压缩动态规划。记 $f_{i,j}$ 表示满足当前经过结点集合为 $i$，且现在在结点 $j$ 上，且第一个结点为结点集合 $i$ 中 **编号最小的那个** 的路径条数。

    对于状态 $f_{i,j}$，枚举下一个结点 $u$。若 $u$ 在集合 $i$ 中且是编号最小的那个（即起点），就将答案加上 $f_{i,j}$。若 $u$ 不在 $i$ 中，就将 $f_{i,j}$ 加上 $f_{i|(1<<u),u}$。
