...
这个算法的时间复杂度为 $O(E\sqrt E)$。

???+ note "时间复杂度证明"
    对于定向部分，遍历了所有的边，时间复杂度 $O(V+E)$。
    
    对于每一对 $(v,\ w)$，$u$ 的数量都不超过 $v$ 的入度 $d^-(v)$。
    
    若 $d^-(v)\leq\sqrt E$，由于 $w$ 的个数至多为 $V$，所以这部分时间复杂度为 $O(V\sqrt E)$。
    
    若 $d^-(v) > \sqrt E$，由于 $v$ 指向 $w$，所以 $d(v) \leq d(w)$，得出 $d(w) > \sqrt E$，但是总边数只有 $E$，所以这样的 $w$ 的个数至多为 $\sqrt E$，故时间复杂度为 $O(E\sqrt E)$。
    
    总时间复杂度为 $O(V+E+V\sqrt E+E\sqrt E)=O(E\sqrt E)$。
...
??? note "解题思路"
    ...
    于是我们就得出了所有情况的算法，时间复杂度为 $O(V+E\sqrt E)$。
