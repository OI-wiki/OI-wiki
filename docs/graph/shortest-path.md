...
我们定义一个数组 `f[k][x][y]` ，表示只允许经过节点 $1$ 到 $k$ ，节点 $x$ 到节点 $y$ 的最短路长度。

很显然， `f[n][x][y]` 就是节点 $x$ 到节点 $y$ 的最短路长度。
...
很显然，只有上一次被 $relax$ 的节点，所连接的边，才有可能引起下一次的 $relax$ 。

那么我们用队列来维护“哪些节点可能会引起 $relax$ ”，就能只访问必要的边了。
...
主要思想是，将节点分成两个集合：已确定最短路长度的，未确定的。

一开始第一个集合里只有 $S$ 。

然后重复这些操作：

（1） $relax$ 那些刚刚被加入第一个集合的节点的所有出边。

（2）从第二个集合中，选取一个最短路长度最小的节点，移到第一个集合中。
...
（注：如果使用 priority_queue，无法删除某一个旧的节点，只能插入一个权值更小的编号相同节点，这样操作导致堆中元素是 $O(m)$ 的）
...
第二步，考虑每次加进来的节点，到他的最短路，上一步必然是第一个集合中的元素（否则他不会是第二个集合中的最小值，而且有第一步的性质），又因为第一个集合已经全部 $relax$ 过了，所以最短路显然确定了。
...
分层图最短路，一般模型为有 $k$ 次零代价通过一条路径，求总的最小花费。对于这种题目，我们可以采用 DP 相关的思想，设 $\text{dis}_{i, j}$ 表示当前从起点 $i$ 号节点，使用了 $j$ 次免费通行权限后的最短路径。显然， $\text{dis}$ 数组可以这么转移：
...
事实上，这个 DP 就相当于把每个节点拆分成了 $k+1$ 个节点，每个新节点代表使用不同多次免费通行后到达的原图节点。换句话说，就是每个节点 $u_i$ 表示使用 $i$ 次免费通行权限后到达 $u$ 节点。
