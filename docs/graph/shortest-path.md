...
设源点为 $S$，定义 $dist(u)$ 为 $S$ 到 $u$（当前）的最短路径长度。

对于边 $(u,v)$，设其边权为 $w(u,v)$。则松弛操作对应下面的式子：$dist(v) = \min(dist(v), dist(u) + w(u, v))$。
...
???+warning "负环判断中存在的常见误区"
    ...
    因此如果需要判断整个图上是否存在负环，最严谨的做法是尝试将每个点都作为源点执行一次 Bellman-Ford 算法。
