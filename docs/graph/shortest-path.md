...
???+ warning "负环判断中存在的常见误区"
    ...
    因此如果需要判断整个图上是否存在负环，最严谨的做法是建立一个超级源点，向图上每个结点连一条权值为 0 的边，然后以超级源点为起点执行 Bellman–Ford 算法。
...
???+ note "Bellman–Ford 的其他优化"
    ...
    -   D´Esopo–Pape 算法：将普通队列换成双端队列，如果一个结点之前没有入队，则将其插入队尾，否则插入队首。
...
我们新建一个虚拟结点（在这里我们就设它的编号为 $0$）。从这个点向其他所有点连一条边权为 $0$ 的边。
