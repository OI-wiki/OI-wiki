...
每次循环是 $O(E)$ 的，那么最多会循环多少次呢？

在最短路存在的情况下，由于一次松弛操作会使最短路的边数至少 $+1$，而最短路的边数最多为 $V-1$，因此整个算法最多执行 $V-1$ 轮松弛操作。故总时间复杂度为 $O(VE)$。
...
朴素的实现方法为每次 2 操作执行完毕后，直接在 $T$ 集合中暴力寻找最短路长度最小的结点。2 操作总时间复杂度为 $O(E)$，1 操作总时间复杂度为 $O(V^2)$，全过程的时间复杂度为 $O(V^2 + E) = O(V^2)$。

可以用堆来优化这一过程：每成功松弛一条边 $(u,v)$，就将 $v$ 插入堆中（如果 $v$ 已经在堆中，直接执行 Decrease-key），1 操作直接取堆顶结点即可。共计 $O(E)$ 次 Decrease-key，$O(V)$ 次 pop，选择不同堆可以取到不同的复杂度，参考 [堆](../ds/heap.md) 页面。堆优化能做到的最优复杂度为 $O(V\log V+E)$，能做到这一复杂度的有斐波那契堆等。

特别地，可以使用优先队列维护，此时无法执行 Decrease-key 操作，但可以通过每次松弛时重新插入该结点，且弹出时检查该结点是否已被松弛过，若是则跳过，复杂度 $O(E\log V)$，优点是实现较简单。

这里的堆也可以用线段树来实现，复杂度为 $O(E\log V)$，在一些特殊的非递归线段树实现下，该做法常数比堆更小。并且线段树支持的操作更多，在一些特殊图问题上只能用线段树来维护。

在稀疏图中，$E = O(V)$，堆优化的 Dijkstra 算法具有较大的效率优势；而在稠密图中，$E = O(V^2)$，这时候使用朴素实现更优。
...
这里同时给出 $O(V^2)$ 的暴力做法实现和 $O(E \log E)$ 的优先队列做法实现。
...
任意两点间的最短路可以通过枚举起点，跑 $V$ 次 Bellman–Ford 算法解决，时间复杂度是 $O(V^2E)$ 的，也可以直接用 Floyd 算法解决，时间复杂度为 $O(V^3)$。

注意到堆优化的 Dijkstra 算法求单源最短路径的时间复杂度比 Bellman–Ford 更优，如果枚举起点，跑 $V$ 次 Dijkstra 算法，就可以在 $O(VE\log E)$（取决于 Dijkstra 算法的实现）的时间复杂度内解决本问题，比上述跑 $V$ 次 Bellman–Ford 算法的时间复杂度更优秀，在稀疏图上也比 Floyd 算法的时间复杂度更加优秀。
...
一开始的 Bellman–Ford 算法并不是时间上的瓶颈，若使用 `priority_queue` 实现 Dijkstra 算法，该算法的时间复杂度是 $O(VE\log E)$。
