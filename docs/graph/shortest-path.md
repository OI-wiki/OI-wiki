...
对于最短路存在的图，松弛操作最多执行 $n-1$ 次，因此当第 $n$ 次松弛仍然能够更新最短路时，证明了图中存在负环。

当图不连通时，增加一个新的源点 $t$，令 $t$ 到原图中每个节点 $u$ 连一条单向边，边权任意。这一改动对原图中的负环没有影响，在新图上执行 Bellman-Ford 算法，即可得知原图中是否存在负环。

新的源点 $t$ 到原图中每个节点 $u$ 连接的单向边都不会引起松弛，因此我们并不需要实际设置一个新的源点并且加入新的单向边，在 Bellman-Ford 执行时也将其松弛。

设 dis 数组表示虚拟源点到每个节点的距离值，初始时即是任意的数值表示单向边权值，更新 dis 的松弛过程中使用的仍然是原图的边。

整个过程等价于在求最短路基础上再判断一轮松弛是否成功，最后判定负环。
...
SPFA 可以用于判断 $s$ 点是否能抵达一个负环，只需记录最短路经过了多少条边，当经过了至少 $n$ 条边时，说明 $s$ 点可以抵达一个负环。
