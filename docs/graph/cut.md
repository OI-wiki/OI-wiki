...
和割点差不多，只要改一处：$low_v>dfn_u$ 就可以了，而且不需要考虑根结点的问题。

割边是和是不是根结点没关系的，原来我们求割点的时候是指点 $v$ 是不可能不经过父结点 $u$ 为回到祖先结点（包括父结点），所以顶点 $u$ 是割点。如果 $low_v=dfn_u$ 表示还可以回到父结点，如果顶点 $v$ 不能回到祖先也没有另外一条回到父亲的路，那么 $u-v$ 这条边就是割边。
...
因为两结点间可能不止有一条边，此时它们都不会是桥。

### 过程

一种思路是将参数 `fa` 改为刚刚走过的边的编号（每条边的编号一致）即可，即将「不用父结点更新」改为「不用来时的边更新」。

另一种更简单的思路是设立一个标记判断是否已有一条边抵达父结点，标记后再访问到父结点时正常更新。
