...
在无权图中，BFS 算法找到的路径是从起点开始的 **最短** 合法路径。换言之，这条路径所包含的边数最小。这使得在 BFS 结束时，每个节点都是通过从起点到该点的最短路径访问的。

在带权图中，BFS 算法仍然可以保证这条路径所包含的边数最小，但并不一定找到最短路径，若要找最短路径，请参见 [最短路](./shortest-path/)。
