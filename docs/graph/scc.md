...
有向图 G 上运行 DFS 算法后，所有发现未访问节点的边会组成一棵树（基图为非连通图的情况下则是组成森林），这颗树叫 **DFS 生成树**（DFS 生成森林）．需要注意的是，这棵树的形态以及下述分类是与 DFS 的初始访问节点有关的．

有向图 G 的边主要有 4 种边（不一定全部出现）：

1.  树边（tree edge）：示意图中以黑色边表示，每次搜索找到一个还没有访问过的结点的时候就形成了一条树边．所有的树边组成 DFS 生成树 $G_\pi$，以下三种分类都是非树边，是在有向图 G 里，但不在 DFS 生成树里的边．
2.  反祖边（back edge）：示意图中以红色边表示（即 $7 \rightarrow 1$），也被叫做回边，即非树边、指向祖先结点的边．
3.  前向边（forward edge）：示意图中以绿色边表示（即 $3 \rightarrow 6$），它是在搜索的时候遇到子树中的结点的时候形成的，即非树边、指向后代结点的边．
4.  横叉边（cross edge）：示意图中以蓝色边表示（即 $9 \rightarrow 7$），它主要是在搜索的时候遇到了一个已经访问过的结点，但是这个结点 **并不是** 当前结点的祖先，即有向图 G 中不属于上述三种分类的边．
