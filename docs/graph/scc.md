...
在有向图 $G$ 上运行 DFS 算法时，由于边具有方向性，从单个结点出发通常无法访问到图中的全部结点．因此，我们需要遍历整个顶点集：对每个尚未被访问的结点，都重新发起一次 DFS．在每一次从某个起始结点出发并完成的 DFS 过程中，其所经过的树边（见下文）会构成一棵树，称为 **DFS 生成树**．当所有结点都被访问后，得到的 DFS 生成树的全体构成了该有向图的 **DFS 生成森林**．
