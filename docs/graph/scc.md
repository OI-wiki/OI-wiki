...
有向图 G 上运行 DFS 算法后，所有发现未访问节点的边会组成一棵树（基图为非连通图的情况下则是组成森林），这颗树叫 **DFS 生成树**（DFS生成森林）． 需要注意的是，这棵树的形态以及下述分类是与DFS的初始访问节点有关的．

有向图 G 的边主要有 4 种边（不一定全部出现）：

1.  树边（tree edge）：示意图中以黑色边表示，每次搜索找到一个还没有访问过的结点的时候就形成了一条树边．所有的树边组成DFS生成树 $G_\pi$ ，以下三种分类都是非树边，是在有向图G里，但不在DFS生成树里的边．
