...
在有向图 $G$ 上运行 DFS 算法时，由于边的有向性，从随意选取的单个结点出发往往无法访问到图中的所有结点．因此，我们通常需要遍历整个顶点集，对每个尚未访问的结点发起 DFS．每次完整的 DFS 调用所经过的树边（见下文）会构成一棵树，称为 **DFS 生成树**；所有这些生成树共同构成了该有向图的 **DFS 生成森林**．需要注意的是，生成森林的具体形态以及下面的边分类，都与 DFS 的起始结点选取和遍历顺序有关．

有向图 $G$ 的边可分为四类：

1.  树边（tree edge）：示意图中以黑色边表示，每次搜索找到一个还没有访问过的结点的时候就形成了一条树边．所有的树边组成 DFS 生成树．
