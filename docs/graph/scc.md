...
Tarjan 算法基于对图进行 [深度优先搜索](https://oi-wiki.org/graph/dfs/)。我们视每个连通分量为搜索树中的一棵子树，在搜索过程中，维护一个栈，每次把搜索树中尚未处理的结点加入栈中。
...
按照深度优先搜索算法搜索的次序对图中所有的结点进行搜索，维护每个结点的 `dfn` 与 `low` 变量，且让搜索到的结点入栈。每当找到一个强连通元素，就按照该元素包含结点数目让栈中元素出栈。在搜索过程中，对于结点 $u$ 和与其相邻的结点 $v$（$v$ 不是 $u$ 的父结点）考虑 3 种情况：
...
Tarjan 算法在处理过程中，实际上是按照某种 **逆拓扑序** 来发现强连通分量的，这是因为算法在深度优先搜索的过程中会先访问那些没有出边的结点，而这与拓扑排序的过程是相反的。

如果我们将图中的所有强连通分量缩成单个结点，那么在这些缩点后的结点形成的 DAG 中进行拓扑排序，得到的顺序将与 Tarjan 算法给出的强连通分量的标号顺序相反。

因此，可以说，在缩点后的 DAG 中，**强连通分量（缩点后）的标号顺序是其拓扑序的逆序**。但要注意的是，这种说法仅在考虑了强连通分量之间的依赖关系（即从一个强连通分量到另一个强连通分量的有向边）时才成立。单个强连通分量内部的结点由于存在环，所以内部并不满足拓扑序的定义。
...
Garbow 算法是 Tarjan 算法的另一种实现，Tarjan 算法是用 dfn 和 low 来计算强连通分量的根，Garbow 维护一个结点栈，并用第二个栈来确定何时从第一个栈中弹出属于同一个强连通分量的结点。从结点 $w$ 开始的 DFS 过程中，当一条路径显示这组结点都属于同一个强连通分量时，只要栈顶结点的访问时间大于根结点 $w$ 的访问时间，就从第二个栈中弹出这个结点，那么最后只留下根结点 $w$。在这个过程中每一个被弹出的结点都属于同一个强连通分量。

当回溯到某一个结点 $w$ 时，如果这个结点在第二个栈的顶部，就说明这个结点是强连通分量的起始结点，在这个结点之后搜索到的那些结点都属于同一个强连通分量，于是从第一个栈中弹出那些结点，构成强连通分量。
