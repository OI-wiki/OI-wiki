...
2. 反祖边（back edge）：示意图中以红色边表示，也被叫做回边，即指向祖先结点的边，如 $7 \rightarrow 1$。
3. 横叉边（cross edge）：示意图中以蓝色边表示，它主要是在搜索的时候遇到了一个已经访问过的结点，但是这个结点 **并不是** 当前结点的祖先时形成的，如 $9 \rightarrow 7$。
4. 前向边（forward edge）：示意图中以绿色边表示，它是在搜索的时候遇到子树中的结点的时候形成的，如 $3 \rightarrow 6$。
