...
???+ question "问题"
    给出一个图，问其中的由 $n$ 个节点构成的边权和最小的环 $(n\geqslant 3)$ 是多大。
...
问题转化为求 $v\leadsto u$ 的路径。由三角不等式 $dis_{u,v}\leqslant dis_{u,i}+dis_{i,v}$，考虑记录 $pos_{u,v}=j$ 表示使得 $dis_{u,v}=dis_{u,j}+dis_{j,v}$ 的点。显然 $j$ 就在 $v\leadsto u$ 的路径上。
