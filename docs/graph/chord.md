...
**Lemma 1**：团数 $\omega(G)\leqslant \chi(G)$ 色数

证明：考虑单独对最大团的导出子图进行染色，至少需要 $\omega(G)$ 种颜色。

**Lemma 2**：最大独立集数 $\alpha(G)\leqslant \kappa(G)$ 最小团覆盖数
...
证明：极小点割集大小 $\leqslant 1$ 时，导出子图一定为一个团。

否则，设极小点割集上有两点为 $x,y$，由 **Lemma 5** 得，$N(x)$ 中有 $V_1,V_2$ 中的点，设为 $x_1,x_2$，同样的，设 $y_1,y_2$，注意，可能有 $x_1=y_1,x_2=y_2$。

由于 $V_1,V_2$ 均为连通块，则在 $x_1,y_1$ 和 $x_2,y_2$ 两个点对之间存在最短路径。设 $x,y$ 在 $V_1,V_2$ 内部的最短路为 $x-x_1\sim y_1-y,x-x_2\sim y_2-y$，则图上存在一个环 $x-x_1\sim y_1-y-y_2\sim x_2-x$，该环的大小一定 $\geqslant 4$，根据弦图的定义，此时该环上一定存在一条弦。
...
**Lemma 10**：任意一个弦图一定不存在一个序列 $v_0,v_1,\dots,v_k(k\geqslant 2)$ 满足下列性质：
...
考虑最小的 $j\in(1,k]$ 满足 $v_jx$ 相连，我们可以推出 $v_0x$ 不相连，否则 $v_0v_1\cdots v_jx$ 构成了一个长度 $\geqslant 4$ 且无弦的环。
...
$A\subsetneqq B$ 当且仅当 $|A|+1\leqslant |B|$。

问题转化为判断是否存在 $y$，满足 $nxt_y=x$ 且 $|N(x)|+1\leqslant |N(y)|$。时间复杂度 $O(n+m)$。
...
正确性证明：设以上方法使用了 $t$ 种颜色，则 $t\geqslant \chi(G)$。由于团上每个点都是不同的颜色，所以 $t=\omega(G)$，由 **Lemma 1**，$t=\omega(G)\leqslant \chi(G)$。综上，可得 $t=\chi(G)=\omega(G)$。
...
正确性证明：设以上方案独立集数和团覆盖数为 $t$，由定义得 $t\leqslant \alpha(G),t\geqslant \kappa(G)$，由 **Lemma 2** 得，$\alpha(G)\leqslant \kappa(G)$，所以 $t=\alpha(G)=\kappa(G)$。
