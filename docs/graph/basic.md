...
1. 第$i$条边是从$U_i->V_i$，边权为$W_i$。
2. 第$i$个点的第一条边的编号是$F_i$。
3. 第$i$条边的下一条以$U_i$为起点的边的编号是$N_i$（如果没有就是-1）。

看到这里，一定能够看出这几个数组的大小了！$U,V,W,N$最多需要边数个，而$F$需要点数个。

怎么存呢？进来第$i$条边，首先将$U_i,V_i,W_i$赋值之后，先将$N_{F_{U_i}}$赋值为i，再将$F_{U_i}$赋值为i即可。（有点绕，慢慢理解）
