...
邻接矩阵的英文名是 adjacency matrix。它的形式是 `bool adj[n][n]` ，这里面 $n$ 是结点个数， $adj[i][j]$ 表示 $i$ 和 $j$ 之间是否有边。
...
选取一个结点的子集，再选取这些结点相关联的边的集合构成的图。

#### 边导出子图

选取一个边的子集，再选取这些边相关联的结点的集合，构成的图。
