...
匈牙利算法又称为 **KM** 算法，可以在 $O(V^3)$ 时间内求出二分图的 **最大权完美匹配**。
...
交错树新增一个点进入 $S$ 的时候需要 $O(V)$ 更新 $slack(v)$。修改顶标需要 $O(V)$ 给每个 $slack(v)$ 减去 $a$。只要交错树找到一个未匹配点，就找到增广路。

一开始枚举 $V$ 个点找增广路，为了找增广路需要延伸 $V$ 次交错树，每次延伸需要 $V$ 次维护，共 $O(V^3)$。
