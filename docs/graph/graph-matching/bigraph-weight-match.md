...
???+ note "可行顶标"
    给每个结点 $i$ 分配一个权值 $l(i)$，对于所有边 $(u,v)$ 满足 $w(u,v) \leq l(u) + l(v)$。
...
???+ note "算法证明"
    ...
    -   每一次权重矩阵被修改，都关系到一个特定结点，这个结点可能是左边的也可能是右边的，因此我们直接记为 $x$, 这个结点和某个结点 $y$ 在原来的最优匹配中匹配上了。每一次修改操作，最多让这一对结点 unpair，因此我们只要跑一轮匈牙利算法中的搜索我们就能得到一个新的 match，而根据定理一，新跑出来的 match 是最优的。
