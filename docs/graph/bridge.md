    ...
    void Tarjan(int u, int father)  // u 当前点的编号，father 自己爸爸的编号
    {
      ...
      for (auto v : edge[u])     // 访问这个点的所有邻居 （C++11）
      {
        if (!vis[v]) {
          ...
          low[u] =
              min(low[u], num[v]);  // 如果这个点不是自己，更新能到的最小结点编号
      }
      if (father == u && child >= 2 &&
...
割边是和是不是根结点没关系的，原来我们求割点的时候是指点 $v$ 是不可能不经过父结点 $u$ 为回到祖先结点（包括父结点），所以顶点 $u$ 是割点。如果 $low_v=num_u$ 表示还可以回到父结点，如果顶点 $v$ 不能回到祖先也没有另外一条回到父亲的路，那么 $u-v$ 这条边就是割边。
