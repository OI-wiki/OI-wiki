...
**树链剖分**（树剖/链剖）分为 **重链剖分**、**长链剖分** 和 **实链剖分**（用于 Link/cut Tree）三种，大多数情况下（没有特别说明时），“树链剖分” 都指 “重链剖分”，本文所讲的也是 “重链剖分”。
...
对于每个询问，最多经过 $\mathcal O(\log n)$ 条重链，每条重链上线段树的复杂度为 $\mathcal O(\log n)$，因此总时间复杂度为 $\mathcal O(n\log n+q\log^2 n)$。实际上复杂度很难跑满，即使跑满了，由于每条重链长度不足 $n$，常数也会较小。
