...
定义 **重子节点** 表示其子节点中子树最大的子节点。如果有相同的，任意取。如果没有子节点，就没有。
...
第一个 DFS 记录每个节点的深度（deep）、子树大小（size）。
...
第二个 DFS 记录每个节点的重子节点（heavy-son）、重边优先遍历时的 DFN 序、所在链的链顶（top，且应初始化为节点本身）。
...
TREE - PATH - SUM(u, v) while u,
    ...
    swap(u, v) 将 u 到 u 所在链的链顶 之间的节点权值求和，累加到计数器中
    u = u 所在链链顶的父节点 将 u,
    v 之间的节点的权值求和累加，返回计数器的值
```
...
有时会要求，维护子树上的信息，譬如将以 x 为根的子树的所有节点的权值增加 v。

在 DFS 搜索的时候，子树中的节点的 DFN 序是连续的。

每一个节点记录 bottom 表示所在子树连续区间末端的节点。

这样就把子树信息转化为连续的一段区间信息。

### 求最近公共祖先

不断向上跳链，当跳到同一条链上时，返回深度较小的节点即为 LCA。
