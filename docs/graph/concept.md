...
如果一张图可以画在一个平面上，且没有两条边在非端点处相交，那么这张图是一张 **平面图 (planar graph)**．一张图的任何子图都不是与 $K_5$ 或 $K_{3, 3}$ **同胚** 的图（**同胚** 指可通过在边上反复插入或删除度数为 2 的顶点而互相转化，包括零次操作，即一个图总与自身同胚）是其为一张平面图的充要条件．对于简单连通平面图 $G=(V, E)$ 且 $V\ge 3$，$|E|\le 3|V|-6$．
