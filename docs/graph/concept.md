...
**途径 (Walk)**/**链 (Chain)**：途径是一个将若干个点连接起来的边的集合。形式化地说，途径 $w$ 是一个边的集合 $\{e_1, e_2, \ldots, e_k\}$，这个边集需要满足条件：存在一个由点构成的序列 $v_0, v_1, \ldots, v_k$ 满足 $e_i$ 的两个端点分别为 $v_{i-1}$ 和 $v_i$。这样的路径可以简写为 $v_0 \to v_1 \to v_2 \to \cdots \to v_k$。通常来说，边的数量 $k$ 被称作这条途径的 **长度**（如果边是带权的，长度通常指路径上的边权之和，题目中也可能另有定义）。
