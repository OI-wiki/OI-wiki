...
2-SAT 与 CNF-SAT 类似，但是所有的方程只有两个变量．简单的说就是给出 $n$ 个布尔方程，每个方程和两个变量相关，如 $a \vee b$，表示变量 $a, b$ 至少满足一个．然后判断是否存在可行方案，一般题中只需要求出一种即可．
...
下证 $(a \to b) \iff (\neg b \to \neg a)$（即，在图上有 $a \to b$ 的边的时候，图上必然也有 $\neg b \to \neg a$ 的边）为上述算法正确的充分条件。

考虑无解的情况。对于 $a$，$a$ 不能成立当且仅当存在 $b$ 使得 $a$ 能够到达 $b$ 和 $\neg b$。对于 $a = b$，上述算法可以正确的进行判断；但是，对于 $a \neq b$，上述算法可能使一个不能成立的 $x$ 成立。

但是，当上文所述条件成立时，可以证明上述算法不会使一个不能成立的 $x$ 成立。考虑 $x \to b_1 \to b_2 \to \cdots \to b_k \to a$ 和 $x \to c_1 \to c_2 \to \cdots \to c_l \to \neg a$ 两条路径同时在图中存在。由于上述条件，$\neg a \to b_k \to b_{k - 1} \to \cdots \to b_1 \to \neg x$ 也在图中存在，所以 $x$ 可以到达 $\neg x$，不会被上述算法选到。

故在 $(a \to b) \iff (\neg b \to \neg a)$ 的情况下，上述算法必然正确。然而，一旦该条件没有满足，上述算法的正确性就无法保证。比如，如下图所示的情况，Tarjan SCC 会误判有解，实际上却无解。
