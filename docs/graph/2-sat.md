...
可以发现，在建图的过程中，$a \vee b$ 变成了两个形如 $a_1 \to b_1$ 的 **独立** 的条件．因此，这个算法可以处理一部分不满足上面通式的问题．具体处理方式会在后文提及．
...
考虑对于某一个人，如果这个人的某一盏灯猜错了，那么另外两盏灯必须猜对．因此可以考虑对每一个灯像上面的方式一样建点，于是前面的限制每一个可以变成 $a_1 \to a_2 \land a_1 \to a_3$ 的形式，连边然后 SCC 即可．注意有三盏灯．
