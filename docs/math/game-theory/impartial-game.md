...
Sprague–Grundy 理论指出，所有公平组合游戏都等价于单堆 Nim 游戏。
...
本节列举了一些常见的公平游戏及其结论。叙述结论时，本节只给出了必胜和必败状态的判断法则。至于必胜策略，就是进行恰当的操作，使得留给对手的局面恰好为必败状态。
...
相较于 Nim 游戏，Moore 的 Nim-$k$ 游戏允许一次性从 $k$ 个石子堆中取石子。

???+ abstract "Moore 的 Nim-$k$ 游戏 "
