...
我们考虑平行向量，可以任做一条直线与这些向量平行，那么任一组平行向量都可以平移到同一直线上，所以平行向量又叫**共线向量**。
...
我们可以取与 $x$ 轴与 $y$ 轴方向相同的单位向量 $i,j$ 作为一组基底，可以发现，$\overrightarrow p=x\overrightarrow i+y\overrightarrow j$，其中 $\vec p$ 为平面内任意向量。

由于平面向量基本定理，我们发现这样的实数对 $(x,y)$ 是唯一的。这样，平面内的任意向量都可以用有序实数对唯一确定，我们把 $(x,y)$ 叫做向量的**坐标**。记作：$\vec p=(x,y)$。

这样没问题？是的，因为有序实数对 $(x,y)$ 与平面直角坐标系上的点唯一确定，那么我们做 $\overrightarrow{OP}=\vec p$，那么终点 $P(x,y)$ 也是唯一确定的。由于我们研究的都是自由向量，可以自由平移起点，这样，在平面直角坐标系里，每一个向量都可以用有序实数对唯一表示。
...
有了坐标运算，我们知道有 $A(a,b),B(c,d)$，想知道 $\overrightarrow{AB}$ 就不难了，只需要 $\overrightarrow{OB}-\overrightarrow{OA}$ 即可，也就是 $\overrightarrow{AB}=(c-a,d-b)$。
