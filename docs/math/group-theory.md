...
拉格朗日定理：如果 $H$ 是 $G$ 的子群，那么 $\lvert G \rvert = [G : H] \lvert H \rvert$，其中 $[G : H]$ 表示 $G$ 中 $H$ 的左陪集数（等价于右陪集数），$\lvert G \rvert$ 表示 $G$ 的阶（即元素的个数，在后面会提到）。
...
**商群**（quotient group）或因子组（factor group）是通过使用保留一些群结构的等价关系聚合更大群的相似元素获得的群。
...
???+note "有关阶的常见误区"
    ...
    反例：循环群 $S_3$ 中存在阶为 $2$ 和 $3$ 的元素，却不存在阶为 $6$ 的元素。
