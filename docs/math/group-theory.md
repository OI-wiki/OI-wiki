...
例如，群 $(\mathbb{Z}/n\mathbb{Z})^\times = \{ a \in \{ 0, 1, \cdots, n-1 \} \mid \gcd(a, n) = 1 \}$ 的阶为 $\varphi(n)$，其中元素 $x$ 的阶为满足 $x^r \equiv 1 \pmod n$ 的最小正整数 $r$（这正是数论中 $x$ 模 $n$ 的阶）。
...
阶为 $m$ 的有限循环群 $G$ 同构于模 $m$ 剩余类对于加法构成的群 $\mathbb{Z}/n\mathbb{Z}$。
