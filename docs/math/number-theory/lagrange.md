...
???+note "证明"
    ...
    设m与n的最大公约数为d，$m=m_0d$，那么上述方程可以简化为$d(x-x_{n+1})\equiv 0\pmod {m_0d}。所有的解 $x$ 在模 $m_0$ 意义下都与 $x_{n+1}$ 同余，根据中国剩余定理，在模 $m_0d$ 意义下的 $x$ 至多有 $d$ 个，而 $d$ 是 $n$ 的约数，不可能大于 $n$，这与假设矛盾，因此原命题成立。
