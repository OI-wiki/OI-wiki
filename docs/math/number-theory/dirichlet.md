...
$$
F(x) = \sum_{n=1}^{\infty}\dfrac{f(n)}{n^x}.
$$

级数中的 $x$ 是形式变元．常见的 Dirichlet 生成函数中，$x$ 往往可以看作是复变量，进而讨论 Dirichlet 级数的解析性质，但这超出了算法竞赛的范围．
...
??? note "证明"
    ...
    $$
    \begin{aligned}
    F(x)G(x) &= \left(\sum_{k=1}^\infty\dfrac{f(k)}{k^x}\right)\left(\sum_{\ell=1}^\infty\dfrac{g(\ell)}{\ell^x}\right)= \sum_{k=1}^\infty\sum_{\ell=1}^\infty\dfrac{f(k)g(\ell)}{(k\ell)^x}\\
    &= \sum_{n=1}^{\infty}\dfrac{\sum_{k\ell = n}f(k)g(\ell)}{n^x} = \sum_{n=1}^\infty\dfrac{(f\ast g)(n)}{n^x}.
    \end{aligned}
    $$
...
积性函数的特殊性同样反映在 Dirichlet 生成函数上．由于整数有 [唯一分解定理](./basic.md#算术基本定理)，积性函数 $f(n)$ 的生成函数 $F(x)$ 可写成如下形式：

$$
\begin{aligned}
F(x) &= \sum_{n=1}^{\infty}\dfrac{f(n)}{n^x} = \sum_{n=1}^{\infty}\prod_{p\in\mathbf P}\dfrac{f(p^{e})}{p^{ex}} = \prod_{p\in\mathbf P}\sum_{e=0}^{\infty}\dfrac{f(p^e)}{p^{ex}}\\
&= \prod_{p\in\mathbf P}\left(1 + \dfrac{f(p)}{p^x} + \dfrac{f(p^2)}{p^{2x}} + \dfrac{f(p^3)}{p^{3x}} + \cdots\right).
\end{aligned}
$$

这意味着，$F(x)$ 可以分解为若干 $F_p(x)$ 的乘积，且每个 $F_p(x)$ 对应的数论函数都只在 $p$ 的幂次处可能取非零值．这一无穷乘积也称为 **Euler 乘积**（Euler product）．如果 $F(x)$ 和 $G(x)$ 都能分解成类似的形式，那么它们的乘积同样如此；将这一观察对应到数论函数上，就是积性函数的 Dirichlet 卷积仍然是积性函数．

进一步地，如果 $f(n)$ 还是完全积性函数，那么 $f(p^e)=f(p)^e$，上式可以继续简化：

$$
F(x) = \prod_{p\in\mathbf P}\sum_{e=0}^{\infty}\dfrac{f(p)^e}{p^{ex}} = \prod_{p\in\mathbf P}\left(1-\dfrac{f(p)}{p^x}\right)^{-1}.
$$

与积性函数不同，完全积性函数的 Dirichlet 生成函数的形式在乘法运算下并不具有封闭性，因此，完全积性函数的 Dirichlet 卷积和 Dirichlet 逆都未必是完全积性函数，但一定是积性函数．

???+ example "例子"
    1.  单位函数 $\varepsilon(n)$ 是完全积性函数．它的 Dirichlet 生成函数是关于不定元 $x$ 的常值函数
    
        $$
        E(x) = \sum_{n=1}^{\infty}\dfrac{\varepsilon(n)}{n^x} = 1.
        $$
    
    2.  常数函数 $1(n)$ 是完全积性函数．它的 Dirichlet 生成函数是 Riemann 函数
    
        $$
        I(x) = \sum_{n=1}^{\infty}\dfrac{1}{n^x} = \prod_{p\in\mathbf P}\dfrac{1}{1-p^{-x}} = \zeta(x).
        $$
    
    3.  莫比乌斯函数 $\mu(n)$ 是常数函数的 Dirichlet 逆．它的 Dirichlet 生成函数是 $\zeta(x)$ 的倒数：
    
        $$
        M(x) = \sum_{n=1}^{\infty}\dfrac{\mu(n)}{n^x} = \prod_{p\in\mathbf P}(1-p^{-x}) = \dfrac{1}{\zeta(x)}.
        $$
    
    4.  幂函数 $\operatorname{id}_k(n)=n^k$ 是完全积性函数．特别地，当 $k=0$ 时，它就是常数函数；当 $k=1$ 时，它就是恒等函数．它的 Dirichlet 生成函数是
    
        $$
        I_k(x) = \sum_{n=1}^{\infty}\dfrac{n^k}{n^x} = \prod_{p\in\mathbf P}\dfrac{1}{1-p^{k-x}} = \zeta(x-k).
        $$
    ...
    5.  欧拉函数 $\varphi(n)$ 是积性函数．它的 Dirichlet 生成函数是
        ...
        $$
        \begin{aligned}
        \Phi(x) &= \prod_{p\in\mathbf P}\left(1+\dfrac{p-1}{p^x} + \dfrac{p(p-1)}{p^{2x}} + \dfrac{p^2(p-1)}{p^{3x}}+\cdots\right)\\
        &= \prod_{p\in\mathbf P}\left(\dfrac{1}{1-p^{1-x}}-\dfrac{1}{p^x}\dfrac{1}{1-p^{1-x}}\right) = \prod_{p\in\mathbf P}\dfrac{1-p^{-x}}{1-p^{1-x}} = \dfrac{\zeta(x-1)}{\zeta(x)}.
        \end{aligned}
        $$
    ...
    6.  约数函数 $\sigma_k(n)=\sum_{d\mid n}d^k$ 是积性函数．它的 Dirichlet 生成函数是
        ...
        $$
        \begin{aligned}
        \Sigma_k(x) &= \prod_{p\in\mathbf P}\left(1+\dfrac{1+p^k}{p^x}+\dfrac{1+p^k+p^{2k}}{p^{2x}}+\dfrac{1+p^k+p^{2k}+p^{3k}}{p^{3x}}+\cdots\right) \\
        &= \prod_{p\in\mathbf P}\dfrac{1}{1-p^k}\left((1-p^k)+\dfrac{1-p^{2k}}{p^x}+\dfrac{1-p^{3k}}{p^{2x}}+\dfrac{1-p^{4k}}{p^{3k}}+\cdots\right)\\
        &= \prod_{p\in\mathbf P}\dfrac{1}{1-p^k}\left(\dfrac{1}{1-p^{-x}} - \dfrac{p^k}{1-p^{k-x}}\right)\\
        &= \prod_{p\in\mathbf P}\dfrac{1}{(1-p^{-x})(1-p^{k-x})} = \zeta(x-k)\zeta(x).
        \end{aligned}
        $$
    ...
    7.  无平方因子数的指示函数 $u(n)=|\mu(n)|$ 是积性函数．它的 Dirichlet 生成函数是
        ...
        $$
        U(x) = \prod_{p\in\mathbf P}(1+p^{-x}) = \prod_{p\in\mathbf P}\dfrac{1-p^{-2x}}{1-p^{-x}} = \dfrac{\zeta(x)}{\zeta(2x)}.
        $$
...
$$
F(x) = \prod_{p\in\mathbf P}\left(1 + \sum_{k=1}^{\infty}\dfrac{p^{3k-1}(p-1)}{p^{kx}}\right) = \prod_{p\in\mathbf P}\dfrac{1-p^{2-x}}{1-p^{3-x}} = \dfrac{\zeta(x-3)}{\zeta(x-2)}.
$$
...
$$
H(x) = F(x)G(x) = F(x)\prod_{p\in\mathbf P}G_p(x).
$$

其中，$G_p(x)$ 是 $G(x)$ 的 Euler 乘积分解中的因式，它只包含 $p$ 的幂次处的系数：

$$
G_p(x) = \sum_{p^k\le n}\dfrac{f(p^k)}{p^{kx}} = 1 + \dfrac{f(p)}{p^x} + \dfrac{f(p^2)}{p^{2x}} + \cdots.
$$

那么，从 $F(x)$ 开始，遍历所有不超过 $n$ 的素数 $p$，将 $G_p(x)$ 逐一乘上去，同样可以得到最终结果 $H(x)$．将 $G_p(x)$ 乘上去时，直接应用一般情形中的暴力枚举算法即可．总枚举次数
