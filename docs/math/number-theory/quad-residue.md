...
二次剩余符号总共有三种：勒让德符号、雅可比符号、克罗内克符号。

三者书写形式完全相同。由于它们是逐步推广的关系，二次剩余符号一般指克罗内克符号。

### 勒让德符号

勒让德（Legendre）符号的定义域是：上方为全体整数，下方限制为奇素数。
...
通过勒让德符号可以判断一个数 $a$ 是否为二次剩余，具体判断 $a$ 是否为模 $p$ 的二次剩余，需要通过欧拉判别准则来实现。

下表为部分勒让德符号的值![](./images/quad_residue.png)

### 雅可比符号

雅可比（Jacobi）符号的定义域是：上方为全体整数，下方限制为大于等于 $3$ 的奇数。也就是说，下方的数从勒让德符号规定的奇素数推广为大于等于 $3$ 的奇数。

当下方的数为奇素数时，雅可比符号与勒让德符号完全一致。

雅可比符号满足上方与下方的完全积性。当下方的数只是一般的奇数，不为奇素数时，按照完全积性的办法定义。

$$
\left(\frac{a}{pq}\right)=\left(\frac{a}{p}\right)\left(\frac{a}{q}\right)
$$

注意，雅可比符号只是勒让德符号的推广。勒让德符号与二次剩余有着直接关系，但是雅可比符号与二次剩余没有直接关系。例如，对于雅可比符号，有：

$$
\left(\frac{2}{15}\right)=\left(\frac{2}{3}\right)\left(\frac{2}{5}\right)=1
$$

然而 $2$ 不是模 $15$ 的二次剩余，判断模 $15$ 的二次剩余仍旧需要分别计算模 $3$ 的二次剩余与模 $5$ 的二次剩余。

雅可比符号在二次互反律中有重要应用，在二次互反律中规定的二次剩余符号，一般是指雅可比符号。

借助二次互反律可以避过大数分解，仅使用更相减损术算法，递归计算大数的二次剩余符号。

### 克罗内克符号

克罗内克（Kronecker）符号的定义域是：上方与下方均为全体整数。也就是说，下方的数从雅可比符号规定的大于等于 $3$ 的奇数推广为全体整数。

当下方的数为奇素数时，克罗内克符号与雅可比符号完全一致。

克罗内克符号也满足上方与下方的完全积性，从而也可以按照完全积性的办法定义。从大于等于 $3$ 的奇数推广到全体整数，只需要引入下方为 $0$、$1$、$-1$ 与 $2$ 四个值的定义即可。

当下方为 $1$ 时，克罗内克符号取值恒为 $1$。

$$
\left(\frac{a}{1}\right)=1
$$

当下方为 $0$ 时，仅当上方为 $1$ 或 $-1$ 时，克罗内克符号取值为 $1$；否则取值为 $0$。

$$
\left(\frac{a}{0}\right)=\begin{cases}
1,\,&a\in\{1\, -1\}\\
0,\,&\text{else}
\end{cases}
$$

当下方为 $-1$ 时，仅当上方大于等于 $0$ 时，克罗内克符号取值为 $1$；否则取值为 $-1$。
...
当下方为 $2$ 时，克罗内克符号的定义与模 $8$ 的余数相关：余 $1$ 或 $7$，取值为 $1$；余 $3$ 或 $5$，取值为 $-1$；其余取值为 $0$。

$$
\left(\frac{a}{2}\right)=\begin{cases}
1,\,&a\equiv 1\, 7\pmod{8}\\
-1,\,&a\equiv 3\, 5\pmod{8}\\
0,\,&\text{else}
\end{cases}
$$

注意，克罗内克符号在下方引入了 $-1$，这导致原本在雅可比符号中成立的上方同余值相同的定理，在克罗内克符号中仅在非负整数成立。即：

$$
\left(\frac{a+bq}{b}\right)=\left(\frac{a}{b}\right)
$$

这个定理只在 $b$ 为非负整数时成立，在 $b$ 为负整数时不成立。

由于克罗内克符号将上方和下方的定义域均推广至全体整数，在不引起混淆的情况下，二次剩余符号与克罗内克符号两者的概念是等同的。
