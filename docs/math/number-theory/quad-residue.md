...
???  note "为什么期望只需要两步"
    感性理解：由于模 $p$ 意义下的二次剩余和二次非剩余都有 $\frac{p-1}{2}$ 个，每次随机都有一半的概率找到一个二次非剩余，于是期望步数就是 $1+\frac{1}{2}+\frac{1}{4}+... \eq 2$

    上面的证明显然是不严谨的，因为 $r^2-a$ 并不一定可以取遍 $[0, p-1]$，下面给出一个严谨的证明：

    考虑 $r^2-a$ 为二次剩余的情况，则存在一个 $x$ 使得 $r^2-a \equiv x^2 \pmod p$，移项可得 $(r+x)(r-x) \equiv a \pmod p$，不难发现对于每一个 $(r+x) \in [1, p-1]$，都一一对应于一组 $(r,x)$ 的解，所以使原方程成立的解一共有 $p-1$ 组。我们分类讨论 $x \equiv 0$ 和 $x \not\equiv 0$ 两种情况。对于 $x \equiv 0$，由于 $a$ 是二次剩余，对应了 $2$ 种 $r$ 的取值；对于 $x \not\equiv 0$，有 $p-1-2$ 种情况，每一个 $r$ 对应其中两种，一共有 $\frac{p-3}{2}$ 种 $r$ 的取值。综上，一共有 $2+\frac{p-3}{2}\eq\frac{p+1}{2}$ 种情况使得 $r^2-a$ 为二次剩余，所以每随机一次得到二次非剩余的概率就是 $\frac{p-1}{2p} \approx \frac{1}{2}$ 接下来期望步数的求法于上述感性理解方法相同。

于是我们定义一个模 $p$ 意义下的虚数单位 $i$ 满足 $i^2=r^2-a$。类比实数域到复数域的扩展，$i$ 只是形式上的记号，没有实际意义，此时我们扩展出了一个模 $p$ 意义下的复数域，其中任何一个数都可以表示为 $A+Bi$ 的形式，参照复数，我们下文中为了方便称其中 $A$ 为实部，$B$ 为虚部， 运算规则可以参考复数的运算理解。

???+ note "模 $p$ 意义下复数域中的运算规则"
    加法：$(A+Bi)+(C+Di) \equiv (A+C)+(B+D)i \pmod p$

    减法：$(A+Bi)-(C+Di) \equiv (A-C)+(B-D)i \pmod p$

    乘法：$(A+Bi)*(C+Di) \equiv AC+(AD+BC)i+BDi^2 \equiv AC+(AD+BC)i+BD(r^2-a) \equiv AC+BD(r^2-a)+(AD+BC)i \pmod p$
