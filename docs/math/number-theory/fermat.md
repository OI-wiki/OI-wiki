...
4.  **合数**：$a$ 为合数的情况，该式成立。
    ...
    **证明**：
    ...
    -   只证 $a$ 拆成两个素数的幂的情况，大于两个的用数学归纳法可证。
        ...
        则 $s \mid \operationname{lcm}(s_1,s_2)$，由于 $s_1 \mid \varphi(m),s_2 \mid \varphi(m)$，那么 $\operatorname{lcm}(s_1,s_2) \mid \varphi(m)$，所以 $s \mid \varphi(m)$，$r=\max(\lceil \frac{r_i}{k_i} \rceil) \le \max(r_i) \le \varphi(m)$；
