...
在证明之前先给出一个直观的理解。见 PeterlitsZo 给出的图：

![](./images/eular-rho.png)

我们需要知道的是，在 $\pmod m$ 的条件下，$a^b \bmod m$ 的取值范围一定在 $[0, m)$，而 $a^i \bmod m = (a^{i-1} \bmod m) \times a \bmod m$，那么对于任意一个数 $a$，那么我们很容易就能知道它的 **后驱**，在有限的空间内这一定会形成一个循环。

根据 [李如一的见解](https://cometoj.com/live/10614/replay?replayId=13894&page=3)，循环分为纯循环和混循环（其中纯循环中不存在节点有两个前驱，而混循环则反之，如上图）。而 $a^i \mod n$ 形成的序列可以是一个混循环，那么我们只需要知道循环节的长度，和前面那一小段未进入循环节的长度，就可以根据这个性质来进行降幂了。

值的注意的是，无论是费马小定理，还是（扩展）欧拉定理，一个很重要的目标就是用于降幂的，从而将不可能的表达式化为可能。
