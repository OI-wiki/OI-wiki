...
在这个引理的证明过程中，实际上得到了如下有用的结论：<a id="mod-pow-2"></a>

???+ note "推论"
    对于整数 $e \ge 2$，所有奇数都与唯一一个 $\pm 5^k$ 形式的整数关于模 $2^{e}$ 同余，其中，$k\in\mathbf N$ 且 $k < 2^{e-2}$．也就是说，模 $2^e$ 的意义下，$\pm 1,\pm 5,\cdots,\pm 5^{2^{e-2}-1}$ 两两不同余，且构成一个既约剩余系．

??? note "证明"
    容易验证，$e=2$ 的情形成立．对于 $e \ge 3$ 的情形，由于前述证明中已经得到 $5$ 模 $2^e$ 的阶是 $2^{e-2}$，所以，$1,5,\cdots,5^{2^{e-2}-1}$ 两两不同余．只需要再说明，不存在整数 $i,j$ 使得 $5^{i}\equiv -5^{j}\pmod{2^e}$ 成立．
    
    反证法．不妨设 $k=(i-j)\bmod 2^{e-2}$，依据阶的性质可知，$5^k\equiv 5^{i-j}\equiv -1\pmod{2^e}$．进而，有 $5^{2k} \equiv (-1)^2 = 1 \pmod {2^e}$．由阶的 [性质 2](#ord-prop-2) 可知，$\delta_{2^e}(5)=2^{e-2}\mid 2k$，又知道 $0 < k < 2^{e-2}$，唯一的可能性是 $k=2^{e-3}$．但是，前述证明中已经得到 $5^{2^{e-3}}\equiv 1 + 2^{e-1}\not\equiv -1\pmod{2^{e}}$．
