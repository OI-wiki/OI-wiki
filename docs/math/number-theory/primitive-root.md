...
阶和原根是理解模 $n$ 的 [既约剩余系](./basic.md#同余类与剩余系) $\mathbf Z_m^*$ 的乘法结构的重要工具．基于此，可以定义 [离散对数](./discrete-logarithm.md) 等概念．关于它们的更为一般的讨论可以参见抽象代数部分的 [群论](../algebra/group-theory.md#阶) 和 [环论](../algebra/ring-theory.md#应用整数同余类的乘法群) 等页面的相关章节．

## 阶

本节中，总是假设模数 $m\in\mathbf N_+$ 和底数 $a\in\mathbf Z$ 互素，即 $(a,m)=1$，也记作 $a\perp m$．

对于 $n\in\mathbf Z$，幂次 $a^n\bmod m$ 呈现一种循环的结构．这个循环节的最小长度，就是 $a$ 模 $m$ 的阶．阶就定义为幂 $a^n \bmod m$ 第一次回到起点 $a^0\bmod m = 1$ 时的指数：

???+ abstract "阶"
    对于 $a\in\mathbf Z,m\in\mathbf N_+$ 且 $a\perp m$，满足同余式 $a^n \equiv 1 \pmod m$ 的最小正整数 $n$ 称作 $a$ 模 $m$ 的阶，记作 $\delta_m(a)$ 或 $\operatorname{ord}_m(a)$．

???+ tip "注"
    在 [抽象代数](../algebra/group-theory.md#阶) 中，这里的「阶」就是模 $m$ 既约剩余系关于乘法形成的群中，元素 $a$ 的阶．用记号 $\delta$ 表示阶也只适用于这个特殊的群．下面的诸多性质可以直接推广到抽象代数中阶的性质．
    
    另外还有「半阶」的概念，在数论中会用 $\delta^-$ 记号表示．它是同余式 $a^n \equiv -1 \pmod m$ 的最小正整数．半阶不是群论中的概念．阶一定存在，半阶不一定存在．
...
[欧拉定理](./fermat.md#欧拉定理) 中，同余关系 $a^{\varphi(m)}\equiv 1\pmod m$ 对于所有 $a\perp m$ 都成立．结合性质 2，这说明对于所有 $a\perp m$，都有 $\delta_m(a)\mid\varphi(m)$．换句话说，$\varphi(m)$ 是所有 $a\perp m$ 的阶的一个公倍数．对于一个正整数 $m$，所有 $a\perp m$ 的阶 $\delta_m(a)$ 的最小公倍数，记作 $\lambda(m)$，就是 $m$ 的 [Carmichael 函数](#carmichael-函数)．可以证明，存在 $a\perp m$，使得 $\delta_m(a)=\lambda(m)$ 成立；也就是说，$\lambda(m)$ 也是所有 $a\perp m$ 的阶的最大值．
...
对于一般的情形，性质 4 得到的界已经是紧的．乘积的阶取得下界的情形很容易构造：例如 $(a,b,m)=(3,5,7)$ 时，$\delta_m(a)=\delta_m(b)=6$，但是它们的乘积的阶 $\delta_m(ab)=1$．

尽管在一般的情形下，乘积 $ab$ 的阶未必是它们的阶的最小公倍数，但是总能找到一个元素使得它的阶等于这个最小公倍数．
...
原根是特殊的元素——它的阶就等于所有模 $m$ 既约剩余系的个数．

???+ abstract "原根"
    对于 $m\in\mathbf N_+$，如果存在 $g\in\mathbf Z$ 且 $g\perp m$ 使得 $\delta_m(g)=|\mathbf Z_m^*|=\varphi(m)$，就称 $g$ 为 **模 $m$ 的原根**（primitive root modulo $m$）．

并非所有的正整数 $m$ 都存在模 $m$ 的原根．由上文的性质 1，如果模 $m$ 的原根 $g$ 存在，那么，$g,g^2,\cdots,g^{\varphi(m)}$ 所在的同余类互不相同，构成模 $m$ 的既约剩余系．特别地，对于素数 $p$，余数 $g^i\bmod p$ 对于 $i=1,2,\cdots,p-1$ 两两不同．
...
???+ note "定理"
    对于整数 $m\ge 3$ 和整数 $g\perp m$，那么，$g$ 是模 $m$ 的原根，当且仅当对于 $\varphi(m)$ 的每个素因数 $p$，都有
...
??? note "证明"
    根据原根的定义，所有模 $m$ 的既约同余类都可以写作 $g^k\bmod m$ 的形式，且 $k=1,2,\cdots,\varphi(m)$．由性质 3，这些元素的阶等于
...
???+ note "定理"
    模 $m$ 的原根存在，当且仅当 $m=1,2,4,p^\alpha,2p^\alpha$，其中，$p$ 是奇素数且 $\alpha\in\mathbf N_+$．

为说明这一结论，需要分别讨论如下四种情形：

1.  $m=1,2,4$，原根分别是 $g=0,1,3$，显然存在．

2.  $m=p^{\alpha}$ 是奇素数的幂，其中，$p$ 为奇素数，$\alpha\in\mathbf N_+$．
    ...
    ???+ note "引理 2"
        对于奇素数 $p$ 和 $\alpha \in \mathbf{N}_+$，模 $p^\alpha$ 的原根存在．
    ...
    ??? note "证明"
        ...
        **第二步**：上文选取的 $g$，对于任意 $\alpha\ge 1$，都有 $g^{\varphi(p^\alpha)}\not\equiv 1\pmod{p^{\alpha+1}}$．
        
        对 $g$ 的选取保证了 $\alpha=1$ 时，该式成立．假设该式对于 $\alpha$ 的情形成立，现要证明 $\alpha+1$ 的情形也成立．对于任意 $\alpha \ge 1$，由欧拉定理可知，存在 $\lambda$ 使得
        
        $$
        g^{\varphi(p^\alpha)} = 1 + \lambda p^\alpha
        $$
        
        成立．由归纳假设，$\lambda\perp p$．因为 $\varphi(p^{\alpha+1})=p\varphi(g^\alpha)$，所以
        
        $$
        g^{\varphi(p^{\alpha+1})} = \left(g^{\varphi(p^{\alpha})}\right)^p = (1 + \lambda p^\alpha)^p \equiv 1 + \lambda p^{\alpha+1} \pmod{p^{\alpha+2}}.
        $$
        
        结合 $\lambda\perp p$ 可知，$g^{\varphi(p^{\alpha+1})}\not\equiv 1\pmod{p^{\alpha+2}}$．由数学归纳法可知，命题成立．
        
        **第三步**：上文选取的 $g$，对于任意 $\alpha\ge 1$，都是模 $p^\alpha$ 的原根．
        
        对 $g$ 的选取保证了 $\alpha=1$ 时，命题成立．假设命题对于 $\alpha$ 成立，现在要证明命题对于 $\alpha+1$ 也成立．将 $\delta_{p^{\alpha+1}}(g)$ 简记为 $\delta$．由于 $g^\delta\equiv 1\pmod{p^{\alpha+1}}$，必然也有 $g^\delta\equiv 1\pmod{p^\alpha}$．由归纳假设可知，$\delta_{p^\alpha}(g) = \varphi(p^\alpha)$．因此，由前文阶的性质 2，就有 $\varphi(p^\alpha)\mid\delta$．又由欧拉定理可知，$\delta\mid\varphi(p^{\alpha+1})$．但是，$\varphi(p^{\alpha+1})=p\varphi(p^\alpha)$．因此，只有两种可能：$\delta=\varphi(p^\alpha)$ 或 $\delta=\varphi(p^{\alpha+1})$．但是，第二步的结论说明，$g^{\varphi(p^\alpha)}\not\equiv 1\pmod{p^{\alpha+1}}$．因此，可能性 $\delta=\varphi(p^\alpha)$ 并不成立．唯一的可能性就是 $\delta=\varphi(p^{\alpha+1})$．这就说明 $g$ 是 $p^{\alpha+1}$ 的原根．由数学归纳法，命题对于所有 $\alpha\ge 1$ 都成立．

3.  $m=2p^{\alpha}$，其中，$p$ 为奇素数，$\alpha\in\mathbf N_+$．

    ???+ note "引理 3"
        对于奇素数 $p$ 和 $\alpha \in \mathbf{N}_+$，模 $2p^\alpha$ 的原根存在．

    ??? note "证明"
        设 $g$ 是模 $p^{\alpha}$ 的原根，则 $g+p^\alpha$ 也是模 $p^{\alpha}$ 的原根．两者之间必然有一个是奇数，不妨设它就是 $g$．显然，$(g,2p^\alpha)=1$．设 $\delta=\delta_{2p^\alpha}(g)$，需要证明 $\delta=\varphi(2p^\alpha)$．由欧拉定理，$\delta\mid\varphi(2p^\alpha)$．同时，根据定义 $g^\delta\equiv 1\pmod{2p^\alpha}$，所以，$g^\delta\equiv 1\pmod{p^\alpha}$，因此，由阶的性质 2 可知，$\delta_{p^\alpha}(g)=\varphi(p^\alpha)\mid \delta$．由欧拉函数表达式可知，$\varphi(2p^\alpha) = \varphi(p^\alpha)$．所以，$\delta=\delta_{2p^\alpha}(g)=\varphi(p^\alpha)$．这就说明 $\delta$ 是模 $2p^\alpha$ 的原根．

4.  $m\ne 1,2,4,p^{\alpha},2p^{\alpha}$，其中，$p$ 为奇素数，$\alpha\in\mathbf N_+$．

    ???+ note "引理 4"
        假设 $m\neq 1,2,4$ 且不存在奇素数 $p$ 和正整数 $\alpha$ 使得 $m=p^\alpha$ 或 $m=2p^\alpha$．那么，模 $m$ 的原根不存在．

    ??? note "证明"
        对于 $m=2^\alpha$ 且 $\alpha\ge 3$，假设模 $m$ 的原根 $g$ 存在．由于 $g\perp m$，它一定是奇数．假设 $g=2k+1$ 且 $k\in\mathbf N$，那么，有
        
        $$
        \begin{aligned}
        g^{2^{\alpha-2}}
        &=(2k+1)^{2^{\alpha-2}} \\
        &\equiv 1 + \binom{2^{\alpha-2}}{1}(2k) + \binom{{2^{\alpha-2}}}{2}(2k)^2 \\
        &= 1 + 2^{\alpha-1}k + 2^{\alpha-1}(2^{\alpha-2}-1)k^2 \\
        &= 1 + 2^{\alpha-1}(k + (2^{\alpha-2}-1)k^2) \\
        &\equiv 1 \pmod{2^{\alpha}}.
        \end{aligned}
        $$
        
        倒数第二行中，因为 $k$ 与 $(2^{\alpha-2}-1)k^2$ 奇偶性相同，所以它们的和是偶数．由阶的定义可知，$\delta_{2^{\alpha}}(g)\le 2^{\alpha-2}< \varphi(2^{\alpha}) = 2^{\alpha-1}$．这与假设中 $g$ 是原根矛盾．由反证法，这样的原根并不存在．
        
        假设 $m$ 满足所述条件，且不是 $2$ 的幂，那么，一定可以存在 $2 < m_1 < m_2$ 且 $m_1\perp m_2$ 使得 $m=m_1m_2$ 成立．假设模 $m$ 的原根 $g$ 存在．因为 $g\perp m$，所以对于 $i=1,2$，都有 $g\perp m_i$．由欧拉定理可知，
...
这也常用作 Carmichael 函数的等价定义．反复应用性质 5 可知，一定存在某个元素 $a\perp m$ 使得 $\delta_m(a)=\lambda(m)$．因此，上式也可以写作
...
??? note "证明"
    设 $a_1$ 和 $a_2$ 分别为模 $m_1$ 和模 $m_2$ 的 $\lambda$‑原根．令 $m=m_1m_2$，由 [中国剩余定理](./crt.md) 可知，存在 $a\perp m$ 使得 $a\equiv a_i\pmod{m_i}$ 对于 $i=1,2$ 都成立．由于 $a^{\lambda(m)}\equiv 1\pmod m$，所以对于 $i=1,2$，都有 $a_i^{\lambda(m)} \equiv 1\pmod m$，进而由性质 2 和 $a_i$ 的选取可知，$\lambda(m_i)=\delta_{m_i}(a_i)\mid \lambda(m)$．这就说明 $[\lambda(m_1),\lambda(m_2)]\mid\lambda(m)$．
...
???+ note "引理"
    对于 $m=2^\alpha$ 且 $\alpha\in\mathbf N_+$，有 $\lambda(2)=1$，$\lambda(4)=2$，且对于 $\alpha\ge 3$ 都有 $\lambda(m)=2^{\alpha-2}$．

??? note "证明"
    对于 $m=2,4$ 的情形，单独讨论即可．对于 $m=2^\alpha$ 且 $\alpha\ge 3$ 的情形，首先重复前文定理 4 的证明的第一部分，就得到 $\lambda(m)\le 2^{\alpha-2}$．进而，只需要证明存在 $2^{\alpha-2}$ 阶元素即可．为此，有
    
    $$
    5^{2^{\alpha-3}} = (1 + 2^2)^{2^{\alpha-3}} = 1 + 2^2\times 2^{\alpha-3} = 1 + 2^{\alpha-1} \not\equiv 1 \pmod{2^\alpha}.
    $$
    
    因此，$\delta_m(5)>2^{\alpha-3}$，它只能是 $2^{\alpha-2}$ 阶元素．这就说明，$\lambda(m)=2^{\alpha-2}$．

然后，处理奇素数幂的情形．

???+ note "引理"
    对于 $m=p^\alpha$，其中，$p$ 是奇素数且 $\alpha\in\mathbf N_+$，有 $\lambda(m)=p^\alpha(p-1)$．

??? note "证明"
    首先证明命题对于 $\alpha=1$，即 $m=p$ 是奇素数的情形成立．为此，由 Carmichael 函数的定义可知，与 $p$ 互素的所有整数 $a$ 都是同余方程 $x^{\lambda(p)}\equiv 1\pmod{p}$ 的解．在模 $p$ 的意义下，该方程共有 $p-1$ 个互不相同的解．根据 [Lagrange 定理](./congruence-equation.md#定理-3lagrange-定理) 可知，$p-1\le\lambda(p)$．同时，欧拉定理要求，$\lambda(p)\mid\varphi(p)=p-1$．因此，$\lambda(p)=p-1$．
    
    对于 $m=p^\alpha$ 且 $\alpha> 1$ 的情形，可以从证明 $1+p$ 是 $p^{\alpha-1}$ 阶元开始．为此，有
    
    $$
    (1+p)^{p^{\alpha-1}} \equiv 1,\quad (1+p)^{p^{\alpha-2}} \equiv 1 + p^{\alpha-1} \not\equiv 1 \pmod{p^\alpha}.
    $$
    
    所以，$\delta_m(1+p)=p^{\alpha-1}$．另外，设模 $p$ 的原根为 $g$，那么，由于 $g^{\delta_m(g)}\equiv 1 \pmod{p}$，所以，由阶的性质 2 可知，$p-1\mid\delta_m(p)$．由 Carmichael 函数的定义和欧拉定理可知
    
    $$
    p^{\alpha-1}(p-1) = [\delta_m(p),p^{\alpha-1}]\mid\lambda(m) \mid \varphi(m) = p^{\alpha-1}(p-1).
    $$
    
    因此，$\lambda(m)=p^\alpha(p-1)$．
...
???+ note "定理"
    ...
    \varphi(m), & \text{if }m=1,2,4,p^\alpha\text{ for odd prime }p,\\
    \frac{1}{2}\varphi(m), &\text{if }m=2^\alpha,\alpha\ge 3,\\
    \prod_p\lambda(p^{e_p}), &\text{if }m = \prod_pp^{e_p}.
    \end{cases}
    $$
...
???+ note "推论"
    模 $m$ 的原根存在，当且仅当 $m=1,2,4,p^\alpha,2p^\alpha$，其中，$p$ 是奇素数且 $\alpha$ 是正整数．

由于本节对于递推公式的证明并没有用到原根存在定理，因此，这就构成了对该定理的又一个证明．

### Carmichael 数

利用 Carmichael 函数，可以讨论 Carmichael 数（卡迈克尔数，OEIS:[A002997](https://oeis.org/A002997)）的性质与分布．这是一类 [Fermat 素性测试](./prime.md#fermat-素性测试) 一定无法正确排除的合数．
...
??? note "证明"
    前两条性质可以直接从 Korselt 判别法及其证明中得到，只需要再证明对于互异素数 $p_1,p_2$ 的乘积 $n=p_1p_2$ 一定不是 Carmichael 数．假设 $n=p_1p_2$ 是 Carmichael 数．由 Korselt 判别法可知，$(p_i-1)\mid n-1$．但是，
...
利用解析数论还可以得到 Carmichael 数的分布的一些性质．设 $C(n)$ 为小于等于 $n$ 的 Carmichael 数个数．Alford, Granville, and Pomerance 在 1994 年证明[^alford1994infinitely]，对于充分大的 $n$，有 $C(n)>n^{2/7}$．由此，Carmichael 数有无限多个．在这之前，Erdős 在 1956 年已经证明[^erdos1956pseudoprimes]，$C(n) < n\exp\left(-c\dfrac{\ln n\ln\ln\ln n}{\ln\ln n}\right)$，其中 $c$ 为常数．因此，Carmichael 数的分布（相对于素数来说）十分稀疏．实际上，有[^pinchcarmichael] $C(10^9)=646$，$C(10^{18})=1~401~644$．
...
-   [Carmichael function - Wikipedia](https://en.wikipedia.org/wiki/Carmichael_function)
-   [Carmichael Number -- from Wolfram MathWorld](https://mathworld.wolfram.com/CarmichaelNumber.html)
