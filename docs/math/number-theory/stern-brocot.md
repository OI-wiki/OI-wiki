...
为左右子结点，就可以构造出整个 Stern–Brocot 树。Stern–Brocot 树的每个结点记录的三元组中，实际存储的分数是位于三元组中间的分数 $\dfrac{p}{q}$，而左右两个分数 $\dfrac{a}{b}$ 和 $\dfrac{c}{d}$ 是更早就出现过的分数。而且，考虑前一种构造就会发现，分数 $\dfrac{p}{q}$ 正是通过插入 $\dfrac{a}{b}$ 和 $\dfrac{c}{d}$ 的中位分数得到的。

#### 矩阵表示与 Stern–Brocot 数系

三元组构造意味着每个 Stern–Brocot 树上的结点都对应着一个矩阵
...
Stern–Brocot 树的根结点是单位矩阵 $I$，而向左子结点移动和向右子结点移动则分别对应将当前结点矩阵右乘以矩阵
...
每个结点实际对应的分数就是 $f(S)=\dfrac{a+c}{b+d}$。每个结点的矩阵 $S$ 都可以写成一系列矩阵 $L$ 和 $R$ 的乘积，这也可以理解为是由 $L$ 和 $R$ 构成的字符串，表示了从根结点到达它的路径。将所有正的有理数唯一地表示为这样的字符串，这可以看作得到了正有理数的一种表示，故而也称作 **Stern–Brocot 数系**（Stern–Brocot number system）。
...
根结点处是单位矩阵，这显然成立。向下移动时，乘以的矩阵 $L$ 和 $R$ 的行列式都是 $1$，根据 [行列式](../linear-algebra/determinant.md) 的性质可知，在下一层同样成立
...
因为 Stern–Brocot 是二叉搜索树，只需要通过比较当前分数和要寻找的分数的大小关系，就可以确定从根结点到给定分数的路径。将路径上向左子结点移动和向右子结点移动分别记作 $L$ 和 $R$，则每个路径就对应一个由 $L$ 和 $R$ 构成的字符串，这就是上文提到的有理数在 Stern–Brocot 数系中的表示。求得到达某个有理数的路径的过程就相当于求得该有理数在 Stern–Brocot 数系中的表示。
...
如果要查找的分数 $\dfrac{p}{q}$ 落入 $\dfrac{a}{b}$ 和 $\dfrac{c}{d}$ 之间，那么连续 $t$ 次向右移动时，右侧边界保持不动，而左侧结点移动到 $\dfrac{a+tc}{b+td}$ 处；反过来，连续 $t$ 次向左移动时，左侧边界保持不动，而右侧结点移动到 $\dfrac{ta+c}{tb+d}$ 处。因此，可以直接通过 $\dfrac{a+tc}{b+td}<\dfrac{p}{q}$ 或 $\dfrac{p}{q}<\dfrac{ta+c}{tb+d}$ 来确定向右和向左移动的次数。此处取严格不等号，是因为算法移动的是左右端点，而要寻找的分数是作为最后得到的端点的中位分数出现的。
...
因此，在目标分数的末尾为一的 [连分数表示](./continued-fraction.md#简单连分数) 中，不计最后的一，前面的项就编码了 Stern–Brocot 树上自根结点到当前结点的路径。其中，偶数项（下标自 $0$ 开始）是向右子结点的边，奇数项是向左子结点的边。
...
利用连分数表示，可以简单地表达出某个结点的父结点和子结点。对于结点 $[t_0,t_1,\cdots,t_n,1]$，它的父结点就是沿最后的移动方向少移动一步的结点：在 $t_k>1$ 时，父结点是 $[t_0,t_1,\cdots,t_n - 1,1]$；否则，父结点是 $[t_0,t_1,\cdots,t_{n-1},1]$。它的两个子结点则分别是 $[t_0,t_1,\cdots,t_n+1,1]$ 和 $[t_0,t_1,\cdots,t_n,1,1]$；两个结点哪个是左子结点，哪个是右子结点，需要根据 $n$ 的奇偶性判断。
...
树的根结点为 $\dfrac{1}{1}$。然后，对于分数 $\dfrac{p}{q}$ 所在的结点，其左右子结点分别为 $\dfrac{p}{p+q}$ 和 $\dfrac{p+q}{q}$。与 Stern–Brocot 树类似，它的每个分数都是最简分数，且包括全体正的最简分数各恰好一次。

### 与连分数的关系

与 Stern–Brocot 树不同，Calkin–Wilf 树不是二叉搜索树，因此不能用于二分查找有理数。

在 Calkin–Wilf 树中，当 $p>q$ 时，分数 $\dfrac{p}{q}$ 的父结点为 $\dfrac{p-q}{q}$；当 $p<q$ 时，为 $\dfrac{p}{q-p}$。对于第一种情形，自 $\dfrac{p}{q}$ 出发，它是父结点的右子结点，可以一直通过父结点的右边向上移动，直到分子不再大于分母为止，此时结点存储的分数是 $\dfrac{p\bmod q}{q}$，本组移动的次数是 $\left\lfloor\dfrac{p}{q}\right\rfloor$；对于第二种情形，它是父结点的左子结点，可以一直通过父结点的左边向上移动，直到分母不再大于分子为止，此时结点存储的分数是 $\dfrac{p}{q\bmod p}$，本组移动的次数是 $\left\lfloor\dfrac{q}{p}\right\rfloor$。

利用连分数的语言，设当前的结点存储的是某个连分数的余项 $s_k$，则沿父结点的右边向上移动 $\lfloor s_k\rfloor$ 次，到达分数 $\dfrac{1}{s_{k+1}}$，随后沿父结点的左边移动 $\lfloor s_{k+1}\rfloor$ 次，到达分数 $s_{k+2}$。因此，从结点 $s_0=\dfrac{p}{q}$ 开始向上到达根结点 $\dfrac{1}{1}$ 的路径由连分数 $[t_0,t_1,\cdots,t_n,1]$ 编码：除去最后的 $1$，偶数项（下标自 $0$ 开始）表示沿父结点的右边移动的次数，奇数项表示沿父结点的左边移动的次数。

对于分数 $\dfrac{p}{q}=[t_0,t_1,\cdots,t_n,1]$ 所在的结点，它的父结点有如下表示：

1.  当 $t_0>0$ 时，它的父结点为 $\dfrac{p-q}{q}=[t_0 - 1, t_1, \cdots, t_n,1]$；
2.  当 $t_0=0$ 且 $t_1>1$ 时，它的父结点为 $\dfrac{p}{q-p} = [0, t_1 - 1, t_2, \cdots, t_n,1]$；
3.  当 $t_0=0$ 且 $t_1=1$ 时，它的父结点为 $\dfrac{p}{q-p} = [t_2, t_3, \cdots, t_n,1]$。

反过来，它的子结点则分别是 $\dfrac{p+q}{q}=[t_0+1,t_1,\cdots,t_n,1]$ 和 $\dfrac{p}{p+q}=[0,1,t_0,t_1,\cdots,t_n,1]$。对于第二个子结点的连分数表示，在 $t_0=0$ 时，应当理解为 $[0,1+t_1,\cdots,t_n,1]$。

### 与 Stern–Brocot 树的关系

同样和连分数建立起联系，Stern–Brocot 树中路径上结点呈现的是渐近分数的递归关系，而 Calkin–Wilf 树中路径上结点呈现的是余项的递归关系。同一个分数的连分数表示是一定的，因此它在 Calkin–Wilf 树上到达根结点的路径的编码和 Stern–Brocot 树的根结点到它的路径的编码是完全一样的。但是，由于路径的方向相反，所以虽然 Stern–Brocot 树和 Calkin–Wilf 树同一层存储的分数是一样的，但是位置并不相同。

如果对两个树分别做 [广度优先搜索](../../graph/bfs.md) 并依次对结点编号，根结点编号为 $1$，那么编号为 $v$ 的结点的左右子结点分别是 $2v$ 和 $2v+1$。从编号的二进制表示来看，除去起始的 $1$，从高位至低位每个 $1$ 均表示向右子结点移动，每个 $0$ 均表示向左子结点移动。Calkin–Wilf 树上，连分数 $[t_0,t_1,\cdots,t_n,1]$ 表示的有理数所在结点的编号是

$$
1\cdots \underbrace{0\cdots 0}_{t_3}\underbrace{1\cdots 1}_{t_2}\underbrace{0\cdots 0}_{t_1}\underbrace{1\cdots 1}_{t_0}.
$$

相应地，Stern–Brocot 树上，连分数 $[t_0,t_1,\cdots,t_n,1]$ 表示的有理数所在结点的编号是
...
正因如此，Stern–Brocot 树上的结点有时会按照对应的 Calkin–Wilf 树上的结点编号，由此得到的编号如下图所示：

![pic](./images/stern-brocot-index.svg)

这个编号可以递归地构造：根结点编号为 $1$，每次移动到左子结点时，就将编号首位的 $1$ 替换成 $10$，而移动到右子结点时，就将编号首位的 $1$ 替换成 $11$。对该编号自右向左解读就可以得到自根结点到该结点的路径。
