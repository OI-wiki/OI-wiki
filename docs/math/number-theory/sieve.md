...
我们注意到筛选时只需要 `bool` 类型的数组。`bool` 数组的一个元素占用 $1$ 字节（即 $8$ 比特），但是存储一个布尔值只需要 $1$ 个比特就足够了。

我们可以使用 [位运算](https://oi-wiki.org/math/bit) 的相关知识，将每个布尔值压到一个比特位中，这样我们仅需使用 $n$ 比特（即 $\dfrac n 8$ 字节）而非 $n$ 字节，可以显著减少内存占用。
