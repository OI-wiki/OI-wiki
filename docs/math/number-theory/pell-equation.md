...
$$
p2k−Dq2k=(pkqk−1−pk−1qk)Qk+1=(−1)k−1Qk+1=(−1)k+1Qk+1
$$
...
定理：如果 $\sqrt{D}$ 连分数循环节长度为奇数，并且 $D$ 存在唯一的平方和表示 $D=r^2+s^2$，则两个方程 $x^2-2py^2=r$、$x^2-2py^2=s$ 至少有一个有解。

如果了解高斯整数的知识，只有当一个数所有的 $4k+3$ 型素因子均成对，这个数才能进行平方和表示，此时平方和表示的个数就是这个数含有的 $4k+1$ 型素因子数的个数。

证明：

因为循环节是奇数，连分数展开中对称部分最中间一个数连续出现两次。根据伽罗瓦连分数定理，对称部分最中间位置下标 $v-1$ 的余项 $\left\lfloor\frac{P_{v-1}+\sqrt{D}}{Q_{v-1}}\right\rfloor$，和取倒数后下标 $v$ 对应的这一项 $\frac{P_v+\sqrt{D}}{Q_v}$ 互为负共轭。

因此，这个情形下有 $Q_v=Q_{v-1}$，即两个渐进分数的分母相同。并且有
...
由于连分数的结论有 $x^2-Dy^2=(-1)^vQ_v$ 的解为相应渐进分数，因此上述两个方程必然存在一个有解为下标 $v$ 或 $v-1$ 的渐进分数。证毕。

如果直接对方程 $x^2-Dy^2=-1$ 两端取模 $8$，能够知道 $D\equiv 1 or 2 \pmod 8$。然而，这只是一个充分条件，而非必要条件。通过取模的方式确定什么时候方程有解，基本做不到。例如，可以给出一个无解的充分条件：

定理：如果 $D$ 存在唯一的平方和表示 $D=r^2+s^2$，并且 $r, s\equiv\pm 3 \pmod 8$，则方程 $x^2-2py^2=-1$ 无解。

例如对于 $34$ 有 $34=3^2+5^2$，于是 $x^2-34y^2=-1$ 无解。

证明：

如果方程 $x^2-2py^2=-1$ 有解，则两个方程 $x^2-2py^2=r$、$x^2-2py^2=s$ 至少有一个有解。模 $8$ 就得到了矛盾。证毕。
