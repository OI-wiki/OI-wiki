...
p 进数（p-adic）由 Hensel 于1900年前后引进，其想法是把幂级数展开的方法引进到数论中来。
...
有理数集的基数是阿列夫零，其中有很多的洞，填补这些洞的方法有若干种。通常的填补这些洞的方法创造出实数，实数集的基数是阿列夫一。

一个例子是，令有理数列2、2.7、2.71、2.718、2.7182、2.71828……收敛，于是创造了一个新数 $e$ 当做这个数列的极限点。

要想理解这件事，需要明确什么叫逼近。按通常理解，如果两个数数位上的数码向右数时有很长一段是一致的，那么这两个数就是逼近的。

用类似的技巧来构造 p 进数，不同之处在于选择了一个新的逼近定义。如果两个数数位上的数码向左数时有很长一段是相同的，那么这两个数是逼近的。例如 10 进数0.002、0.072、0.172、8.172、28.172、828.172、1828.172……变得越来越靠近某个数。
...
上面例子的……6666666.0事实上表示-1，在 p 进数里没有负号。
...
因此可以找一个计数方法巧妙记录有理数，例如将 $\frac{1}{3}$ 记录成……10101010101等等，在数论方面的性质仍旧保留了。

## 完备化方法

根据上述讨论，可以给出两种等价的定义。

定义1：p 进数域定义为有理数域关于 p 进绝对值作完备化得到的数域。

p 进数域的一个元素是柯西列的等价类。

柯西列是指有理数列 $x_n$ 满足：对任意的 $\varepsilon>0$ ，存在正整数 $N$，使得任意的 $m,n\geq N$ 有 $|x_m-x_n|_p<\varepsilon$。

称两个柯西列 $x_n$、$y_n$ 是等价的，当且仅当 $\lim_{n\to \infty} |x_{n}-y_{n}|_{p}\to 0$。
...
实数对于开方和对数运算不是封闭的，从而向实数中添加虚数单位 $i$ 构成复数，复数对于开方和对数运算是封闭的。向实数中添加虚数单位 $i$、$j$、$k$ 构成四元数，四元数牺牲了乘法交换律，仍旧构成一种新的代数系统。
...
p 进数的指对数计算也是构造这样的同构关系。用 $e^x$ 和 $\ln x$ 引入，计算时采用展开成无穷级数的方法。
...
上面的定义依赖展开式，即只与多项式本身有关，不依赖于x的取值。

只用多项式理论即可证得，这种定义满足乘加转换。对于任意的a和b，满足：
...
- 对于定义域 $p^m\mathbb{Z}_{p}$，由于 $v_p\left(\frac{x^n}{n!}\right)\geq m$，所以 $e^x$的值域在 $1+p^m\mathbb{Z}_{p}$。
- 对于定义域 $1+p^m\mathbb{Z}_{p}$，由于 $v_p\left({(-1)}^{n-1}\frac{{(t-1)}^n}{n}\right)\geq m$，所以 $\ln t$ 的值域在 $p^m\mathbb{Z}_{p}$。

对这个范围内的 $x$ 和 $t$ 则可以只用多项式理论证得 $\ln(e^x)=x$ 和  $e^(\ln t)=t$，证毕。
...
这意味着什么呢？所有奇数均在对数收敛域内。然而，全体被 4 除余 3 的数算出来的对数不满足双射条件，算出的对数值再代入指数无法得到原数，而是它的相反数——被 4 除余 1 的数。这是由于如果代入计算 $\ln (-1)$，会算出它等于 $0$。
...
这里 p 进数理论中的指对数，底数、指数（幂次）和幂（真数）均在p进数中。它建立了一个双射：模p（或4）余1的乘法，与模p（或4）余0的加法之间的双射。

### 2 进数的示例

可以计算 2 进数下的 $e^4$ 和 $\ln 5$，进而直观了解指对数：任意指数都是 $e^4$ 的指数，利用换底公式可以转换成以 5 为底的对数，从而与以 5 为底的对数构成对比。
...
- 对于p 为 $2$ 的情形，按照指对数的双射性，有乘法群 $1+4 mathbb{Z}_p$ 同构于 $4 mathbb{Z}_p$，定理显然。证毕。
