...
由于逆元的求解，就相当于求解线性同余方程。因此，可以仿照解线性同余方程的方法，使用 [扩展欧几里得算法](./gcd.md#扩展欧几里得算法) 在 $O(\log\min\{a,m\})$ 时间内求解逆元。同时，由于逆元对应的线性方程比较特殊，可以适当地简化相应的步骤。
...
这一方法主要适用于模数是素数 $p$ 的情形。此时，由 [费马小定理](./fermat.md#费马小定理) 可知对于任意 $\gcd(a,p)=1$，都有
...
当然，理论上，这一方法可以利用 [欧拉定理](./fermat.md#欧拉定理) 推广到一般的模数 $m$ 的情形，即通过计算 $a^{\varphi(m)-1}$ 来计算逆元。但是，单次求解 [欧拉函数](./euler-totient.md) $\varphi(m)$ 并不容易，因此该算法在一般情况下效率不高。

## 多个逆元的求法

有些场景下，需要快速处理出多个整数 $a_1,a_2,\cdots,a_n$ 在模 $m$ 意义下的逆元。此时，逐个求解逆元，总共需要 $O(n\log m)$ 的时间复杂度。实际上，如果将它们统一处理，就可以在 $O(n+\log m)$ 的时间下求出所有整数的逆元。
...
如果要处理前 $n$ 个正整数在素数模 $p$ 下的逆元，还可以通过本节将要讨论的线性递推关系在 $O(n)$ 时间内求解。这一方法常用于前 $n$ 个正整数的阶乘的倒数的预处理。

对于 $1\le i < p$ 的正整数 $i$，考察带余除法：
...
这就是用于线性递推求逆元的公式。由于 $p\bmod i < i$，这一公式将求解 $i^{-1}\bmod p$ 的问题转化为规模更小的问题 $(p\bmod i)^{-1}\bmod p$。因此，从 $1^{-1}\bmod p=1$ 开始，逐个应用该公式，可以在 $O(n)$ 时间内获得前 $n$ 个整数的逆元。
...
另外，得到该递推公式后，一种自然的想法是直接递归求解任意一个数 $a$ 的逆元。每次递归时，都将它转化为更小的余数 $p\bmod a$ 的逆元，直到余数变为 $1$ 时停止。目前尚不清楚这样做的复杂度[^linear-recursion]，因此，推荐使用前文所述的常规方法求解。
