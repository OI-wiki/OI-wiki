...
对于 $F_{k}(n)$ 的计算，其第一种方法的时间复杂度被证明为 $O\left(n^{1 - \epsilon}\right)$（见朱震霆集训队论文《一些特殊的数论函数求和问题》2.3）；  
对于第二种方法，其本质即为洲阁筛的第二部分，在任之洲论文《积性函数求和的几种方法》中也有提及（6.5.4），其时间复杂度被证明为 $O\left(\frac{n^{\frac{3}{4}}}{\log{n}}\right)$．
