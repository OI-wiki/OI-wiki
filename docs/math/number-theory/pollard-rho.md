...
我们通过 $f(x)=(x^2+c)\bmod n$ 来生成一个序列 $\{x_i\}$：随机取一个 $x_1$，令 $x_2=f(x_1),x_3=f(x_2),\dots,x_i=f(x_{i-1})$。其中 $c=rand()$，是一个随机的常数。
...
同理，任何满足 $\forall x \equiv y \pmod p, f(x) \equiv f(y) \pmod p$ 的函数 $f(x)$（例如多项式函数）都可以用在此处，且我们希望它对大部分初始值尽可能快地进入循环、循环节尽可能短（但对于一个合数与其任一质因子，循环节长度不能相同，否则 Pollard Rho 算法将无法分离出这个因子）。
