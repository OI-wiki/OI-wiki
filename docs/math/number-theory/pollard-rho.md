...
当 $N\geqslant10^{18}$ 时，这个算法的运行时间我们是无法接受的，希望有更优秀的算法。一种想法是通过随机的方法，猜测一个数是不是 $N$ 的因数，如果运气好可以在 $O(1)$ 的时间复杂度下求解答案，但是对于 $N\geqslant10^{18}$ 的数据，成功猜测的概率是 $\frac{1}{10^{18}}$, 期望猜测的次数是 $10^{18}$。如果是在 $[2,\sqrt N]$ 里进行猜测，成功率会大一些。我们希望有方法来优化猜测。
...
至少有两个人生日相同的概率为 $P(\overline A)=1-P(A)$。根据题意可知 $P(\overline A)\geqslant\frac{1}{2}$, 那么就有

$$
P(A)=\prod_{i=0}^{k-1}\frac{n-i}{n} \leqslant \frac{1}{2}
$$

由不等式 $1+x\leqslant \mathrm{e}^x$ 可得

$$
P(A) \leqslant \prod_{i=1}^{k-1}\exp\left({-\frac{i}{n}}\right)=\exp \left({-\frac{k(k-1)}{2n}}\right)
$$

因此

$$
\exp\left({-\dfrac{k(k-1)}{2n}}\right) \leqslant \frac{1}{2}\implies P(A) \leqslant \frac{1}{2}
$$

将 $n=365$ 代入，解得 $k\geqslant 23$。所以一个房间中至少 $23$ 人，使其中两个人生日相同的概率达到 $50\%$, 但这个数学事实十分反直觉，故称之为一个悖论。
...
$$
E(X) = \sum_{x=1}^{n+1}P(X\geqslant x+1) = \sum_{x=0}^n\frac{n!}{(n-x)!n^x} = \sqrt{\frac{\pi n}{2}}-\frac13+o(1).
$$
