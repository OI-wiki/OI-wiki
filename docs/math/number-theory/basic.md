...
-   设 $b\ne0$，那么 $a\mid b\implies|a|\leqslant|b|$。
...
???+ note "余数"
    设 $a,b$ 为两个给定的整数，$a\ne0$。设 $d$ 是一个给定的整数。那么，一定存在唯一的一对整数 $q$ 和 $r$，满足 $b=qa+r,d\leqslant r<|a|+d$。

无论整数 $d$ 取何值，$r$ 统称为余数。$a\mid b$ 等价于 $a\mid r$。

一般情况下，$d$ 取 $0$，此时等式 $b=qa+r,0\leqslant r<|a|$ 称为带余数除法（带余除法）。这里的余数 $r$ 称为最小非负余数。

余数往往还有两种常见取法：

-   绝对最小余数：$d$ 取 $a$ 的绝对值的一半的相反数。即 $b=qa+r,-\dfrac{|a|}{2}\leqslant r<|a|-\dfrac{|a|}{2}$。
-   最小正余数：$d$ 取 $1$。即 $b=qa+r,1\leqslant r<|a|+1$。
...
-   若 $(a,b)=1$，则 $(a,bc)=(a,c)$；
-   若 $(a_i,b_j)=1,~\forall 1\leqslant i\leqslant n,1\leqslant j\leqslant m$，则 $\left(\prod_i a_i,\prod_j b_j\right)=1$。特别地，若 $(a,b)=1$，则 $(a^n,b^m)=1$；
-   对整数 $a_1,\dots,a_n$，若 $\exists v\in \mathbf{Z},~\prod_i a_i=v^m$，且 $(a_i,a_j)=1,~\forall i\ne j$，则 $\forall 1\leqslant i\leqslant n,~\sqrt[m]{a_i}\in\mathbf{Z}$。
...
-   若 $a\mid b$，则 $[a,b]=|b|$；
-   $[a_1,\dots,a_n]=[[a_1,a_2],a_3,\dots,a_n]$。进而 $\forall 1<k<n-1,~[a_1,\dots,a_n]=[[a_1,\dots,a_k],[a_{k+1},\dots,a_n]]$；
-   若 $a_i\mid m,~\forall 1\leqslant i\leqslant n$，则 $[a_1,\dots,a_n]\mid m$；
...
-   对于合数 $a$，一定存在素数 $p\leqslant\sqrt{a}$ 使得 $p\mid a$。
-   素数有无穷多个。
...
???+ note "算术基本定理（唯一分解定理）"
    ...
    其中 $p_j(1\leqslant j\leqslant s)$ 是素数。并且在不计次序的意义下，该表示唯一。
...
-   设 $f(x)=\sum_{i=0}^n a_ix^i$ 和 $g(x)=\sum_{i=0}^n b_ix^i$ 是两个整系数多项式，$m\in\mathbf{N}^*$，且 $a_i\equiv b_i\pmod m,~0\leqslant i\leqslant n$，则对任意整数 $x$ 均有 $f(x)\equiv g(x)\pmod m$。进而若 $s\equiv t\pmod m$，则 $f(s)\equiv g(t)\pmod m$。
...
$$
\mathbf{Z}_m:=\{r\bmod m:0\leqslant r<m\}
$$
...
$$
\mathbf{Z}_m^*:=\{r\bmod m:0\leqslant r<m,(r,m)=1\}
$$
...
???+ note "既约剩余系"
    对 $t=\varphi(m)$ 个整数 $a_1,a_2,\dots,a_t$，若 $(a_i,m)=1,~\forall 1\leqslant i\leqslant t$，且对任意满足 $(x,m)=1$ 的数 $x$，有且仅有一个数 $a_i$ 使得 $x$ 与 $a_i$ 模 $m$ 同余，则称这 $t$ 个整数 $a_1,a_2,\dots,a_t$ 为模 $m$ 的 **既约剩余系**、**缩剩余系** 或 **简化剩余系**。
...
-   若 $m=m_1m_2,~1\leqslant m_1,m_2$，令 $Z_{m_1},Z_{m_2}$ 分别为模 $m_1,m_2$ 的 **完全** 剩余系，则对任意与 $m_1$ 互质的 $a$ 均有：

    $$
    Z_m=aZ_{m_1}+m_1Z_{m_2}.
    $$

    为模 $m$ 的 **完全** 剩余系。进而，若 $m=\prod_{i=1}^k m_i,~1\leqslant m_1,m_2,\dots,m_k$，令 $Z_{m_1},\dots,Z_{m_k}$ 分别为模 $m_1,\dots,m_k$ 的 **完全** 剩余系，则：
...
-   若 $m=m_1m_2,~1\leqslant m_1,m_2,(m_1,m_2)=1$，令 $Z_{m_1}^*,Z_{m_2}^*$ 分别为模 $m_1,m_2$ 的 **既约** 剩余系，则：
