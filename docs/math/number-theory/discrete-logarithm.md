...
前文的 BSGS 算法时间复杂度为单次 $O(\sqrt m)$，在询问量级比较大的时候效率较低．若每次求解的模数是一个固定的质数 $P$，我们就有一个基于值域预处理的、更加快速的算法．

前文我们已经知道，$\operatorname{ind}_g(ab)\equiv\operatorname{ind}_g a+\operatorname{ind}_g b\pmod{P-1}$．由此，我们可以只对于所有质数利用 BSGS 求出离散对数；对于合数，则可以通过上述公式转化为两个已经求得的离散对数值来得到结果．此时复杂度仍然不优，我们考虑只预处理一部分的离散对数．具体来说，我们预处理到 $L = \lfloor\sqrt p\rfloor + 1$，注意此时 BSGS 块长 $B$ **不能直接取** 根号量级 $O(\sqrt{L})$，因为预处理（插入哈希表）部分是 $O(B)$，而查询一共需要 $O(\pi(L))$ 次，总时间为 $O\left(B+\dfrac{\pi(L)P}{B}\right)$，取 $B=O(\sqrt{P\pi(L)})$ 才是最优．由 [素数定理](./prime.md) $\pi(n)\sim\dfrac{n}{\log n}$，则总时间复杂度可以平衡为 $O\left(\dfrac{P^{3/4}}{\log^{1/2} P}\right)$．

接下来是如何求答案．假设当前要求的是 $\operatorname{ind}_g y$，若 $y\le L$ 则直接返回，否则设 $v=\left\lfloor\dfrac{P}{y}\right\rfloor$，$r=P\bmod y$，则 $P=vy+r$，因此 $y=\dfrac{P-r}{v}$，从而得出 
...
注意到此时 $P-1$ 的离散对数可以提前算好，$v\le L$，只需递归计算 $r$ 的离散对数，但时间复杂度并不优秀．

为此，考虑 $y$ 的另一种表达方式．由于 $P=vy+r=(v+1)y+r-y$，所以 $y=\dfrac{P-r+y}{v+1}$，同理即得

$$
\operatorname{ind}_g y\equiv \operatorname{ind}_g (y-r)-\operatorname{ind}_g (v+1) \pmod{P-1}.
$$

此时仍然有 $v+1 \le L$，故只需要递归计算 $y-r$ 的离散对数即可．

综合这两种计算方式，我们发现 $\min\{r,y-r\}\le \dfrac{y}{2}$，于是递归计算较小的一方可以做到 $O(\log P)$．

至此得到一个 $O\left(\dfrac{P^{3/4}}{\log^{1/2} P}\right)-O(\log P)$ 的较优秀算法．
