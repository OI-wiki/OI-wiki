...
前文已经说过，$D(n)$ 既是每块中 $\left\lfloor\dfrac{n}{i}\right\rfloor$ 的取值集合，也是每块的右端点集合．这意味着，如果需要递归地应用数论分块，也就是说，函数在 $n$ 处的取值依赖于它在 $m\in D(n)\setminus\{n\}$ 处的取值，那么，整个计算过程中涉及的取值集合和右端点集合都是集合 $D(n)$．一个这样的例子是 [杜教筛](./du.md)．
...
假设单次计算 $s(\cdot)$ 的时间复杂度为 $O(1)$，整个过程的时间复杂度为 $O(\sqrt{n})$．
