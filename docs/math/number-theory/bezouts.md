...
??? note "证明"
    ...
    反过来，需要说明存在 $x,y$ 使得等式成立．由于 $\gcd(a,b)=\gcd(-a,b)=\gcd(a,-b)$，所以不妨设 $a,b$ 都是非负数．如果 $a,b$ 之一是 $0$，不妨设 $b=0$，那么它们的最大公约数为 $d=a$，显然有 $(x,y)=(1,0)$ 使得等式成立．接下来，考虑 $a,b$ 均不为零的情形，此时 $a,b$ 都是正数．
...
??? note "解答"
    ...
    **解法 2**：从数组 $l_1, \cdots, l_n$ 选择若干个数，满足它们的最大公约数为 $1$，且代价和最小，由此可以想到 0-1 背包问题．
    
    设 $f_{i, j}$ 表示考虑前 $i$ 个数且最大公约数为 $j$ 的最小代价，则有转移方程：
    
    $$
    f_{i, j} = \min\left\{f_{i - 1, j},~\min_{\gcd(k, l_i) = j}\{f_{i - 1, k} + c_i\}\right\}.
    $$
    ...
    实际上，这里解法 1 建出的图便是解法 2 中动态规划的状态转移图，解法 2 相当于用动态规划求有向无环图的最短路，因此解法 1 和解法 2 是等价的．但解法 2 无需储存全图，同时 DP 的时间复杂度为 $O(n + m)$（其中 $m$ 为状态转移图的边数），相比 Dijkstra 算法更低，因此解法 2 在时间和空间上更优．
...
的形式，其中，$x^\circ$ 为一个特解，$x^{(k)}$ 为相应的齐次方程的 $(n-1)$ 个线性无关的解．

要求出通解的具体形式，可以通过将 $n$ 元方程转化为 $(n-1)$ 元方程来完成．记 $d_1 = \gcd(a_1,a_2)$，那么，根据裴蜀定理，$a_1x_1+a_2x_2$ 的全体恰为 $d_1$ 的所有倍数．因此，可以首先求解 $(n-1)$ 元一次不定方程：
...
??? note "证明一"
    ...
    取 $t$ 为 $x_2^\circ$ 对 $a_1$ 作带余除法得到的商，那么，余数 $x_2 = x_2^\circ-ta_1$ 位于 $0$ 和 $a_1-1$ 之间．考察此时得到的一组解 $(x_1,x_2)$．因为 $x_2$ 是它能够取到的最小非负整数值，所以 $k$ 可表示当且仅当 $x_1\ge 0$．
    ...
    **第二步**：证明 $C$ 不可表示．进而，$C$ 是最大的不可表示的整数，且 $k$ 和 $C-k$ 并非都可表示．
...
??? note "证明二"
    此处仅证明 $C=a_1a_2-a_1-a_2$ 是最大的不可表示的整数，其余部分的证明类似证明一．
    
    考虑模 $a_2$ 意义下，每个剩余类中最小的可表示的自然数．因为同一个剩余类中的不同自然数可以通过加减若干个 $a_2$ 互相转化，所以，在讨论最小可表示数时，只需要考虑加减 $a_1$ 的可能性就可以了．由于 $a_1$ 和 $a_2$ 互素，所以，每个剩余类中最小的可表示的自然数恰好就是 $a_1$ 的倍数
...
将方程 $a_1x_1 + a_2x_2 = k$ 看作是一条直线．那么，$k$ 可表示，当且仅当这条直线在第一象限（包括坐标轴）内通过至少一个整点．当 $k < a_1a_2$ 时，这条直线在第一象限至多只能通过一个整点．因此，对于 $0\le k < a_1a_2$，整数 $k$ 可以表示，当且仅当这条直线在第一象限恰好通过一个整点．

因此，对于 $k < a_1a_2$，小于等于 $k$ 且可以表示的自然数的数量，恰好等于第一象限内直线 $a_1x_1 + a_2x_2 = k$ 下的整点个数（包含边界上的点）．这一数量就等于

$$
\sum_{i=0}^{\lfloor k / a_1 \rfloor} \left(\left\lfloor\dfrac{k-ia_1}{a_2}\right\rfloor+1\right).
$$
