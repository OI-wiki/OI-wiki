...
由此得出了若选择的卡牌的数通过数次相加或相减得出的绝对值为 $1$，那么这些数的最大公因数一定为 $1$，此时可以考虑动态规划求解。
...
同解法 1，原题可转化为从数组 $l_1, \dots, l_n$ 选择若干个数，满足它们的最大公因数为 1，且代价和最小。由此可以想到 0-1 背包问题。
...
如同一般的 0-1 背包问题，可以用滚动数组优化，去掉第一维。而这里 300 个数可达的最大公因数 $j$ 是很稀疏的，因此还可以使用 Hash 表（`unordered_map`）代替数组，优化内存并进一步减少枚举量。
...
实际上，这里解法 1 建出的图便是解法 2 中动态规划的状态转移图，解法 2 相当于用动态规划求有向无环图的最短路，因此解法 1 和解法 2 是等价的。
