...
它们需要满足条件 $a < R$，故而这一长度至少为 $2\ell(a) - \ell(m)$。但是，需要取模时，一般都有 $\ell(m)\le\ell(a)$，因此，这一中间变量的长度可能大于输入长度 $\ell(a)$。例如，如果需要将 $64$ 位整数对 $32$ 位整数取模，实际上中间变量需要 $64 \times 2 - 32 = 96$ 位整数。
...
所以同余类的加法、减法就对应它们的 Montgomery 形式的加法、减法。而要计算同余类的乘法，则不能直接将两个 Montgomery 形式相乘。因为
...
-   将 $a$ 转换为它的 Montgomery 形式就是 $\operatorname{REDC}(a(R^2\bmod m))$。
