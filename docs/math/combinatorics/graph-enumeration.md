...
我们将 POJ 1737 的递推过程封装成连通化函数，它等价于对其 EGF 求 ln。
...
注意到这里的连通化过程其实等价于对其 EGF 求多项式 ln，同理我们也可以写出逆连通化函数，它等价于对其 EGF 求 exp。
...
$$g_n = \sum_{i=0}^{n} \binom{n}{i}2^{i(n-i)} $$[^2]
...
方法二和方法三均使用连通二分图 $b1_n$ 作为 $g_n$ 与 $b_n$ 之间的建立桥梁。
...
做法二与做法三本质相同，且一般情况下做法三可以得到更优的时间复杂度。
