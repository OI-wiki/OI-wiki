...
方法二是使用连通二分图为这两个数列之间建立桥梁。

设 $G(x)$ 为 $g_n$ 的 EGF，$B1(x)$ 为连通二分图的 EGF，$B(x)$ 为二分图的 EGF，对于每个连通二分图，我们有两种不同的染色方法，对应到两组不同的连通 2 染色图，应用前两问中的方法，我们有：
...
我们可以使用上面的 $O(n^2)$ 递推分别替代此处的多项式 exp 和 ln 过程，直接得到 $b_n$ 通过本题。
...
对等式两边进行求导并比较两边系数，以得到易于编码的递推公式，也可通过此题。
