...
移项得到 $c_n$ 序列的 $\operatorname{O}(n^2)$ 递推公式，可以通过此题。
...
上述的递推式可以看作一种自卷积形式，因而可以使用分治 FFT 进行计算，复杂度 $\operatorname{O}(n\log^2n)$。
...
连通图问题在之前的例题中已被解决，考虑欧拉图。注意到上述对连通图计数的几种方法，均可以对满足任意性质的有标号连通图可以进行推广。例如我们可以将连通图递推公式中的 $g_n$，从任意图改成满足顶点度数均为偶数的图，那么得到的 $c_n$ 即为欧拉图。
...
void ln(Int C[], Int G[]){
    REP_1(i, n){
        C[i] = G[i];
        REP_1(j, i-1) C[i] -= binom[i-1][j-1]*C[j]*G[i-j];
    }
}
```
...
void exp(Int G[], Int C[]){
    REP_1(i, n){
        G[i] = C[i];
        REP_1(j, i-1) G[i] += binom[i-1][j-1]*C[j]*G[i-j];
    }
}
```
...
[^2]: [粉兔的 blog 告诉我们](https://www.luogu.com.cn/blog/PinkRabbit/solution-sp4420)，这个序列也可以使用 [Chirp Z-Transform](../../poly/czt/) 优化。
...
不难得到 $b_n$ 的递推关系，复杂度 $\operatorname{O}(n^3)$，进一步使用冗斥原理，可以优化到 $\operatorname{O}(n^2)$ 通过本题。
...
int main(){
    ...
    n = 1000; REP(i, n+1){binom[i][0] = 1; REP(j, i) binom[i][j+1] = binom[i-1][j] + binom[i-1][j+1];}
...
本题中置换群 $G$ 为顶点的 $n$ 阶对称群生成的边集置换群，但暴力做法的枚举量为 $\operatorname{O}(n!)$，无法通过此题。
...
考虑 $c(p)$，p 所影响的点集的循环即为 |p|，但题目考察的时边染色，考察点置换所生成的边置换，

如果一条边关联的顶点处在同一个循环内，假设该循环大小为 $p_i$，那么边所生成的循环数恰好为 $\left\lfloor \frac{p_i}{2} \right\rfloor$。

如果一条边关联的顶点处在两个不同的循环中，假设分别时 $p_i$, $p_j$，那么边所生成的循环数恰好为 $\frac{p_i p_j}{\lcm(p_i,p_j)} = \gcd(p_i, p_j) $
