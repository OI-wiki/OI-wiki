...
设路径 **第一次** 走到对角线 $y=x$ 的点是 $(k,k)~(k \in [1,n])$。

从 $(0,0)$ 到 $(k,k)$ 的除起点和终点外，中间的点 **不经过对角线（不能碰到）** 的路径数。

那么第一步一定向右，从 $(0,0)$ 到 $(1,0)$，最后一步一定向上，从 $(k,k-1)$ 到 $(k,k)$。
...
乘法原理，方案数为 $T_{k-1} \cdot T_{n-k}$。

总方案数为 $T_n = \sum_{k=1}^n T_{k-1} \cdot T_{n-k}$。

即为 Catalan 数列的递推式，所以 $T_n = C_n$。
...
因此答案为 $T_n = \binom{2n}{n} - \binom{2n}{n-1} = C_n$。

两种推导的结果其实是等价的，将在下面证明。
