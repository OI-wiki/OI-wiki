...
定理的描述用到了置换群的轮换指标的概念。它和具体的染色问题无关。它描述了置换群的结构，在组合计数问题中起到重要的作用。关于它的应用，文末会给出简单的例子，更一般的讨论可以参考 [组合问题的形式化方法](../poly/symbolic-method.md#有限制的构造) 相关讨论。
...
根据 [置换与排列](../permutation.md#置换的型) 一文的分析，它的轮换表示是
...
给定 $n$ 个顶点的完全图，则它的空间对称群正是 $S_n$。它对全体顶点的集合的作用的轮换指标就由上文的 $Z(S_n)$ 给出。但是，它对全体边的集合的作用的置换表示并不相同。比如说，集合的大小就不相同，全体边的数目是 $n(n-1)/2$。对于边的情形，需要额外的分析。
...
-   四面体群（tetrahedral group），即正四面体的空间对称群：
    ...
    共计 $1+2\times4+1\times3=12$ 个对称操作。它对应的置换群的轮换指标如下。
...
-   八面体群（octahedral group），即正方体（和正八面体）的空间对称群：
    ...
    共计 $1+2\times 4+1\times 6+3\times 3=24$ 个对称操作。它对应的正方体的置换群的轮换指标如下。
...
-   二十面体群（icosahedral group），即正十二面体（和正二十面体）的空间对称群：
    ...
    共计 $1+2\times 10+1\times 15+6\times 4=60$ 个对称操作。它对应的正十二面体的置换群的轮换指标如下。
