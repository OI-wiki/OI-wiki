...
???+ note "不定方程非负整数解计数"
    给出不定方程 $\sum_{i=1}^nx_i=m$ 和 $n$ 个限制条件 $x_i\leqslant b_i$，其中 $m,b_i \in \mathbb{N}$. 求方程的非负整数解的个数。
...
2.  元素：变量 $x_i$.
3.  属性：$x_i$ 的属性即 $x_i$ 满足的条件，即 $x_i\leqslant b_i$ 的条件
...
那么问题变成，对于一些 $\overline{S_{a_i}}$ 的交集求大小。考虑 $\overline{S_{a_i} }$ 的含义，表示 $x_{a_i}\geqslant b_{a_i}+1$ 的解的数目。而交集表示同时满足这些条件。因此这个交集对应的不定方程中，有些变量有 **下界限制**，而有些则没有限制。

能否消除这些下界限制呢？既然要求的是非负整数解，而有些变量的下界又大于 $0$，那么我们直接 **把这个下界减掉**，就可以使得这些变量的下界变成 $0$，即没有下界啦。因此对于

$$
\left|\bigcap_{a_i<a_{i+1} }^{1\leqslant i\leqslant k}S_{a_i}\right|
$$
...
???+ note "HAOI2008 硬币购物"
    ...
    $n\leqslant 10^3,S\leqslant 10^5$.

如果用背包做的话复杂度是 $O(4nS)$，无法承受。这道题最明显的特点就是硬币一共只有四种。抽象模型，其实就是让我们求方程 $\sum_{i=1}^4C_ix_i=S,x_i\leqslant D_i$ 的非负整数解的个数。

采用同样的容斥方式，$x_i$ 的属性为 $x_i\leqslant D_i$. 套用容斥原理的公式，最后我们要求解
...
是不是很有容斥的味道了？由于容斥原理本身没有二元组的形式，因此我们把 **所有** 的边 $(i,j)$ 映射到 $T=\frac{n(n+1)}{2}$ 个整数上，假设将 $(i,j)$ 映射为 $k,1\leqslant k\leqslant T$，同时 $Q_{i,j}$ 映射为 $Q_k$. 那么属性 $x_i=x_j$ 则定义为 $P_k$.
...
???+ note " 求最大公约数为 $k$ 的数对个数 "
    设 $1 \leqslant x, y \leqslant N$，$f(k)$ 表示最大公约数为 $k$ 的有序数对 $(x, y)$ 的个数，求 $f(1)$ 到 $f(N)$ 的值。
...
$$
f(k)= \lfloor (N/k) \rfloor ^2 - \sum_{i=2}^{i*k \leqslant N} f(i*k)
$$
...
???+ note "欧拉函数公式"
    求欧拉函数 $\varphi(n)$。其中 $\varphi(n)=|\{1\leqslant x\leqslant n|\gcd(x,n)=1\}|$。
...
???+ note "DAG 计数"
    对 $n$ 个点带标号的有向无环图进行计数，对 $10^9+7$ 取模。$n\leqslant 5\times 10^3$。
...
**证明：** 不妨设 $\forall 1\leqslant i<n,x_i\leqslant x_{i+1}$。则有：
...
???+ note "[PKUWC2018 随机游走](https://loj.ac/problem/2542)"
    ...
    $1\leqslant n\leqslant 18,1\leqslant Q\leqslant 5000,1\leqslant |S|\leqslant n$。
