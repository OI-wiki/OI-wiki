author: Marcythm, sshwy

本文介绍如何用牛顿迭代法（Newton's method for finding roots）求解方程的近似解，该方法于 17 世纪由牛顿提出。 
...
假设我们目前的近似解是 $x_i$ ，那么为了得到更优的近似解，我们画出与 $f(x)$ 切于点 $(x_i,f(x_i))$ 的直线 $l$ ，将 $l$ 与 $x$ 轴的交点横坐标记为 $x_{i+1}$ 。并重复这个迭代的过程。
根据导数的几何意义，可以得到如下关系：
