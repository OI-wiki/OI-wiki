...
???+ note "思路分析"
    ...
    现在回到该问题的树上版本。对于树上的结点 $i$，其子结点集合记作 $S(i)$，则该处的 DP 方程为
    ...
    首先，通过树链剖分将问题转化为链上版本。设 $h$ 是 $i$ 的重子结点，那么上述递推方程可以写作
    ...
    总结了轻子结点的贡献。根据上文描述，这些变换都可以写作热带半环上的矩阵形式，所以，整个问题也就可以在树剖后的线段树上维护。但是，直接用树剖加线段树的单次修改是 $O(\log^2n)$ 的，所以需要用到上文提到的全局平衡二叉树优化到 $O(\log n)$，当然也可以用 LCT 维护。
