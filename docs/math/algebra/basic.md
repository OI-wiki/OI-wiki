本章节将简要介绍抽象代数的相关知识。现阶段算法竞赛的主要内容并不直接考察抽象代数的知识，但是在算法的描述或是问题的题解中常常会牵涉一些抽象函数的基本概念，这使得掌握了基础抽象代数概念的读者能够更快速理解一些算法。因此，这部分内容并不是任何选手的必修知识，而仅供那些感兴趣或者可能从中受益的读者参考使用。又因为 [OI Wiki 不是百科全书](https://oi-wiki.org/intro/what-oi-wiki-is-not/#oi-wiki-%E4%B8%8D%E6%98%AF%E7%99%BE%E7%A7%91%E5%85%A8%E4%B9%A6)，本章节将避免过全过深的介绍抽象代数的知识，而会集中在基础概念以及与 OI 其他部分知识联系最为紧密的部分。想系统学习抽象代数知识的读者，应当参考专业的抽象代数教科书学习。
...
- 组合数学中，[Polya 计数原理](../combinatorics/polya.md) 的严格表述和证明需要用到群论的相关概念。
...
2. 有单位元（identity element）：存在 $e\in G$，使得对于任意 $a\in G$，都成立 $a\cdot e = e\cdot a = a$。这里，$e$ 称为 $G$ 的单位元，也称幺元。
3. 存在逆元（inverse element）：对于所有 $a\in G$，都存在相应的 $b\in G$ 使得 $a\cdot b=b\cdot a=e$。这里，$b$ 称为 $a$ 的逆元。
...
- 空间对称群（symmetry group）：对于给定几何图形，能够使其与自身重合的变换全体也在映射的复合下构成群。这描述了该几何图形的空间对称性。
- 整数的加法群：整数集 $\mathbf Z$ 在加法 $+$ 运算下构成群 $(\mathbf Z,+)$。单位元是 $0$，逆元是相反数。
- 模 $n$ 的整数的乘法群：给定模 $n$，所有与 $n$ 互质的整数对应的剩余系，在乘法运算下构成群 $((\mathbf Z/n\mathbf Z)^\times,\times)$。单位元是 $\bar 1$，即 $1$ 对应的剩余系，逆元就是模 $n$ 的乘法逆元（对应的剩余系），其存在性由裴蜀定理保证。
- 一般线性群：给定 $n$ 和数域 $F$，则 $n$ 维的全体可逆方阵在乘法运算下构成群 $GL_n(F)$。单位元是单位矩阵，逆元是逆矩阵。
...
- 模 $n$ 的所有非零剩余系在乘法意义下往往不构成群。比如说 $\mathbf Z_6\setminus\{\overline 0\}$ 中，$\overline 2\times\overline 3=\overline 0$ 不属于这个集合，这意味着乘法都不是这个集合上良定义的二元运算。

有时，也需要讨论这些更不完善的结构的性质。因此，可以定义如下概念，它们比群更宽泛。

- 半群：给定集合 $G$ 和其上二元运算 $\cdot$，如果该运算满足结合律，则称 $(G,\cdot)$ 是一个 **半群**（semigroup）。 
- 幺半群：给定半群 $(G,\cdot)$，如果还存在幺元，则称 $(G,\cdot)$ 是一个 **幺半群**（monoid）。

上面的例子中，$(\mathbf Z,\times)$ 是幺半群，而 $(\mathbf N_+,+)$ 是半群。

最后，很多熟悉的群上的运算除了满足结合律外，还满足交换律。

- 交换律（commutative property）：对于所有 $a,b\in G$，成立 $a\cdot b=b\cdot a$。

这类群的结构相对简单，它们称作 Abel 群，也称作交换群。

- Abel 群：给定群 $(G,\cdot)$，如果该运算满足交换律，则称 $(G,\cdot)$ 是一个 **Abel 群**（Abelian group）。

比如说 $(\mathbf Z,+)$ 就是一个 Abel 群，但是当 $n\ge3$ 时，$S_n$ 并不是 Abel 群。
...
- 幺环：给定环 $(R,+,\cdot)$，如果存在乘法单位元，记作 $1$，则称 $(R,+,\cdot)$ 是一个 **幺环**（ring with identity）。
- 除环：给定非零幺环 $(R,+,\cdot)$，如果对于所有非 $0$ 元素 $a\in R$，都存在乘法逆元，记作 $a^{-1}$，则称 $(R,+,\cdot)$ 是一个 **除环**（division ring）。

这里除环的定义中有趣的一点是，它将 $0$ 视为乘法结构中的特殊元素。这是因为 [^zero-multiplication]

$$
0\cdot a+0 = 0\cdot a = (0+0)\cdot a = 0\cdot a + 0\cdot a \implies 0 = 0\cdot a,
$$

同理，$0 = a\cdot 0$。也就是说，环中加法单位元乘以任何元素都得到其自身。这样，它自然不会存在乘法逆元，除非它本身就是乘法单位元。这样的环只有零环（见下面的例子）。
...
- 整环：给定非零环 $(R,+,\cdot)$，称它为整环（integral domain），如果它是交换环，有乘法单位元，且无零因子。
...
???+ note "[【模板】"动态DP"&动态树分治（加强版）](https://www.luogu.com.cn/problem/P4751)"
