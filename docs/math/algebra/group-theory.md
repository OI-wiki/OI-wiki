...
-   群同态：给定群 $(G,\cdot_G)$ 和 $(H,\cdot_H)$，则称映射 $\pi:G\rightarrow H$ 是一个自群 $G$ 到群 $H$ 的 **同态**（homomorphism），如果 $\pi(g_1\cdot_G g_2)=\pi(g_1)\cdot_H\pi(g_2)$。
...
-   同态的核：给定自群 $G$ 到群 $H$ 的同态 $\pi:G\rightarrow H$，则称 $\ker\pi=\{g\in G:\pi(g)=e\}$ 是同态 $\pi$ 的 **核**（kernel），这里，$e$ 是 $H$ 的单位元。
