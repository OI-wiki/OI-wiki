...
???+ abstract "子群"
    给定群 $(G,\cdot)$，如果 $(H,\cdot)$ 也是一个群，则称子集 $H\subseteq G$ 是 $G$ 的一个 **子群**（subgroup），记作 $H\le G$。
...
一般地，给定群 $G$ 和其中的一部分操作 $S$，从 $S$ 中的操作出发，重复进行复合和取逆运算有限次，能够得到的所有结果的集合成为 $G$ 的一个子群。这称为由子集 $S$ 生成的子群。

???+ abstract "由子集生成的子群"
    给定群 $G$ 和它的非空子集 $S\subseteq G$，如果 $H$ 是包含 $S$ 的 $G$ 的子群中（依包含关系）最小的，则子群 $H$ 称为 **由子集 $S$ 生成的子群**（subgroup generated by a subset），并记作 $\langle S\rangle$。

可以证明，任给定这样的子集 $S$，总能找到这样的子群：$\langle S\rangle$ 可以构造为所有包含 $S$ 的 $G$ 的子群的交。

??? example " 例子：正三角形的空间对称群 $D_6$（续）"
    在群 $D_6$ 中选定旋转操作 $r$，重复应用它和它的逆操作，就得到子群 $\{e,r,r^2\}$。它可以记作 $\langle r\rangle$。这里，略去了列举元素的集合记号。群 $D_6$ 中的所有非平凡子群都可以通过选定某个操作来生成。

有些子集生成的子群是群本身。这样的子集尤为特殊，称为群的生成子集。

???+ abstract "群的生成子集"
    给定群 $(G,\cdot)$ 和子集 $S\subseteq G$，如果 $\langle S\rangle=G$，则称 $S$ 是 $G$ 的 **生成子集**（generating set of a group）。
...
???+ abstract "循环群"
    给定群 $G$，如果存在 $x\in G$，成立 $G=\langle x\rangle$，则称 $G$ 是一个 **循环群**（cyclic group）。
...
???+ note "Lagrange 定理"
    给定有限群 $G$ 和它的子群 $H\le G$，成立 $|G|=[G:H]|H|$，这里，$[G:H]$ 表示 $G$ 中子群 $H$ 的左（右）陪集数，称为群 $G$ 中子群 $H$ 的 **指数**（index）。
...
???+ abstract "正规子群"
    给定群 $G$ 和它的子群 $N\le G$，如果对所有 $h\in N$ 和 $g\in G$，都成立 $ghg^{-1}\in N$，换言之，对所有 $g\in G$，都成立 $gNg^{-1}\subseteq N$，则称 $N$ 是 $G$ 的一个 **正规子群**（normal subgroup），记作 $N\trianglelefteq G$。
...
???+ abstract "群同态"
    给定群 $(G,\cdot_G)$ 和 $(H,\cdot_H)$，如果 $\varphi$ 保持群的运算，即对所有 $g_1,g_2\in G$ 都成立 $\varphi(g_1\cdot_G g_2)=\varphi(g_1)\cdot_H\varphi(g_2)$，则称映射 $\varphi:G\rightarrow H$ 是一个自群 $G$ 到群 $H$ 的 **同态**（homomorphism）。
...
???+ abstract "群同构"
    给定自群 $G$ 到群 $H$ 的同态 $\varphi:G\rightarrow H$，如果 $\varphi$ 是双射，则称 $\varphi$ 是群 $G$ 和群 $H$ 之间的 **同构**（isomorphism），记作 $G\cong H$。
...
???+ abstract "群在集合上的作用"
    给定群 $G$ 和集合 $X$，以及映射 $G\times X\rightarrow X$，并记 $(g,x)$ 在该映射下的像为 $g\cdot x$，如果该映射对所有 $g_1,g_2\in G$ 和 $x\in X$ 都满足条件 $g_1\cdot(g_2\cdot x)=(g_1g_2)\cdot x$ 和 $e\cdot x=x$，那么就称该映射为群 $G$ 在集合 $X$ 上的 **群作用**（group action）。
...
???+ abstract "置换群"
    给定群 $G$，如果它是某个对称群的子群，则称群 $G$ 是一个 **置换群**（permutation group）。
...
???+ abstract "$p$‑群 "
    给定群 $G$，如果 $|G|=p^\alpha$，这里，$\alpha$ 是正整数，则称它为 **$p$‑群**（$p$-group）。

???+ abstract "$p$‑子群 "
    给定群 $G$ 和它的子群 $P\le G$，如果 $P$ 本身是一个 $p$‑群，则称 $P$ 为 **$p$‑子群**（$p$-subgroup）。

???+ abstract "Sylow $p$‑子群 "
    给定群 $G$ 和它的子群 $P\le G$，如果 $|G|=p^\alpha m$ 且 $p$ 不能整除 $m$，且 $|P|=p^\alpha$，则称 $P$ 为 **Sylow $p$‑子群**（Sylow $p$-subgroup）。
...
???+ abstract "有限生成"
    给定群 $G$，如果它的一个生成子集是有限的，则称它是 **有限生成的**（finitely generated）。
...
???+ abstract "直积"
    给定群 $(G,\cdot_G)$ 和群 $(H,\cdot_H)$，则群 $G$ 和群 $H$ 的 **直积**（direct product）是群 $(G\times H,\cdot)$，其中，二元运算 $\cdot:(G\times H)\times(G\times H)\rightarrow G\times H$ 定义为 $(g_1,h_1)\cdot(g_2,h_2)=(g_1\cdot_G g_2,h_1\cdot_H h_2)$。群 $G$ 和群 $H$ 的直积记为 $G\times H$。
