...
所有二次整数环 $\mathbf Z[\omega]$ 都是整环。其中，当 $D=-1$ 时，$\mathbf Z[\sqrt{-1}]$（或记作 $\mathbf Z[\mathrm{i}]$）也称 Gauss 整环；当 $D=-3$ 时，$\mathbf Z\left[\dfrac{1+\sqrt{-3}}{2}\right]$ 则称为 Eisenstein 整环。
...
尽管很多二次整数环并不是唯一分解整环，但是它们都是 [Dedekind 整环](https://en.wikipedia.org/wiki/Dedekind_domain)。这意味着，所有二次整环中的非平凡理想都可以唯一分解为一系列素理想之积。但如果二次整数环本身并非主理想整环，这些素理想因子并不一定对应着素元，因而唯一分解定理（即数分解成素数的乘积）不再成立：这也是研究理想而不是数的最初动机。
...
虽然域上的多项式成立唯一分解定理，但是并没有一般的办法判断给定的多形式是否可约。次数比较小的情形相对容易。比如说，所有的一次多项式都是不可约多项式。在特殊的域上，所有的不可约多项式都是一次多项式。这样的域称为 [代数闭域](./field-theory.md#代数闭域)。在这样的域中，所有不恒等于非零常数的多项式都有根，因而任何大于一次的多项式都可以进一步分解。一个这样的例子是复数域 $\mathbf C$。而对于实数域 $\mathbf R$，则存在二次的不可约多项式；对于有理数域 $\mathbf Q$，不可约多项式的结构就更为复杂。
...
对于唯一分解整环 $R$，因为相应的分式域 $F$ 上的多项式环是唯一分解整环，而 Gauss 引理说明，分式域上的多项式环 $F[x]$ 和 $R[x]$ 中的多项式的分解是相互对应的，所以 $R[x]$ 也是唯一分解整环。因此，有如下定理：
