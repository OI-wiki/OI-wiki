...
???+ note "对应定理"
    设环 $R$ 有理想 $I$，则全体包含 $I$ 的环 $R$ 的子环 $\mathcal S=\{S:I\subseteq S\subseteq R\}$ 和商环 $R/I$ 的全体子环 $\mathcal T=\{T:T\text{ 是 }R/I\text{ 的子环}\}$ 之间存在双射 $\varphi:\mathcal S\rightarrow\mathcal T$，它将 $S\in\mathcal S$ 映射至 $S/I\in\mathcal T$．这个双射保持子环的包含关系，且环 $R$ 的理想总是映射到 $R/I$ 的理想．
