...
???+ abstract "欧几里得整环"
    对于整环 $R$，如果存在映射 $N:R\rightarrow\mathbf N$，满足 $N(0)=0$，而且对于任意 $a,b\in R$，其中，$b\neq0$，都存在 $q,r\in R$ 使得 $a=qb+r$ 成立，这里，$r=0$ 或 $N(r)<N(b)$，则称整环 $R$ 为 **欧几里得整环**（Euclidean domain, ED）。映射 $N$ 称为欧几里得整环中元素的范数（norm）。
...
整环上的多项式环中性质最为简单的，当然是域上的多项式环。域 $F$ 上的多项式环 $F[x]$ 因为系数可以做除法，所以可以定义带余除法。不妨设非零多项式 $f(x)$ 的范数 $N(a(x))=\deg f(x)$，而且 $N(0)=0$。那么，对于 $F[x]$ 中的多项式 $f(x)$ 和非零多项式 $g(x)$，显然可以做带余除法
...
??? example "例子"
    1.  多项式 $x^3-2$ 不可约。对 $p=2$ 应用 Eisenstein 判别法即可。
    2.  多项式 $x^4+1$ 不可约。否则，$(x+1)^4+1=x^4+4x^3+6x^2+4x+2$ 也可约。但是，对 $p=2$ 应用 Eisenstein 判别法可知，后者并不可约。 
