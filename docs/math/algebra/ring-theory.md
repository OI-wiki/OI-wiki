...
下面考虑唯一分解整环 $R$ 上的多项式环 $R[x]$。直接在 $R[x]$ 上做运算，因为系数时常不能做除法，很多运算受到限制。不妨考虑将 $R$ 扩充到它的分式域 $F$，进而考虑将 $R[x]$ 中的多项式 $f(x)$ 在 $F[x]$ 中做分解。已知 $F[x]$ 是唯一分解整环，那就可以通过 $F[x]$ 中 $f(x)$ 的分解反推出 $R[x]$ 中的分解。幸运的是，这样的想法总是可行的。
