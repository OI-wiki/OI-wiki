...
???+ warning "注意"
    当指数很大时，需利用 [扩展欧拉定理](./number-theory/fermat.md) 降幂后计算。
...
现在，每一种操作都被表示为了一个矩阵，变换序列可以用矩阵的乘积来表示，而一个 Repeat 操作相当于取一个矩阵的 $k$ 次幂。这样可以用 $O(m \log k)$ 计算出整个变换序列最终形成的矩阵。最后将它应用到 $n$ 个点上，总复杂度 $O(n + m \log k)$。
