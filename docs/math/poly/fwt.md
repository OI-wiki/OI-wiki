...
我们重新看看我们的 $\cup$ 运算，发现他实际上就是二进制下的取 $\max$。我们将其拓展到 $K$ 进制，定义 $i\cup j=\max(i,j)$，有：
...
我们重新看看我们的 $\cap$ 运算，发现他实际上就是二进制下的取 $\min$。我们将其拓展到 $K$ 进制，定义 $i\cap j=\min(i,j)$，有：
...
我们重新看看我们的 $\oplus$ 运算，发现他实际上就是二进制下的不进位加法。我们将其拓展到 $K$ 进制，定义 $i\oplus j=(i+j)\bmod K$，有：
