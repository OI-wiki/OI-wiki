...
$$
FWT[A]_i = \sum_{j=0}^{(n-2)/2} c(i,j) A_j+\sum_{j=n/2}^{n-1} c(i,j) A_j
$$
...
$$
FWT[A]_i = c(i_0,0)\sum_{j=0}^{(n-2)/2} c(i',j') A_j+c(i_0,1)\sum_{j=n/2}^{n-1} c(i',j') A_j
$$

如果 $i_0=0$，则有：

$$
FWT[A]_i = c(0,0)\sum_{j=0}^{(n-2)/2} c(i',j') A_j+c(0,1)\sum_{j=n/2}^{n-1} c(i',j') A_j
$$

$i_0=1$ 则有：

$$
FWT[A]_i = c(1,0)\sum_{j=0}^{(n-2)/2} c(i',j') A_j+c(1,1)\sum_{j=n/2}^{n-1} c(i',j') A_j
$$
