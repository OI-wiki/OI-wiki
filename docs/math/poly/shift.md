...
多项式平移是简单情况的多项式复合变换即 $f(x)\mapsto f(x+c)$，现给出 $f(x)=\sum _ {i=0}^nf_ix^i$ 的系数和一个常数 $c$，求 $f(x+c)$ 的系数。
...
考虑二项式定理 $(a+b)^n=\sum _ {i=0}^n\binom{n}{i}a^ib^{n-i}$ 那么
...
对问题稍加修改，假设对于某个 $d$ 给出的点值为 $f(d),f(d+k),\dots ,f(d+nk)$，我们可以计算 $f(c+d),f(c+d+k),\dots ,f(c+d+nk)$，视作平移 $g(x)=f(d+kx)$ 的点值 $g(0),g(1),\dots ,g(n)$ 为 $g\left(\frac{c}{k}\right),g\left(\frac{c}{k}+1\right),\dots ,g\left(\frac{c}{k}+n\right)$。
...
???+ note "例题 [P5408 第一类斯特林数·行](https://www.luogu.com.cn/problem/P5408)"
    在模素数 $167772161$ 意义下求 ${n\brack 0},{n\brack 1},\dots ,{n\brack n}$，其中 $1\leq n\lt 262144$。
...
所以只需 $2d+1$ 个点值可以唯一确定 $g _ {2d}(x)$，那么使用连续点值平移计算 $g_d((d+1)v),g_d((d+2)v),\dots ,g_d(2dv)$（即平移 $h(x)=g_d(vx)$ 的点值 $h(0),h(1),\dots ,h(d)$ 为 $h(d+1),h(d+2),\dots ,h(2d)$）和 $g_d(d),g_d(v+d),\dots ,g_d(2dv+d)$（即平移 $h(x)=g_d(vx)$ 的点值 $h(0),h(1),\dots ,h(d)$ 为 $h\left(\frac{d}{v}\right),h\left(\frac{d}{v}+1\right),h\left(\frac{d}{v}+2\right),\dots ,h\left(\frac{d}{v}+2d\right)$）后将这两者的对应点值相乘即得 $g _ {2d}(0),g _ {2d}(v),\dots ,g _ {2d}(2dv)$。
...
记 $H_n=\sum _ {k=1}^n\frac{1}{k}$，一步递推为
