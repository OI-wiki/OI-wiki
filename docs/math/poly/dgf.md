...
$$
\tilde{F}(x) = \sum_{i\geqslant 1}\frac{f_i}{i^x}
$$
...
序列 $[1, 1, 1, \ldots]$ 的 DGF 是 $\sum_{i\geqslant 1}\frac{1}{i^x} = \zeta(x)$。$\zeta$ 是黎曼函数。
...
$$
\tilde{F}(x) = \prod_{p \in \mathcal{P}} \left(1 + \sum_{k\geqslant 1} \frac{p^{3k-1}(p-1)}{p^{kx}} \right) = \prod_{p\in \mathcal{P}} \frac{1-p^{2-x}}{1-p^{3-x}} = \frac{\zeta(x-3)}{\zeta(x-2)}
$$
