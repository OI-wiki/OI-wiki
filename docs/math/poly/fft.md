...
值的注意的是，因为是单位复根，所以说我们需要令 $n$ 项式的高位补为零，使得 $n = 2 ^ k, k \in \mathbf{N}$。
...
- $G(\omega_{n/2}^k)$ 的值存储在数组下标为 $k$ 的位置，$H(\omega_{n/2}^k)$ 的值存储在数组下标为 $k + n/2$ 的位置。
- $f(\omega_n^k)$ 的值将存储在数组下标为 $k$ 的位置，$f(\omega_n^{k+n/2})$ 的值将存储在数组下标为 $k + n/2$ 的位置。

因此可以直接在数组下标为 $k$ 和 $k + n/2$ 的位置进行覆写，而不用开额外的数组保存值。此方法即称为 **蝶形运算**。

最后，再详细说明一下如何借助位逆序置换和蝶形运算实现 FFT：

1. 对原数组进行位逆序置换；
2. 枚举段长 $s = n/2 = 1, 2, 4, 8, \cdots, N/2$，即序列 $\{G(\omega_{n/2}^k)\}$ 和 $\{H(\omega_{n/2}^k)\}$ 的长度；
3. 同时枚举序列 $\{G(\omega_{n/2}^k)\}$ 的左端点 $l_g = 0, 2s, 4s, \cdots, N-2s$ 和序列 $\{H(\omega_{n/2}^k)\}$ 的左端点 $l_h = s, 3s, 5s, \cdots, N-s$；
4. 合并两个段时，枚举 $k = 0, 1, 2, \cdots, s-1$，此时 $G(\omega_{n/2}^k)$ 存储在数组下标为 $l_g + k$ 的位置，$H(\omega_{n/2}^k)$ 存储在数组下标为 $l_h + k$ 的位置；使用蝶形运算求出 $f(\omega_n^k)$ 和 $f(\omega_n^{k+n/2})$ 并直接覆写。
