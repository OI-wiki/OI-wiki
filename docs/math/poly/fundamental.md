...
在 1940 年 8 月、1943 年 8 月和 1947 年 7 月，林士谔先后在 MIT 出版的《数学物理》杂志上接连正式发表了3篇关于解算高阶方程式复根方法的论文，每次均有改进。

这个方法今天还在现代计算机中进行快速运算，计算机程序包（如MATLAB）中的多项式求根程序依据的原理也是这个算法。

[[1]林士谔. 论劈因法解高阶特征方程根值的应用问题[J]. 数学进展, 1963(03):207-217.](https://cnki.net/kcms/detail/detail.aspx?filename=SXJZ196303000&dbcode=CJFD&dbname=CJFD1979)
...
```C
    void Shie(double a[], int n, double *p, double *q)//a是原始的多项式，n是多项式次数，p是待求的一次项，q是待求的常数项
    {
        memset(b,0,sizeof(b));//数组b是多项式a除以当前迭代二次三项式的商
        memset(c,0,sizeof(c));//数组c是多项式b乘以x平方再除以当前迭代二次三项式的商
        *p = 0;
        *q = 0;
        double dp = 1;
        double dq = 1;
