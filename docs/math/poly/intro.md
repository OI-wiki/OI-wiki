...
复数域上，由代数基本定理可得，复系数 $n$ 次多项式 $f$ 可唯一分解为如下形式
...
此时类比数的最大公因数，可得多项式的 **最大公因式**  
（the greatest common factor）。其可用欧几里得算法求解
...
[HALF-GCD](https://loj.ac/p/172) 允许我们在 $O(n\log n)$ 时间内计算多项式欧几里得。
