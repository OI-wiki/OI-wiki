...
???+ note "证明"
    一个最优解无非是线性规划的约束解出来的高维超多面体的顶点。而一个顶点无非是将一些线性无关的满秩不等式改成等式后的线性方程组的解。如果矩阵是全幺模的，那么所有如上的线性方程组，依 [Cramer 法则](https://en.wikipedia.org/wiki/Cramer%27s_rule) 可知，解为整数。
