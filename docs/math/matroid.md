...
**定义**：有色拟阵是一种特殊形式的划分拟阵的拟阵，其中每个元素都赋予了颜色。给定基础集 $E$ 和颜色集 $C$，每个元素 $e \in E$ 都与某个颜色 $c \in C$ 相关联。有色拟阵的独立集不仅需要满足普通拟阵的独立性条件，还必须遵守颜色上指定的限制，例如同一种颜色的元素在独立集中最多选取一定数量。
...
设 $y$ 为 $P$ 中权值最大的元素，则 $A' = A \cup \{ y \}$ 也是某个权值最大独立集的子集。

1.  假设 $A' = A \cup \{ y \}$ 不是任何权值最大独立集的子集，$T$ 为一个权值最大独立集，且 $ |A'| < |T| $。

2.  由于 $|A'|< |T|$，根据拟阵的 **扩张性**，存在 $x \in T \setminus A'$ 使得 $A' \cup \{ x \} \in \mathcal{I}$。

3.  利用 **扩张性**，不断将 $x$ 加入 $A'$，最终构造出一个新的独立集 $A'$，使得 $|A'| = |T|$，且 $A' \subseteq T$。

4. 设 $K = A' \cap T$，此时有 $x = T \setminus K$，$y = A' \setminus K$。
   由于 $y$ 为 $P$ 中权值最大的元素，有 $w(x) \leq w(y)$。

5. 因此，$w(A') = w(K) + w(y) \geq w(K) + w(x) = w(T)$。
...
对于定义在同一基础集 $S$ 上的两个拟阵 $M_1 = (S, \mathcal{I}_1)$ 和 $M_2 = (S, \mathcal{I}_2)$，若 $\mathcal{I} = \mathcal{I}_1 \cap \mathcal{I}_2$ 满足拟阵的独立集公理，则称 $M = (S, \mathcal{I})$ 为 $M_1$ 和 $M_2$ 的 **交**。
