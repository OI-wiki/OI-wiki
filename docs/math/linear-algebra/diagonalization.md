...
定理：设 $T$ 是空间 $V$ 的一个幂零变换，$W$ 是一个 $r$ 维 $T$ 循环子空间，$\xi$ 是 $W$ 中的向量。如果存在一个整数 $k$，使得
...
定理：设 $T$ 是 $n$ 维空间 $V$ 的一个幂零变换，$x^r$ 是 $T$ 的最小多项式，令 $W_1$ 是一个 $r$ 维 $T$ 循环子空间，那么存在 $W_1$ 的一个余子空间 $W_2$，使得：
...
定理：设 $T$ 是 $n$ 维空间 $V$ 的一个幂零变换，那么 $V$ 可以分解为 $T$ 循环子空间的直和：

$$
V=W_1\oplus W_2\oplus\cdots\oplus W_S
$$

定理：每一个 $n$ 阶幂零矩阵都与一个形如：
...
定理：如果规定 $T$ 循环子空间 $W_i$ 按照维数 $r_i$ 降序排列 $r_1\geq\cdots\geq r_S$，那么将 $V$ 分解为 $T$ 循环子空间的方法是由 $T$ 唯一确定的。
