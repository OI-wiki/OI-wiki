...
由于对向量的模长没有要求，所以每条直线的方向向量都有无数个。以下是几种空间中方向向量的求解方法：

-   若已知直线 $l：ax+by+c=0$，则直线 $l$ 的方向向量为 $\boldsymbol{s}=(-b,a)$ 或 $(b,-a)$。

-   若直线 $l$ 的斜率为 $k$, 则 $l$ 的一个方向向量为 $\boldsymbol{s}=(1,k)$。

-   若有 $A(x1,y1),B(x2,y2)$，则 $AB$ 所在直线的一个方向向量为 $\boldsymbol{s}=(x_2-x_1,y_2-y_1)$。
