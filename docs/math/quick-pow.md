...
既然使用了 `long double`，就无疑会有进度误差。极端情况就是第一个有效数字（二进制下）在小数点后一位。在 linux 环境下，gcc 编译器将 `long double` 解释成 $80$ 位拓展小数（即符号为 $1$ 位，偏置指数 **至少** 为 $12$ 位，有效数 **至少** 为 $64$ 位，至少 $77$ 位，但根据 **编译器不同各部分占据位数不同**)。所以 $\dfrac ap$ 最差从第 $65$ 位开始出错，误差范围为 $\left(-2^{-64},2^{64}\right)$。乘上 $b$ 这个 $64$ 位整数，误差范围为 $(-0.5,0.5)$，再加上 $0.5$ 误差范围为 $(0,1)$，取整后误差范围位 $\{0,1\}$。于是乘上 $-m$ 后，误差范围变成 $\{0,-m\}$，我们需要判断这两种情况。
