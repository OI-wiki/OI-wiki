...
首先对于 p 进行质因数分解： $p=p_{1}^{k_1}p_{2}^{k_2}\cdots p_{n}^{k_n}$ 如果可以求出每个 $C_{n}^{m}\equiv a_i \pmod {p_{i}^{q_i}}$ 那么对于同余方程组
...
但是可以发现 $p_{i}^{q_i}$ 也不是素数，接下来介绍如何计算 $C_{n}^{m}\bmod p^t$ 。

可以利用求组合数的公式 $C_{n}^{m}=\frac{n!}{m!(n-m)!}$ ，如果可以分别计算出 $n!\bmod p^t,m!\bmod\ p^t$ 和 $(n-m)!\bmod p^t$ 那么就可以得到答案。以第一个式子为例，当 $p=3,t=2,n=19$ 时：
