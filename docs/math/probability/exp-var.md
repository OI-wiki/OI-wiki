...
??? note "题解"
    记 $f(x)$ 表示长度为 $x$ 的期望次数。$x \leqslant d$ 的情形平凡。
...
Pearson 相关系数描述了两个随机变量之间线性关联的紧密程度。$|\rho_{X,Y}|$ 越大，则 $X$ 与 $Y$ 之间的线性关联程度越强。不难证明 $|\rho_{X,Y}| \leqslant 1$，且 $|\rho_{X,Y}| = 1$ 仅可能出现在以下两种情况
