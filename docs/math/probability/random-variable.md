...
受篇幅限制，本文并不打算不展开描述定语“Borel 可测”的含义。如果你不明白其中的含义，简单地将其无视并不会影响后续内容的理解，或你也可以将其感性理解为“看上去性质不太差”的函数。
...
???+note ""
    可能有些同学注意到了，[快速上手概率论](/math/expectation/) 中对随机变量独立性的定义是用形如 $P(X = \alpha)$ 的概率定义的，但由于连续性随机变量取特定值的概率通常是 $0$，故在定义时借助分布函数才是更加明智的选择。
