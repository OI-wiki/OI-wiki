...
所以我们令 $x=g^c$，$g$ 是 $p$ 的原根（我们一定可以找到这个 $g$ 和 $c$），则为求 $$(g^c)^a \equiv b \bmod p$$ 的关于 $c$ 的解集.

稍加变换，则有 $$(g^a)^c \equiv b \bmod p$$，于是就转换成了我们熟知的 **BSGS** 的基本模型了，即可在 $O(\sqrt p)$ 解决.
...
于是可以得到一种基于**原根分布**的算法来找原根，首先把 $p-1$ 的因数全部求出来，然后从 $2$ 到 $p-1$ 枚举，判断是否为原根，如果对于数 $g$，$\exist g^t \equiv 1 \bmod p$，$t$ 是 $p-1$ 的因数，则 $g$ 一定不是 $p$ 的原根.
