...
上述两段代码循环部分，不会处理值为 $0$ 的子掩码。不过我们可以在循环的外面处理它们，或者增添一段不太优雅的代码，例如：
...
现在我们可以一起来检验一下为什么上面的算法可以不重复地降序访问 $m$ 的所有子掩码。

假设我们现在有一个位掩码 $s$ ，想要继续获得下一个掩码，可以通过将掩码 $s$ 减去一位的方法来实现：去除最右边的设置位，然后在它右边的所有位都会置为 1。这样我们删除所有没有包含在 $m$ 中的“额外的”一位，这样他们就不能成为子掩码的一部分。我们可以通过按位操作 $(s-1)\&m$ 实现上述的删除操作。这样的话，我们就截取了掩码 $s-1$ 以确定它可以采用的最大值，即，按降序排列的 $s$ 的下一个子掩码。

因此，该算法可以降序生成该掩码的所有子掩码，每次迭代仅执行两个操作。

特别地，当 $s=0$ 时，在执行 $s-1$ 操作后，我们得到一个掩码，其中所有位都被设置（位表示为 $-1$ ），因此在 $(s-1) \& m$ 后，我们有 $s=m$ 。所以，遇到 $s=0$ 时我们开始小心——如果循环不以 $0$ 结束，算法可能会进入无限循环。

## 遍历所有掩码及其子掩码

在很多使用位掩码动态编程的问题中，要求便利所有位掩码，并且对于每个掩码，遍历其所有子掩码：
 ...
 **证明二** ：注意如果 $m$ 共有 $k$ 个使能位，就有 $2^k$ 个子掩码。由于我们共有 $\dbinom{n}{k}$ 个掩码和 $k$ 个使能位（参见 [二项式系数](./combination.md) ），则所有掩码的组合总数为：
