...
补码：正数的补码为其（二进制）本身，负数的补码是其（二进制）取反后加一。有符号整数的第一位是符号位，同样会取反。
...
与前面的四种运算类似，这两种运算仍是把整数转换为二进制后进行操作。

 `num << i` 表示将 $num$ 转换为二进制后向左移动 $i$ 位所得的值。

 `num >> i` 表示将 $num$ 转换为二进制后向右移动 $i$ 位所得的值。
...
位运算的优先级低于算术运算符（除了取反，详见 [运算页面](../lang/op.md) ），所以使用时需多加注意，在必要时添加括号。
...
void getBit(int a, int b) { return (a >> b) & 1; }
```
