...
除了函数的一般表达方式外，我们还可以用 [真值表](https://en.wikipedia.org/wiki/Truth_table)（Truth table）来表示布尔函数。
...
实际上，我们只用与非或者或非即可表达其余的布尔函数，CPU 也是基于这一点构建的。由于与、或、非、异或这四种布尔函数的性质更好，所以我们一般只使用这四种函数。
...
位运算指的是对位数组或二进制数逐位应用某些布尔函数的运算。形式化地说，对布尔函数 $f:\mathbb{B}^k\to \mathbb{B}$，位运算即为形如
...
注意区分位运算与布尔函数的区别。网络中的资料中使用的符号多有不规范之处，以上下文为准。
...
取反的数学符号表示为 $\operatorname{NOT}$，其对应的运算符为 `~`。它的作用是把 $n$ 的二进制补码中的 $0$ 和 $1$ 全部取反（$0$ 变为 $1$，$1$ 变为 $0$）。有符号整数的符号位在 `~` 运算中同样会取反。
...
\begin{aligned}
5&=(00000101)_2\\
\operatorname{NOT} 5&=(11111010)_2=-6\\
-5\text{ 的补码}&=(11111011)_2\\
\operatorname{NOT} (-5)&=(00000100)_2=4
\end{aligned}
$$
...
5 &=(101)_2\\
6 &=(110)_2\\
5 \operatorname{AND} 6 &=(100)_2 =\ 4\\
5 \operatorname{OR} 6 &=(111)_2 =\ 7\\
5 \operatorname{XOR} 6 &=(011)_2 =\ 3\\
\end{aligned}
$$
...
移位为一类将位数组或二进制数「按位向左或向右移动」的二元运算，第一个参数为位数组或二进制数，第二个参数一般为非负整数。向左移动称为 **左移**，向右移动称为 **右移**。根据对移动后的空位填充方式，可将移位操作分为 **算术移位**、**逻辑移位**、**循环移位**。

`num << i` 表示将 $n$ 的二进制表示向左移动 $i$ 位所得的值。

`num >> i` 表示将 $n$ 的二进制表示向右移动 $i$ 位所得的值。
...
和 `+=`,`-=` 等运算符类似，位操作也有复合赋值运算符：`&=`,`|=`,`^=`,`<<=`,`>>=`。（取反是单目运算，所以没有。）

## 关于优先级

位操作的优先级低于算术运算符（除了取反），而按位与、按位或及异或低于比较运算符（详见 [C++ 运算符优先级总表](../lang/op.md#c-运算符优先级总表)），所以使用时需多加注意，在必要时添加括号。
