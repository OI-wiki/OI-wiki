...
根据这些定义，容易推导出 $\varphi \ast 1 = I$，$\ast$ 表示狄利克雷卷积。因为 $\dgf{\Phi}(x)\zeta(x) = \zeta(x-1)$。
...
$$
\dgf{F}(x) = \prod_{p \in \Prime} \left(1 + \sum_{k\ge 1} \frac{p^{3k-1}(p-1)}{p^{kx}} \right) = \prod_{p\in \Prime} \frac{1-p^{2-x}}{1-p^{3-x}} = \frac{\zeta(x-3)}{\zeta(x-2)}
$$

因此 $\dgf{F}(x)\zeta(x-2) = \zeta(x-3)$。而 $\zeta(x-2)$ 对应的积性函数为 $I_2$，所以令 $g = I_2$ 即可。这样有 $f\ast g = I_3$，两者都是可以快速计算前缀和的。
