...
// Find all the K-th roots of A modulo prime power.
std::vector<int> find_root_pe(int k, int a, PrimePower pp) {
...
// Find all the K-th root of A modulo positive integer M.
std::vector<int> find_root(int k, int a, int m) {
  auto factors = factorize(m);
  int m0 = 0;
  std::vector<std::vector<int>> sols;
  for (const auto& pp : factors) {
    sols.push_back(find_root_pe(k, a, pp));
  ...
  for (int i = 0; i < (int)factors.size(); ++i) {
    ...
    if (!i) {
      ...
      long long m1 = pp.pe * inverse(pp.pe, m0);
      long long m2 = m0 * inverse(m0, pp.pe);
      m0 *= pp.pe;
      std::vector<int> _ans;
