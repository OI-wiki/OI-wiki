...
-   模恒正：$|\exp z|=\exp x>0$。
-   辐角：$\operatorname{arg}(\exp z)=\{y + 2k\pi \mid k\in\mathbf Z\}$。
...
???+ note "性质"
    对于整数 $n$ 和 $k$，设 $d=(n,k)$，有 $\omega_n^k = \omega_{n/d}^{k/d}$。
...
这说明，只要 $(n,k)\neq 1$，那么，$\omega_n^k$ 就一定是 $\dfrac{n}{(n,k)}$ 次（本原）单位根。因此，满足前述性质的单位根 $\omega_n^k$ 一定是满足 $(n,k)=1$。作为这些分析的简单推论，有：

???+ note "推论"
    当 $k$ 遍历 $n$ 的因数，所有 $k$ 次本原单位根恰构成 $n$ 次单位根的一个分划。

尽管本原单位根有很多选择，但是由于第一个根 $\omega_n$ 形式最为简单，算法竞赛中还是 $\omega_n$ 最为常用。
