...
作为该过程的副产品，Lehmer 码中的所有数字的和就是排列的逆序数。相应的逆问题，即给定排名求解相应的排列，只要将上述过程反过来操作即可。

编程实现时，关键是要能够快速计算「尚未使用的元素中给定元素的排名」（求排名时）和「尚未使用的元素中给定排名的元素」（求排列时），这些都可以通过 [树状数组](../ds/fenwick.md) 或 [权值线段树](../ds/seg.md) 等数据结构维护。正反操作的时间复杂度均为 $O(n\log n)$。
