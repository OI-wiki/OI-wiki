...
??? example "示例"
    ...
    要计算排列 $\sigma=265431$ 的排名，就是要计算有多少排列的字典序小于 $\sigma$，再加一。这里的思想类似于 [数位 DP](../dp/number.md)，都是逐位讨论。

    - 第 $1$ 位的选取要小于 $\sigma$，只能取自 $\{1\}$，后面 $5$ 位可以任意选取，共 $1\times 5!$ 个可选的排列；
    - 如果第 $1$ 位也选择 $2$，那么第 $2$ 位的选取要小于 $\sigma$，只能取自 $\{1,3,4,5\}$（这里，$2$ 已经选过了），后面的 $4$ 位可以任意选取，共 $4\times 4!$ 个可选的排列；
    - 类似地，前 $2$ 位的选取和 $\sigma$ 相同时，第 $3$ 位的选取要小于 $\sigma$，只能取自 $\{1,3,4\}$，后面的 $3$ 位可以任意选取，共 $3\times 3!$ 个可选的排列；
    ...
    $$
    1+1\times 5!+4\times 4!+3\times 3!+2\times 2!+1\times 1!+0\times 0!=240.
    $$
...
相应的逆问题，即给定排名求解相应的排列，只要将上述过程反过来操作即可。作为该过程的副产品，Lehmer 码中的所有数字的和就是排列的逆序数。

编程实现时，关键是要能够快速计算「排在该位置后面却小于该位置的元素个数」（求排名时）和「尚未使用的元素中给定排名的元素」（求排列时），这些可以通过 [树状数组](../ds/fenwick.md) 维护，时间复杂度为 $O(n\log n)$。
