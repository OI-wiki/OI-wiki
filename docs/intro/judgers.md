...
此测评软件仅能在 NOI Linux 下找到，不过市面上有热心群众[提取出了Arbiter](https://github.com/leedagee/arbiter-distro)供其他操作系统使用，不过仍建议使用 NOI Linux 及相关配套编译器。
...
由于Linux运行时栈限制，如果要开无限栈，应在终端先输入`ulimit -s unlimited`后执行`arbiter`打开测评器。
