...
-    `for (int i = 0; i < strlen(s); ++i)` ：在循环中重复执行复杂度非 $O(1)$ 的函数。（严格来说，这句可能会引起时间复杂度的改变。）
...
-   将递归转化成迭代的时候，引入了大量额外运算，例如：

    -   使用了自己手写的栈或 `std::stack` 将递归转化成迭代。（如果在没有开启无限栈的情况下，为了防止爆栈 RE，可以这么做。）
