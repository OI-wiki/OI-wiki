...
除现有内容质量较低的情况外，建议尽量从 **补充** 的角度来做贡献，而非采取直接覆盖的方式。如果拿不准主意，可以参考 [F.A.Q. 的“我要怎么与项目组交流”](faq.md#我要怎么与项目组交流) 一节，与 **OI Wiki** 项目组联系。
...
-   内容：必须给出对修复问题的叙述。如果修复了某个 issue 中的问题，请写上 `fix #xxxx` ，其中 `xxxx` 是 issue 的编号。（非强制要求）推荐在叙述前删除 commit message 中的模板信息（“首先，十分感谢……”这一段）。
