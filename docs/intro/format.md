...
OI wiki 的符号书写规范，见 [符号表](./notation.md)。符号之外的一些行文规范包括：
...
相关内容在 [符号表](./notation.md) 中给出，已经向 MathJax 做了尽可能多的妥协。导出工具兼容了一部分原本仅能在 MathJax 中正常输出的写法。
