...
-   请尽量为链接提供完整的标题、或者可被识别的提示，避免使用裸地址和“这”、“此”之类的模糊不清的描述。每一个超链接都应尽量对其加以清楚明确的描述，方便读者明白该超链接将指向何处。在站内引向某一内容的链接（例如 [平衡三进制](../math/balanced-ternary.md) 中的 [转换算法](../math/balanced-ternary.md#转换算法)），请将超链接 `#` 字符后的部分写为标题文字。
