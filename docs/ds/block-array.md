...
我们要询问一个块内大于等于一个数的数的个数，所以需要一个 `t` 数组对块内排序。`a`为原来的（未被排序的）数组。对于整块的修改，使用类似于标记永久化的方式，用 `dlt` 保存现在块内整体加上的值。设q为查询和修改的操作次数总和，则时间复杂度 $O(q\sqrt{n}\log n)$
...
int Answer(int l, int r, int c) {
  ...
  for (int i = x + 1; i <= y - 1; i++)
    ans += ed[i] - (lower_bound(t + st[i], t + ed[i] + 1, c - dlt[i]) - t) + 1;//用lower_bound找出中间每一个整块中第一个大于等于c的数的位置
  return ans;
}
```
