...
类似于 WBL（weight-balanced trees，加权平衡树），WBLT 体现了 leafy 的性质，即结点多，怎么多呢？

对于 n 个数，不同于 treap 等，WBLT 会建立 2n 个结点，每个结点的权值为其右儿子的权值，且右儿子的权值大于等于左儿子
...
而在旋转的过程中，会产生很多垃圾结点，我们采用垃圾回收的方式就可以回收废弃结点，将建立结点的操作稍作修改即可。
