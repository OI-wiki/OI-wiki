...
-   左子结点的权值（$\textit{val}$）比父结点小。
-   右子结点的权值（$\textit{val}$）比父结点大。

堆的性质是：

-   子结点优先级（$\textit{priority}$）比父结点大或小（取决于是小根堆还是大根堆）。
...
下图就是一个 Treap 的例子（这里使用的是小根堆，即根结点的优先级最小）。

![一个 Treap 的例子](./images/treap-treap-example.svg)

那我们为什么需要大费周章的去让这个数据结构符合树和堆的性质，并且随机给出堆的值呢？

要理解这个，首先需要理解朴素二叉搜索树的问题。在给朴素搜索树插入一个新结点时，我们需要从这个搜索树的根结点开始递归，如果新结点比当前结点小，那就向左递归，反之亦然。

最后当发现当前结点没有子结点时，就根据新结点的值的大小，让新结点成为当前结点的左或右子结点。

如果插入结点的权值是随机的（换言之，是随机插入的），那这个朴素搜索树的高度较小（接近 $\log n$，其中 $n$ 为结点数），而每一层的结点数较多，即它的形状会非常的「胖」。上图的 Treap 就是一个例子。因此此时的任意操作复杂度都将会是 $O(\log n)$ 左右。

不过，这只是在随机情况下的复杂度，如果我们按照下面这个非常有序的顺序给一个朴素的搜索树插入结点：

```plain
1 2 3 4 5
```

那么这棵树将会退化成链，即变得非常「瘦长」（每次插入的结点都比前面的大，所以都被安排到右子结点了）：

![退化成链的例子](./images/treap-search-tree-chain.svg)

不难看出，查询的复杂度也从 $O(\log n)$ 变成了 $O(n)$.

而 treap 为了解决这个问题、达到一个较为「平衡」的状态，通过维护随机的优先级满足堆性质，「打乱」了结点的插入顺序，从而让二叉搜索树达到了理想的复杂度，避免了退化成链的问题。
...
-   $\operatorname{dep}(x)$ 表示结点 $x$ 的深度。规定根结点的深度是 $0$.
...
??? note "引理的证明"
    **证明**：考虑分类讨论 $x_i$ 和 $x_j$ 的情况。
    ...
    1.  若 $x_i$ 是根结点：由于优先级满足小根堆性质，$x_i$ 的优先级最小，并且对于任意的 $x_j$，$x_i$ 都是 $x_j$ 的祖先。
    2.  若 $x_j$ 是根结点：同理，$x_j$ 优先级最小，因此 $x_i$ 不是 $X_{i,j}$ 中优先级最小的；同时 $x_i$ 也不是 $x_j$ 的祖先。
    3.  若 $x_i$ 和 $x_j$ 在根结点的两个子树中（一左一右），那么根结点 $r\in X_{i,j}$. 因此 $x_i$ 的优先级不可能是 $X_{i,j}$ 中最小的（因为根结点的比它小）。同时，由于 $x_i$ 和 $x_j$ 分属两个子树，$x_i$ 也不是 $x_j$ 的祖先。
    4.  若 $x_i$ 和 $x_j$ 在根结点的同一个子树中，此时可以将这个子树单独拿出来作为一棵新的 treap，递归进行上面的证明即可。$\square$
...
???+ note "期望复杂度的感性理解"
    首先，我们需要认识到一个结点的 $\textit{priority}$ 属性是和它所在的层数有直接关联的。再回忆堆的性质：
    
    -   子结点值（$\textit{priority}$）比父结点大或小（取决于是小根堆还是大根堆）
    
    我们发现层数低的结点，比如整个树的根结点，它的 $\textit{priority}$ 属性也会更小（在小根堆中）。并且，在朴素的搜索树中，先被插入的结点，也更有可能会有比较小的层数。我们可以把这个 $\textit{priority}$ 属性和被插入的顺序关联起来理解，这样，也就理解了为什么 treap 可以把结点插入的顺序通过 $\textit{priority}$ 打乱。

给 treap 插入新结点时，需要同时维护树和堆的性质。其中，搜索树的性质可以在插入时维护，而堆性质的维护则有两种处理方法，分别是旋转和分裂、合并。使用这两种方法的 treap 被分别称为 **旋转 treap** 和 **无旋 treap**。
...
旋转操作是 treap 的一个非常重要的操作，主要用来在保持 treap 树性质的同时，调整不同结点的层数，以达到维护堆性质的作用。

旋转操作的左旋和右旋可能不是特别容易区分，以下是两个较为明显的特点：

旋转操作的含义：

-   在不影响搜索树性质的前提下，把和旋转方向相反的子树变成根结点（如左旋，就是把右子树变成根结点）
-   不影响性质，并且在旋转过后，跟旋转方向相同的子结点变成了原来的根结点（如左旋，旋转完之后的左子结点是旋转前的根结点）
...
void _rotate(Node *&cur,
  ...
  // cur，变量是跟着一起改的，如果这个 cur 是别的 树的子结点，根据 ch
  // 找过来的时候，也是会找到这里的

  // 以下的代码解释的均是左旋时的情况
  Node *tmp = cur->ch[dir];  // 让 C 变成根结点，
                             // 这里的 tmp
                             // 是一个临时的结点指针，指向成为新的根结点的结点
   ...
   *        D   E          B   D
   */
  cur->ch[dir] = tmp->ch[!dir];    // 让 A 的右子结点变成 D
  tmp->ch[!dir] = cur;             // 让 C 的左子结点变成 A
  cur->upd_siz(), tmp->upd_siz();  // 更新大小信息
  cur = tmp;  // 最后把临时储存 C 树的变量赋值到当前根结点上（注意 cur 是引用）
}
```
...
void _insert(Node *&cur, int val) {
  if (cur == nullptr) {
    ...
    // 如果有这个值相同的结点，就把重复数量加一
    cur->rep_cnt++;
    cur->siz++;
  } else if (val < cur->val) {
    // 维护搜索树性质，val 比当前结点小就插到左边，反之亦然
    _insert(cur->ch[0], val);
    if (cur->ch[0]->rank < cur->rank) {
      // 小根堆中，上面结点的优先级一定更小
      // 因为新插的左子结点比父结点小，现在需要让左子结点变成父结点
      _rotate(cur, RT);  // 注意前面的旋转性质，要把左子结点转上来，需要右旋
    }
...
主要就是分类讨论，不同的情况有不同的处理方法，删完了树的大小会有变化，要注意更新。并且如果要删的结点有左子树和右子树，就要考虑删除之后让谁来当父结点（维护 rank 小的结点在上面）。
  ...
  if (val > cur->val) {
    ...
    switch (state) {
      ...
      case 3:
        ...
        _del(
            cur->ch[!dir],
            val);  // 旋转完成后原来的根结点就在旋方向那边，所以需要
                   // 继续把这个原来的根结点删掉
                   // 如果说要删的这个结点是在整个树的「上层的」，那我们会一直通过这
...
操作含义：查询以 cur 为根结点的子树中，val 这个值的大小的排名（该子树中小于 val 的结点的个数 + 1）
...
int _query_rank(Node *cur, int val) {
  ...
  if (val == cur->val)
    ...
    if (cur->ch[0] != nullptr)
      ...
      return 1;  // 如果左子树是空的，说比最小的结点还要小，那这个数字就是最小的
  } else {
    if (cur->ch[1] != nullptr)
      // 如果要查的值比这个结点大，那这个结点的左子树以及这个结点自身肯定都比要查的值小
...
要根据排名查询值，我们首先要知道如何判断要查的结点在树的哪个部分：

以下是一个判断方法的表：

| 左子树         | 根结点/当前结点                           | 右子树                    |
| ----------- | ---------------------------------- | ---------------------- |
| 排名 ≤ 左子树的大小 | 排名 > 左子树的大小，并且 ≤ 左子树的大小 + 根结点的重复次数 | 排名 > 左子树的大小 + 根结点的重复次数 |

注意如果在右子树，递归的时候需要对原来的 `rank` 进行处理。递归的时候就相当去查，在右子树中为这个排名的值，为了把排名转换成基于右子树的，需要把原来的 `rank` 减去左子树的大小和根结点的重复次数。

可以把所有结点想象成一个排好序的数组，或者数轴（如下），

    1 -> |左子树的结点|根结点|右子树的结点| -> n
                               ^
                               要查的排名
                         ⬇转换成基于右子树的排名
    1 -> |右子树的结点| -> n
...
这里的转换方法就是直接把排名减去左子树的大小和根结点的重复数量。
...
这个值是只有在 val 比当前结点值大的时候才会被更改的，所以返回这个变量就是返回 val 最后一次比当前结点的值大，之后就是更小了。
...
int _query_prev(Node *cur, int val) {
  if (val <= cur->val) {
    ...
    q_prev_tmp = cur->val;
    // 当前结点已经比 val，小了，但是不确定是否是最大的，所以要到右子树继续找
...
该过程首先判断 $\textit{key}$ 是否小于 $\textit{cur}$ 的值，若小于，则说明 $\textit{cur}$ 及其右子树全部大于 $\textit{key}$，属于第二个 treap。当然，也可能有一部分的左子树的值大于 $\textit{key}$，所以还需要继续向左子树递归地分裂。对于大于 $\textit{key}$ 的那部分左子树，我们把它作为 $\textit{cur}$ 的左子树，这样，整个 $\textit{cur}$ 上的结点都是大于 $\textit{key}$ 的。

相应的，如果 $\textit{key}$ 大于等于 $\textit{cur}$ 的值，说明 $\textit{cur}$ 的整个左子树以及其自身都小于等于 $\textit{key}$，属于分裂后的第一个 treap。并且，$\textit{cur}$ 的部分右子树也可能有部分小于等于 $\textit{key}$，因此我们需要继续递归地分裂右子树。把小于等于 $\textit{key}$ 的那部分作为 $\textit{cur}$ 的右子树，这样，整个 $\textit{cur}$ 上的结点都小于等于 $\textit{key}$。
...
比起按值分裂，这个操作更像是旋转 treap 中的根据排名（某个结点的排名是树中所有小于此结点值的结点的数量 $+ 1$）查询值：

此函数接受两个参数，结点指针 $\textit{cur}$ 和排名 $\textit{rk}$，返回分裂后的三个 treap。

其中，第一个 treap 中每个结点的排名都小于 $\textit{rk}$，第二个的排名等于 $\textit{rk}$，并且第二个 treap 只有一个结点（不可能有多个等于的，如果有的话会增加 `Node` 结构体中的 `cnt`），第三个则是大于。

此操作的重点在于判断排名和 $\textit{cur}$ 相等的结点在树的哪个部分，这也是旋转 treap 根据排名查询值操作时的重要部分，在前文有非常详细的解释，这里不过多讲解。
...
tuple<Node *, Node *, Node *> split_by_rk(Node *cur, int rk) {
  ...
  if (rk <= ls_siz) {
    ...
    // cur 的左子树被设成 r 后，整个 cur 中结点的排名都大于 rk
    cur->upd_siz();
    return {l, mid, cur};
    ...
    // 分裂后第二个 treap 只有一个结点，所有要把它的子树设置为空
    return {lt, cur, rt};
  } else {
    // 排名和 cur 相等的结点在右子树
    // 递归过程同上
...
合并过程接受两个参数：左 treap 的根指针 $\textit{u}$、右 treap 的根指针 $\textit{v}$。必须满足 $\textit{u}$ 中所有结点的值小于等于 $\textit{v}$ 中所有结点的值。一般来说，我们合并的两个 treap 都是原来从一个 treap 中分裂出去的，所以不难满足 $\textit{u}$ 中所有结点的值都小于 $\textit{v}$
...
在实现插入操作时，我们利用了分裂操作的一些性质。也就是值小于等于 $\textit{val}$ 的结点会被分到第一个 treap。
...
其中 $T_1$ 表示分裂后所有被分到第一个 treap 的结点的集合，$T_2$ 则是第二个。
...
其中 $T_{1\ \text{left}}$ 表示 $T_1$ 分裂后所有被分到第一个 treap 的结点的集合，$T_{1\ \text{right}}$ 则是第二个。并且上面的式子中，后半部分的 $\And \ T_{1\ \text{right}} \le val$ 来自于 $T_1$ 所符合的条件 $T_1 \le val$。

不难发现，只要 $\textit{val}$ 和结点的值是一个整数（大多数使用场景下会使用整数）那么符合 $T_{1\ \text{right}}$ 条件的结点只有一个，也就是值等于 $\textit{val}$ 的结点。

在插入时，如果我们发现符合 $T_{1\ \text{right}}$ 的结点存在，那就可以直接增加重复次数，否则，就新开一个结点。

注意把树分裂好了还需要用合并操作把它「粘」回去，这样下次还能继续使用。并且，还需要注意合并操作的参数顺序是有要求的，第一个树的所有结点的值都需要小于第二个。
...
void insert(int val) {
  ...
  // l_tr 的左子树 <= val - 1，如果有 = val 的结点，那一定在右子树
  Node *new_node;
  if (l_tr.second == nullptr) {
    // 没有这个结点就新开，否则直接增加重复次数。
...
删除操作也使用和插入操作相似的方法，找到值和 $\textit{val}$ 相等的结点，并且删除它。
...
排名是比这个值小的结点的数量 $+ 1$，所以我们根据 $\textit{val} - 1$ 分裂当前树，那么分裂后的第一个树就符合：

$$
T_1 \le val - 1
$$

如果树的值和 $\textit{val}$ 为整数，那么 $T_1$ 就包含了所有值小于 $\textit{val}$ 的结点。
...
调用 `split_by_rk()` 函数后，会返回分裂好的三个 treap，其中第二个只包含一个结点，它的排名等于 $\textit{rk}$，所以我们直接返回这个结点的 $\textit{val}$。
...
可以把这个问题转化为，在比 $\textit{val}$ 小的所有结点中，找出排名最大的。我们根据 $\textit{val}$ 来分裂这个 treap，返回的第一个 treap 中的结点的值就全部小于 $\textit{val}$，然后我们调用 `qval_by_rank()` 找出这个树中值最大的结点。
...
和上个操作类似，可以把这个问题转化为，在比 $\textit{val}$ 大的所有结点中，找出排名最小的。那么根据 $\textit{val}$ 分裂后，返回的第二个 treap 中的所有结点的值就大于 $\textit{val}$。

然后我们去查询这个树中排名为 $1$ 的结点（也就是值最小的结点）的值，就可以成功查到第一个比 $\textit{val}$ 大的结点。
...
将一个有 $n$ 个结点的序列 $\{a_n\}$ 转化为一棵 treap。

可以依次暴力插入这 $n$ 个结点，每次插入一个权值为 $v$ 的结点时，将整棵 treap 按照权值分裂成权值小于等于 $v$ 的和权值大于 $v$ 的两部分，然后新建一个权值为 $v$ 的结点，将两部分和新结点按从小到大的顺序依次合并，单次插入时间复杂度 $O(\log n)$，总时间复杂度 $O(n\log n)$。

在某些题目内，可能会有多次插入一段有序序列的操作，这是就需要在 $O(n)$ 的时间复杂度内完成建树操作。

方法一：在递归建树的过程中，每次选取当前区间的中点作为该区间的树根，并对每个结点钦定合适的优先值，使得新树满足堆的性质。这样能保证树高为 $O(\log n)$。

方法二：在递归建树的过程中，每次选取当前区间的中点作为该区间的树根，然后给每个结点一个随机优先级。这样能保证树高为 $O(\log n)$，但不保证其满足堆的性质。这样也是正确的，因为无旋式 treap 的优先级是用来使 `merge` 操作更加随机一点，而不是用来保证树高的。
...
我们只需要把区间的下标依次插入 treap 中，这样在中序遍历（先遍历左子树，然后当前结点，最后右子树）时，就可以得到这个区间[^ref3]。

我们知道在朴素的二叉查找树中按照递增的顺序插入结点，建出来的树是一个长链，按照中序遍历，自然可以得到这个区间。

<div align=center>
  <img style="width: 50%; " src="../images/treap-search-tree-chain.svg" >
</div>

如上图，按照 $1\ 2\ 3\ 4\ 5$ 的顺序给朴素搜索树插入结点，中序遍历时，得到的也是 $1\ 2\ 3\ 4\ 5$。

但是在 treap 中，按增序插入结点后，在合并操作时还会根据 $\textit{priority}$ 调整树的结构，在这样的情况下，如何确保中序遍历一定能正确的输出呢？

可以参考 [笛卡尔树的单调栈建树方法](./cartesian-tree.md) 来理解这个问题。

设新插入的结点为 $\textit{u}$。

首先，因为是递增地插入结点，每一个新插入的结点肯定会被连接到 treap 的右链（即从根结点一直往右子树走，经过的结点形成的链）上。

从根结点开始，右链上的结点的 $\textit{priority}$ 是递增的（小根堆）。那我们可以找到右链上第一个 $\textit{priority}$ 大于 $\textit{u}$ 的结点，我们叫这个结点 $\textit{v}$，并把这个结点换成 $\textit{u}$。

因为 $\textit{u}$ 一定大于这个树上其他的全部结点，我们需要把 $\textit{v}$ 以及它的子树作为 $\textit{u}$ 的左子树。并且此时 $\textit{u}$ 没有右子树。

可以发现，中序遍历时 $\textit{u}$ 一定是最后一个被遍历到的（因为 $\textit{u}$ 是右链中的最后一个，而中序遍历中，右子树是最后被遍历到的）。

下图是一个 treap 根据递增顺序插入 $1 \sim 5$ 号结点时，插入 $5$ 号结点时的变化，可以用这张图更好的理解按照增序插入的过程。
...
翻转的具体操作是把区间内的子树的每一个左，右子结点交换位置。如下图就展示了翻转上图中 treap 的 $[3, 4]$ 和 $[3, 5]$ 区间后的 treap。

![区间翻转](./images/treap-none-rot-seg-flip-ex.svg)

注意如果按照这个方法翻转，那么每次翻转 $[l, r]$ 区间时，就会有 $r - l$ 个结点会被交换位置，这样频繁的操作显然不能满足 $10^5$ 的数据范围，其 $O(n \times \log_2 n)$ 的单次翻转复杂度甚至不如暴力（因为我们除了需要花线性时间交换结点外，还需要在树中花费 $O(\log_2 n)$ 的时间找到需要交换的结点）。

再观察题目要求，可以发现因为只需要最后输出操作完的区间，所以并不需要每次都真的去交换。如此一来，便可以使用线段树中常用的懒标记（lazy tag）来优化复杂度。交换时，只需要在父结点打上标记，代表这个子树下的每个左右子结点都需要交换就行了。

在线段树中，我们一般在更新和查询时下传懒标记。这是因为，在更新和查询时，我们想要更新/查询的范围不一定和懒标记代表的范围重合，所以要先下传标记，确保查到和更新后的值是正确的。

在无旋 treap 中也是一样。具体操作时我们会把 treap 分裂成前文讲到的三个树，然后给中间的树打上懒标记后合并这三棵树。因为我们想要翻转的区间和懒标记代表的区间不一定重合，所以要在分裂时下传标记。并且，分裂和合并操作会造成每个结点及其懒标记所代表的结点发生变动，所以也需要在合并前下传懒标记。

换句话说，是当树的结构发生改变的时候，当我们进行分裂或合并操作时需要改变某一个点的左右儿子信息时之前，应该下放标记，而非之后，因为懒标记是需要下传给儿子结点的，但更改左右儿子信息之后若懒标记还未下放，则懒标记就丢失了下放的对象。[^ref4]
...
需要注意这里的懒标记代表需要把这个树中的每一个子结点交换位置。所以如果当前结点的子结点也有懒标记，那两次翻转就抵消了。如果子结点不需要翻转，那么这个懒标记就需要继续被下传到子结点上。
...
所以这里的分裂跟普通无旋 treap 中的按排名分裂更相似，是根据当前树的大小判断往左还是右子树递归的，换言之，我们是按照开始时这个结点在树中的位置来判断的。

返回的第一个 treap 中结点的排名全部小于等于 $\textit{sz}$，而第二个 treap 中结点的排名则全部大于 $\textit{sz}$。
...
??? note "完整代码"
    ...
    struct Node {
      ...
      int cnt;
      int siz;
      bool to_rev = false;  // 需要把这个子树下的每一个结点都翻转过来
      ...
      void pushdown() {
        swap(ch[0], ch[1]);
        if (ch[0] != nullptr) ch[0]->to_rev ^= 1;
        // 如果原来子结点也要翻转，那两次翻转就抵消了，如果子结点不翻转，那这个
        //  tag 就需要继续被 push 到子结点上
        if (ch[1] != nullptr) ch[1]->to_rev ^= 1;
        to_rev = false;
      }
