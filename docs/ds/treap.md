treap 是一种弱平衡的二叉搜索树。treap 这个单词是 tree 和 heap 的组合，表明 treap 是一种由树和堆组合形成的数据结构。treap 的每个结点上要额外储存一个值 $priority$ 。treap 除了要满足二叉搜索树的性质之外，还需满足父结点的 $priority$ 大于等于两个儿子的 $priority$ 。而 $priority$ 是每个结点建立时随机生成的，因此 treap 是期望平衡的。
...
可以依次暴力插入这 $n$ 个结点，每次插入一个权值为 $v$ 的结点时，将整棵 treap 按照权值分裂成权值小于等于 $v$ 的和权值大于 $v$ 的两部分，然后新建一个权值为 $v$ 的结点，将两部分和新结点按从小到大的顺序依次合并，时间复杂度 $O(n\log_2 n)$ ，单点插入时间复杂度 $O(\log_2 n)$ 。
...
在某些题目内，可能会有多次插入一段序列的操作，这是就需要在 $O(n)$ 的时间复杂度内完成建树操作。（假设这些结点已经有序）

方法一：直接将这 $n$ 个结点构造成一棵类线段树，即每次选取最中间的结点作为一段区间的树根，这样能保证树高为 $O(\log_2 n)$ 。然后对每个结点钦定优先值，保证其满足堆的性质。

方法二：直接将这 $n$ 个结点构造成一棵类线段树，即每次选取最中间的结点作为一段区间的树根，这样能保证树高为 $O(\log_2 n)$ 。然后给每个结点一个随机优先级，不保证其满足堆的性质。因为非旋式 treap 的优先级不是维护堆的性质，保证树高的，而是使 `merge` 操作更加随机一点，所以也是正确的。
...
因为普通的二叉搜索树会被递增或递减的数据卡，用 treap 对每个结点定义一个权值，由 rand 得到，从而防止特殊数据卡。
