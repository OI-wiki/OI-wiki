...
（例如下图，修改了 $[1,8]$ 中对应权值为 1 的节点，红色的点即为更改的点）  
![](./images/persistent-seg.png)

只更改了 $O(\log{n})$ 个节点，形成一条链，也就是说每次更改的节点数 = 树的高度。  
...
关于空间问题，我们分析一下：由于我们是动态开点的，所以一棵线段树只会出现 $2n-1$ 个节点。  
然后，有 $n$ 次修改，每次增加 $\log{n}$ 个节点。那么最坏情况节点数会达到 $2n-1+n\log{n}$ ，那么此题的 $n \leq 10^5$ ，通过计算得到 $\lceil\log_2{10^5}\rceil = 17$ 。  
...
（较真的同学请注意，如果你真的很吝啬，可以自己造个数据输出一下节点数量，但是如果数据没造好把自己卡掉了就~~尴尬了~~赖你了）
