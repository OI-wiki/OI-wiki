...
并查集是一种用于管理元素所属集合的数据结构，实现为一个森林，其中每棵树表示一个集合，树中的结点表示对应集合中的元素。
...
合并时，选择哪棵树的根结点作为新树的根结点会影响未来操作的复杂度。我们可以将结点较少或深度较小的树连到另一棵，以免发生退化。
...
普通的并查集无法支持删除操作，是因为删除一个结点的时候，不可避免地会将以它为根的子树上所有结点都删除。为了解决这一问题，在带删除操作的并查集中，可以通过建立虚点的方法保证所有实际存储数据的结点总是叶子结点。为此，需要在初始化时，就为每个数据结点都建立一个虚点，并将数据结点的父结点设置为该虚点。由于每次合并两个集合时，都只会将两个集合的树根连接，所以，从始至终只有虚点会有子结点。这就保证了删除一个结点时，不会误删其他结点。

注意，删除单个结点后，需要重新为该结点建立一个虚点作为其父结点；否则，无法正确执行后续的合并和删除操作。
...
为了维护并查集中的边权，需要将边权下放到子结点中存储。因此，每个结点存储的都是它到它的父结点之间的边权。只有当一个结点的父结点发生变化时，才需要相应地调整边权。一般情形中，这可能发生在路径压缩和合并两个结点时。例如，如果边权是当前结点与父结点之间的距离，那么，在路径压缩时，每次将当前结点的父结点替换为根结点，都需要将父结点到根结点的距离加到当前结点存储的边权上；类似地，在合并两个结点所在集合时，需要计算两个根结点之间新连接的边的权值。
...
??? note "解答"
    这道题目中，操作 1 和操作 3 都容易处理，难点在于操作 2。假定要将元素 $x$ 移动到元素 $y$ 所在的集合。在普通的并查集中，直接将元素 $x$ 的父亲设为元素 $y$ 所在集合的根结点是不行的，因为这样会将元素 $x$ 所在子树的元素都一起移动。针对这个问题，解决方法就是保证元素 $x$ 没有子结点。为此，在建立并查集时为每个元素 $x$ 都建立一个虚点 $\tilde x$，并将元素 $x$ 的父亲指向对应的虚点 $\tilde x$。这样，在合并两个集合的时候，因为总是将一个树根连接到另一个树根，而树根又全部是虚点，所以，只有虚点会有子结点，而所有实际存储元素的点都没有子结点。此时，要移动元素，就容易实现得多。
...
??? note "解答一"
    ...
    具体地，对于每一句话，除去那些那些 $x>n$ 或 $y>n$ 的显然的假话外，需要判断 $x$ 和 $y$ 是否已经连接：如果已经连接，计算两者的模意义下的距离，并与这句话声称的信息进行比较；否则，将两者按照这句话提供的信息连接。除了显然的情形外，一句话是假话，当且仅当提到的两个结点已经连接，且对应的距离与这句话声称的信息矛盾。
