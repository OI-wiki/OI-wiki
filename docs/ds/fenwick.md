...
怎么计算 `lowbit`？根据位运算知识，可以得到 `lowbit(x) = x & -x`。原理并不重要，无需了解，如果好奇，可以自行在网上查找相关资料。
...
观察上面的过程，每次往前跳，一定是跳到现区间的左端点的左一位，作为新区间的右端点，这样才能将前缀不重不漏地拆分。比如上次 $c_6$ 管的是 $a[5 \cdots 6]$，下一次肯定得跳到 $5 - 1 = 4$，也就是去访问 $c_4$。
