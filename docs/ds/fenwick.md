...
除了传统的基于 $lowbit$ 函数的树状数组，还有一种天然支持 0 下标，细节略有偏差的树状数组，通过使用 $i|=i+1$ 和 $i&=i-1$ 来替代 $+=lowbit()$ 与 $-=lowbit()$ 实现 0 下标的无缝访问．实际上核心思路并没有任何区别，两种代码都基于 C++ 模板实现，均支持 0 下标，传统的树状数组通过 pos++ 来支持．
