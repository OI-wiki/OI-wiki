...
那么怎么做区间加呢？考虑给原数组 $a[l \ldots r]$ 区间加 $x$ 给 $d$ 带来的影响。发现只有 $d[l]$ 的值多了 $v$，$d[r + 1]$ 的值少了 $v$。证明平凡。
...
原因十分自然：出现在 $b[1 \ldots x-1]$ 中的元素一定比当前的 $a[i]$ 小，而 $i$ 的倒序枚举致使这些已经在权值数组中的元素的索引大于 $i$。
