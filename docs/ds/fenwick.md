...
- 设二进制最低位为第 $0$ 位，则 $n$ 恰好为 $x$ 二进制表示中，最低位的 `1` 所在的二进制位数；
...
**这里注意：$\boldsymbol{\operatorname{lowbit}}$ 指的不是最低位 `1` 所在的位数 $\boldsymbol{n}$，而是这个 `1` 和后面所有 `0` 组成的 $\boldsymbol{2^n}$。**
...
有且只有 $c[p(0)], c[p(1)], c[p(2)], \cdots$ 覆盖 $a_x$。
