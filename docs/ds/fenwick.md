...
除了传统的基于$lowbit$函数的树状数组，还有一种天然支持0下标，细节略有偏差的树状数组，通过使用$i|=i+1$和$i&=i-1$来替代$+=lowbit()$与$-=lowbit()$实现0下标的无缝访问。实际上核心思路并没有任何区别，两种代码都基于C++模板实现，均支持0下标，传统的树状数组通过pos++来支持。
...
???+ note "实现"
    === "支持0下标"
        ...
        uint n;
        vector<T> s;
        explicit FenWickTree(int n) : n(n), s(n + 1) {}
        void update(uint pos, T dif) { // a[pos] += dif
            for (; pos < s.size(); pos |= pos + 1) {
                s[pos] += dif;
            }
        }
        T query(uint pos) { // sum of values in [0, pos)
            T res = 0;
        ...
        T query2(uint l, uint r) { // sum of values in [l, r)
    ...
    === "传统"
        ```cpp
        template <typename T> constexpr static T lowbit(T x) {
            return x & -x;
        }
