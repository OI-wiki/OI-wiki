...
struct node
{
    ...
    i64 max_val() { return std::max({mx_ab[0][0], mx_ab[0][1], mx_ab[1][0], mx_ab[1][1]}); }
    i64 mx1[2], mx2[2]; // 0 : A 1 : B
    i64 tag_add[2], tag_mn[2];
    inline void clr_tag() { tag_add[0] = tag_add[1] = 0, tag_mn[0] = tag_mn[1] = INF; }
    ...
    inline void modify_mn(i64 tg, bool id) // id 0 A 1 B
    {
        ...
        mx_ab[id == 0][id == 1] += (mx_ab[id == 0][id == 1] > -INF) ? (tg - mx1[id]) : 0;
        mx1[id] = tag_mn[id] = tg;
    }
    ...
    inline void pushup(const node& l, const node& r)
    {
        ...
        else
        {
            ...
            mx_ab[0][0] = std::max({l.mx_ab[0][0], l.mx_ab[0][1], l.mx_ab[1][0], l.mx_ab[1][1]});
        }
        ...
        else if (r.mx1[0] != mx1[0] && r.mx1[1] == mx1[1])
        {
            ...
            mx_ab[0][0] = std::max({mx_ab[0][0], r.mx_ab[0][0], r.mx_ab[0][1], r.mx_ab[1][0], r.mx_ab[1][1]});
    }
} tr[N << 2];
...
inline void pushdown(int u)
{
    for (int i = 0; i < 2; ++i)
        if (tr[u].tag_add[i])
            tr[lc(u)].modify_add(tr[u].tag_add[i], i), tr[rc(u)].modify_add(tr[u].tag_add[i], i);
    for (int i = 0; i < 2; ++i)
        if (tr[u].tag_mn[i] < INF)
            tr[lc(u)].modify_mn(tr[u].tag_mn[i], i), tr[rc(u)].modify_mn(tr[u].tag_mn[i], i);
    tr[u].clr_tag();
}
...
{
    if (L > r || R < l || tr[u].mx1[i] <= v)
        return;
    if (l <= L && R <= r && tr[u].mx2[i] < v)
        return tr[u].modify_mn(v, i);
    pushdown(u);
    int M = (L + R) >> 1;
    _modify_mn(lc(u), l, r, L, M, v, i), _modify_mn(rc(u), l, r, M + 1, R, v, i);
    pushup(u);
}
inline void modify_mn(int l, int r, int v, bool i) { _modify_mn(1, l, r, 1, n, v, i); }
inline void _modify_add(int u, int l, int r, int L, int R, int v, bool i)
{
    if (L > r || R < l)
        return;
    if (l <= L && R <= r)
        return tr[u].modify_add(v, i);
    pushdown(u);
    int M = (L + R) >> 1;
    _modify_add(lc(u), l, r, L, M, v, i), _modify_add(rc(u), l, r, M + 1, R, v, i);
    pushup(u);
}
inline void modify_add(int l, int r, int v, bool i) { _modify_add(1, l, r, 1, n, v, i); }
...
inline node _query(int u, int l, int r, int L, int R)
{
    ...
    int M = (L + R) >> 1;
    if (r <= M)
        return _query(lc(u), l, r, L, M);
    if (l > M)
        return _query(rc(u), l, r, M + 1, R);
    node ret;
