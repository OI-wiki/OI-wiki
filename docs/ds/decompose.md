...
设序列长度为 $n$，分块长度为 $b$，序列被分为 $\frac{n}{b}$ 块，有 $q$ 次询问。操作区间 $[L,R]$ 被分为以 $L$ 起始的后缀碎块、中间若干个整块、以 $R$ 结尾的前缀碎块三部分。

-   **预处理**：双指针遍历所有区块，预处理区块 $i$ 到区块 $j(i<j)$ 的最值。复杂度为 $O((\frac{n}{b})^2)$。
-   **各块前后缀最值**：枚举每个块，分别计算出每个块内每个元素的前缀和后缀元素最值。复杂度 $O(n)$。
-   **处理询问**：若询问 $[L,R]$ 与多个区块有交，复杂度为 $O(1)$；否则复杂度为 $O(b)$。复杂度 $O(q\times b)$。
-   **确定分块长度**：以上步骤整体复杂度为 $O((\frac{n}{b})^2+q\times b)$。根据基本不等式可知 $(\frac{n}{b})^2+q\times b\ge2\sqrt{\frac{n^2\times q}{b}}$，当 $(\frac{n}{b})^2=q\times b$ 时复杂度取最小值，即 $b=\frac{\sqrt[3]{n^2}}{\sqrt[3]{q}}$，复杂度 $O(\sqrt[3]{(n\times q)^2})$。
