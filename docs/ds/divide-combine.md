...
在图中我们没有标明本原段。而图中 **每个节点都代表一个本原段** 。我们只标明了每个本原段的值域。举个例子，节点 $[5,8]$ 代表的本原段就是 $(P,[6,9])=\{5,7,6,8,4\}$ 。于是这里就有一个问题： **什么是析点合点？** 

### 析点与合点

这里我们直接给出定义，稍候再来讨论它的正确性。

1.   **值域区间** ：对于一个节点 $u$ ，用 $[u_l,u_r]$ 表示该节点的值域区间。
2.   **儿子序列** ：对于析合树上的一个节点 $u$ ，假设它的儿子节点是一个 **有序** 序列，该序列是以值域区间为元素的（单个的数 $x$ 可以理解为 $[x,x]$ 的区间）。我们把这个序列称为儿子序列。记作 $S_u$ 。
3.   **儿子排列** ：对于一个儿子序列 $S_u$ ，把它的元素离散化成正整数后形成的排列称为儿子排列。举个例子，对于节点 $[5,8]$ ，它的儿子序列为 $\{[5,5],[6,7],[8,8]\}$ ，那么把区间排序标个号，则它的儿子排列就为 $\{1,2,3\}$ ；类似的，节点 $[4,8]$ 的儿子排列为 $\{2,1\}$ 。节点 $u$ 的儿子排列记为 $P_u$ 。
4.   **合点** ：我们认为，儿子排列为顺序或者逆序的点为合点。形式化地说，满足 $P_u=\{1,2,\cdots,|S_u|\}$ 或者 $P_u=\{|S_u|,|S_u-1|,\cdots,1\}$ 的点称为合点。 **叶子节点没有儿子排列，我们也认为它是合点** 。
5.   **析点** ：不是合点的就是析点。
...
析点与合点的命名来源于他们的性质。首先我们有一个非常显然的性质：对于析合树中任何的节点 $u$ ，其儿子序列区间的并集就是节点 $u$ 的值域区间。即 $\bigcup_{i=1}^{|S_u|}S_u[i]=[u_l,u_r]$ 。
...
我们考虑增量法。用一个栈维护前 $i-1$ 个元素构成的析合森林。在这里我需要 **着重强调** ，析合森林的意思是，在任何时侯，栈中节点要么是析点要么是合点。现在考虑当前节点 $P_i$ 。

1.  我们先判断它能否成为栈顶节点的儿子，如果能就变成栈顶的儿子，然后把栈顶取出，作为当前节点。重复上述过程直到栈空或者不能成为栈顶节点的儿子。
2.  如果不能成为栈顶的儿子，就看能不能把栈顶的若干个连续的节点都合并成一个节点（判断能否合并的方法在后面），把合并后的点，作为当前节点。
3.  重复上述过程直到不能进行为止。然后结束此次增量，直接把当前节点圧栈。

接下来我们仔细解释一下。

#### 具体的策略

我们认为，如果当前点能够成为栈顶节点的儿子，那么栈顶节点是一个合点。如果是析点，那么你合并后这个析点就存在一个子连续段，不满足析点的性质。因此一定是合点。

如果无法成为栈顶节点的儿子，那么我们就看栈顶连续的若干个点能否与当前点一起合并。我们预处理一个数组 $L$ ， $L_i$ 表示右端点下标为 $i$ 的连续段中，左端点的最小值。当前节点为 $P_i$ ，栈顶节点记为 $t$ 。

1.  如果 $t_l<L_i$ 那么显然当前节点无法合并；
2.  如果 $t_l=L$ ，那么这就是两个节点合并，合并后就是一个 **合点** ；
3.  如果在栈中存在一个点 $t'$ 的左端点 ${t'}_l=L_i$ ，那么一定可以从当前节点合并到 $t’$ 形成一个 **析点** ；
4.  否则，我们找到栈中的一个点 $t'$ 使得 ${t'}_l<L_i\le {t'}_r$ 。由连续段的差运算可知 $(P,[{t'}_r+1,i])$ 也是连续段，于是合并 $t'$ 之后的节点到当前节点成一个 **析点** 即可。
  ...
  for (int i = 1; i <= n; ++i) {
    ...
    while (tp && L[st[tp]] >= le) {
      if (typ[st[tp]] && judge(M[st[tp]], i)) {
        ...
        add(++cnt, now);  // 新建一个节点，把 now 添加为儿子
        // 如果从当前节点开始不能构成连续段，就合并。
        // 直到找到一个节点能构成连续段。而且我们一定能找到这样
        // 一个节点。
        do
          add(cnt, st[tp--]);
