...
红黑树是一种自平衡的二叉搜索树。每个结点额外存储了一个 color 字段 ("RED" or "BLACK")，用于确保树在插入和删除时保持平衡。
...
???+ note
    部分资料中还加入了第五条性质，即根结点必须为黑色，这条性质要求完成插入操作后若根结点为红色则将其染黑，但由于将根结点染黑的操作也可以延迟至删除操作时进行，因此，该条性质并非必须满足（本文给出的代码实现中满足该性质）。为严谨起见，这里同时引用 [维基百科原文](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree#Properties) 进行说明：
...
???+ note
    在红黑树结点的存储中，用数组来存储子结点指针可以提高代码复用率。
...
-   用 `[p]` 表示结点 `p` 为红色；
-   用 `{p}` 表示结点 `p` 为红色或黑色；
-   用 `|p|` 表示结点 `p` 为 NIL 结点或颜色为黑色。

### 旋转

旋转操作是多数平衡树能够维持平衡的关键，它能在不改变一棵合法 BST 中序遍历结果的情况下改变局部结点的深度。
...
红黑树的插入操作与普通的 BST 类似，对于红黑树来说，新插入的结点初始为红色，完成插入后需根据插入结点及相关结点的状态进行修正以满足上文提到的四条性质。
...
由于插入的结点若不为根结点则必为红色，所以插入后可能违反性质 3，需要维护平衡性。

令插入的结点为 $n$，其父结点为 $p$，祖父结点为 $g$，叔结点为 $u$。由性质 3 可知 $g$ 必为黑色。
...
此时我们需要旋转 $g$ 结点以将 $p$ 转为子树的根，之后交换 $p$ 和 $g$ 的颜色即可。
...
-   若待删除的结点 $n$ 有两个子结点，则交换 $n$ 和左子树中最大结点 $s$ 的数据，并将 $n$ 设为 $s$。此时 $n$ 不可能有两个子结点。
-   若待删除的结点 $n$ 有一个子结点 $s$。由性质 4 可知 $s$ 必为红色，再由性质 3 可知 $n$ 必为黑色。所以只需将 $n$ 在父结点 $p$ 中对应的指针替换为 $s$ 的地址，以及将 $s$ 的父结点指针替换为 $p$ 的地址，之后再将 $s$ 染黑即可。
-   若待删除的结点 $n$ 没有子结点。若 $n$ 是根结点或 $n$ 是红色结点，则直接删除即可，否则直接删除会违反性质 4，需要维护平衡性。
...
由上文讨论可知 $n$ 是黑色叶子结点且不为根结点。我们设 $n$ 的父结点为 $p$，兄弟结点为 $s$，侄结点分别为 $c$ 和 $d$。
...
此时我们旋转 $p$，将 $s$ 转为子树根结点，之后交换 $s$ 和 $p$ 的颜色来转为其余三种情况之一。
...
此时需要旋转 $s$ 使 $c$ 为原来 $s$ 对应子树的根结点，并交换 $s$ 和 $c$ 的颜色转为第四种情况即可。
...
此时需要旋转 $p$ 使 $s$ 为子树的根结点，交换 $s$ 和 $p$ 的颜色，并将 $d$ 染黑即可终止维护平衡。
...
2-3-4 树是 4 阶 B 树，与一般的 B 树一样，2-3-4 树可以实现在 $O(\log n)$ 时间内进行搜索、插入和删除操作。2-3-4 树的结点分为三种，2 结点、3 结点和 4 结点，分别包含一个、两个或三个数据元素。所有的叶子结点都处于同一深度（最底层），所有数据都有序存储。

2-3-4 树和红黑树是同构的，任意一棵红黑树都唯一对应一棵 2-3-4 树。在 2-3-4 树上的插入和删除操作导致结点的扩展、分裂和合并，相当于红黑树中的变色和旋转。下图是 2-3-4 树的 2 结点、3 结点和 4 结点对应的红黑树结点。注意到 2-3-4 树的 3 结点对应红黑树中红色结点左偏和右偏两种情况，所以一棵红黑树可能对应多棵 2-3-4 树。

![2-3-4-tree-rbt-1](images/2-3-4-tree-rbt-1.svg)

下图是一棵红黑树和与之对应的 2-3-4 树。将红黑树中的红色结点上移到父结点的左右两侧，形成一个 B 树结点，就可以得到与之对应的 2-3-4 树。可以发现，红黑树的结点数等于 2-3-4 树的结点个数。
...
nginx 中的用户态定时器是通过红黑树实现的。在 nginx 中，所有 timer 结点都由一棵红黑树进行维护，在 worker 进程的每一次循环中都会调用 `ngx_process_events_and_timers` 函数，在该函数中就会调用处理定时器的函数 `ngx_event_expire_timers`，每次该函数都不断的从红黑树中取出时间值最小的，查看他们是否已经超时，然后执行他们的函数，直到取出的结点的时间没有超时为止。
