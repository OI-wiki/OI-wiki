...
**Splay 树**，或 **伸展树**，是一种平衡二叉查找树，它通过 **伸展（splay）操作** 不断将某个结点旋转到根结点，使得整棵树仍然满足二叉查找树的性质，能够在均摊 $O(\log N)$ 时间内完成插入、查找和删除操作，并且保持平衡而不至于退化为链。
...
Splay 树是一棵二叉查找树，查找某个值时满足性质：左子树任意结点的值 $<$ 根结点的值 $<$ 右子树任意结点的值。
...
| :---: | :-----: | :----: | :----------: | :-----: | :-----: | :----: |
| 根结点编号 | 已使用结点个数 |   父亲   |    左右儿子编号    |   结点权值  |  权值出现次数 |  子树大小  |
...
-   `dir(x)`：判断结点 $x$ 是父亲结点的左儿子还是右儿子；
-   `push_up(x)`：在改变结点位置后，根据子结点信息更新结点 $x$ 的信息。
...
为了使 Splay 保持平衡，需要进行旋转操作。旋转的作用是将某个结点上移一个位置。
...
观察图示可知，如果要通过旋转将结点 $x$（左旋时的 $1$ 和右旋时的 $2$）上移，则旋转的方向由该结点是其父结点的左结点还是右结点唯一确定。因此，实现旋转操作时，只需要将要上移的结点 $x$ 传入即可。

具体分析旋转步骤：（假设需要上移的结点为 $x$，以右旋为例）

1.  首先，记录结点 $x$ 的父结点 $y$，以及 $y$ 的父结点 $z$（可能为空），并记录 $x$ 是 $y$ 的左子结点还是右子结点；
2.  按照旋转后的树中自下向上的顺序，依次更新 $y$ 的左子结点为 $x$ 的右子结点，$x$ 的右子结点为 $y$，以及若 $z$ 非空，$z$ 的子结点为 $x$；
3.  按照同样的顺序，依次更新当前 $y$ 的左子结点（若存在）的父结点为 $y$，$y$ 的父结点为 $x$，以及 $x$ 的父结点为 $z$；
4.  自下而上维护结点信息。
...
在所有函数的实现时，都应注意不要修改结点 $0$ 的信息。

### 伸展操作

Splay 树要求每访问一个结点 $x$ 后都要强制将其旋转到根结点。该操作也称为伸展操作。

设刚访问的结点为 $x$。要做伸展操作，就是要对 $x$ 做一系列的 **伸展步骤**。每次对 $x$ 做一次伸展步骤，$x$ 到根结点的距离都会更近。定义 $p$ 为 $x$ 的父结点。伸展步骤有三种：

1.  **zig**: 在 $p$ 是根结点时操作。Splay 树会根据 $x$ 和 $p$ 间的边旋转。**zig** 存在是用于处理奇偶校验问题，仅当 $x$ 在伸展操作开始时具有奇数深度时作为伸展操作的最后一步执行。
...
2.  **zig-zig**: 在 $p$ 不是根结点且 $x$ 和 $p$ 都是右侧子结点或都是左侧子结点时操作。下方例图显示了 $x$ 和 $p$ 都是左侧子结点时的情况。Splay 树首先按照连接 $p$ 与其父结点 $g$ 边旋转，然后按照连接 $x$ 和 $p$ 的边旋转。
...
3.  **zig-zag**: 在 $p$ 不是根结点且 $x$ 和 $p$ 一个是右侧子结点一个是左侧子结点时操作。Splay 树首先按 $p$ 和 $x$ 之间的边旋转，然后按 $x$ 和 $g$ 新生成的结果边旋转。
...
比较三种伸展步骤可知，要区分此时应使用哪种操作，关键是要判断 $x$ 是否是根结点的子结点，以及 $x$ 和它父结点是否在各自的父结点同侧。

此处提供的实现，可以指定任意根结点 $z$，并将它的子树内任意结点 $x$ 上移至 $z$ 处：

1.  首先记录根结点 $z$ 的父结点 $w$，从而可以利用 `fa[x] == w` 判断 $x$ 已经位于根结点处；
2.  记录 $x$ 当前的父结点 $y$，如果 $y$ 和 $w$ 相同，说明 $x$ 已经到达根结点；
3.  否则，利用 `fa[y] == w` 判断 $y$ 是否是根结点。如果是，直接做 zig 操作将 $x$ 旋转；如果不是，利用 `dir(x) == dir(y)` 判断使用 zig-zig 还是 zig-zag，前者先旋转 $y$ 再旋转 $x$，后者直接旋转两次 $x$。
...
伸展操作是 Splay 树的核心操作，也是它的时间复杂度能够得到保证的关键步骤。请务必保证每次向下访问结点后，都进行一次伸展操作。

另外，伸展操作会将当前结点 $x$ 到根结点 $z$ 的路径上的所有结点信息自下而上地更新一遍。正是因为这一点，才可以修改非根结点，再通过伸展操作将它上移至根来完成整个树的信息更新。
...
??? note "基于势能分析的复杂度证明"
    为此只需分析 **zig**、**zig-zig** 和 **zig-zag** 三种操作的复杂度。为此，我们采用 **势能分析法**，通过研究势能的变化来推导操作的均摊复杂度。假设对一棵包含 $n$ 个结点的 Splay 树进行了 $m$ 次伸展操作，可以通过如下方式进行分析：
    
    **定义**：
    
    1.  **单个结点的势能**：$w(x) = \log(\text{size}(x))$，其中 $\text{size}(x)$ 表示以结点 $x$ 为根的子树大小。
    2.  **整棵树的势能**：$\varphi = \sum w(x)$，即树中所有结点势能的总和，初始势能满足 $\varphi_0 \leq n \log n$。
    3.  **第 $i$ 次操作的均摊成本**：$c_i = t_i + \varphi_i - \varphi_{i-1}$，其中 $t_i$ 为实际操作代价，$\varphi_i$ 和 $\varphi_{i-1}$ 分别为操作后和操作前的势能。
    
    **性质**：
    
    1.  如果 $p$ 是 $x$ 的父结点，则有 $w(p) \geq w(x)$，即父结点的势能不小于子结点的势能。
    2.  由于根结点的子树大小在操作前后保持不变，因此根结点的势能在操作过程中不变。
    ...
    接下来，分别对 **zig**、**zig-zig** 和 **zig-zag** 操作进行势能分析。设操作前后的结点 $x$ 的势能分别是 $w(x)$ 和 $w'(x)$。结点的记号与 [上文](#伸展操作) 一致。
    ...
    **单次伸展操作**：
    ...
    令 $w^{(n)}(x)=(w^{(n-1)})'(x)$ 且 $w^{(0)}(x)=w(x)$。假设一次伸展操作依次访问了 $x_{1}, x_{2}, \cdots, x_{n}$ 等结点，最终 $x_{1}$ 成为根结点。这必然经过若干次 **zig-zig** 和 **zig-zag** 操作和至多一次 **zig** 操作，前两种操作的均摊成本均不超过 $3(w'(x)-w(x))$，而最后一次操作的均摊成本不超过 $3(w'(x) - w(x))+1$，所以总的均摊成本不超过
...
??? info " 为什么 Splay 树的再平衡操作可以获得 $O(\log n)$ 的均摊复杂度？"
    朴素的再平衡思路就是对结点反复进行旋转操作使其上升，直到它成为根结点。这种朴素思路的问题在于，对于所有子结点都是左（右）结点的链状树来说，它相当于反复进行 **zig** 操作，因而 **zig** 操作的均摊复杂度中的常数项 $1$ 会不断累积，造成最终的均摊复杂度达到 $O(\log n+n)$ 级别。Splay 树的再平衡操作的设计，避免了连续 **zig** 的情形中的常数累积，使得一次完整的伸展操作中，至多进行一次单独的 **zig** 操作，从而优化了时间复杂度。

## 平衡树操作

本节讨论基于 Splay 树实现平衡树的常见操作的方法。其中，较为重要的是按照值或排名查找元素，它们可以将某个特定的元素找到，并上移至根结点处，以便后续处理。

作为例子，本节将讨论模板题目 [普通平衡树](https://loj.ac/problem/104) 的实现。

### 按照值查找

作为二叉查找树，可以通过值 $v$ 查找到相应的结点，只需要将待查找的值 $v$ 和当前结点的值比较即可，找到后将该元素上移至根部即可。

应注意，经常存在树中不存在相应的结点的情形。对于这种情形，要记录最后一个访问的结点（即实现中的 $y$），并将 $y$ 上移至根部。此时，结点 $y$ 存储的值必然要么是所有小于 $v$ 的元素中最大的（即 $v$ 的前驱），要么是所有大于 $v$ 的元素中最小的（即 $v$ 的后继）。这是因为查找过程保证，左子树总是存储小于 $v$ 的值，而右子树总是存储大于 $v$ 的值。
...
该实现允许指定任何结点 $z$ 作为根结点，并在它的子树内按值查找。
...
-   否则，如果 $k$ 不大于左子树加上根的大小，那么根结点就是要寻找的；
...
设两棵树的根结点分别为 $x$ 和 $y$，那么为了保证结果仍是二叉查找树，需要要求 $x$ 树中的最大值小于 $y$ 树中的最小值。这条件通常都可以满足，因为两棵树往往是从更大的子树中分裂出的。

合并操作如下：

-   如果 $x$ 和 $y$ 其中之一或两者都为空树，直接返回不为空的那一棵树的根结点或空树；
-   否则，通过 `loc(y, 1)` 将 $y$ 树中的最小值上移至根 $y$ 处，再将它的左结点（此时必然为空）设置为 $x$，并更新结点信息，返回结点 $y$。
...
-   类似按值查找的过程，根据 $v$ 向下查找到存储 $v$ 的结点或者空结点，过程中记录父结点 $y$；
-   如果存在存储 $v$ 的结点 $x$，直接更新信息，否则就新建结点 $x$；
-   做伸展操作，将最后一个结点 $x$ 上移至根部。
...
该实现允许直接向空树内插入值。若不想处理空树，可以在树中提前插入哑结点。

### 删除操作

删除操作也是一个比较复杂的操作。具体步骤如下：（假设删除的值为 $v$）

-   首先按照值 $v$ 查找存储它的结点，并上移至根部；
-   如果不存在存储它的结点，直接返回；（上一步已经做了伸展操作）
-   否则，更新结点信息；
-   如果得到的根结点为空结点，就合并左右子树作为新的根结点，注意合并前需要更新两个子树的根的父结点为空。
...
-   Splay 树上的一个结点代表原序列的一个元素；
-   Splay 树上的一棵子树，代表原序列的一段区间。
...
在操作之前，需要根据所给的序列先把 Splay 树建出来。根据 Splay 树的特性，直接建出一棵只有左儿子的链即可。时间复杂度是 $O(n)$ 的。
...
最后的伸展操作自下而上地更新了结点信息。为了后文区间操作方便，序列左右两侧添加了两个哨兵结点。

### 区间翻转

以区间翻转为例，可以理解区间操作的方法：（设区间为 $[L,R]$）

-   首先将结点 $L-1$ 上移到根结点，再在其右子树中，将结点 $R+1$ 上移到右子树的根结点；
-   此时，设 $x$ 为根结点的右子结点的左子结点，则以 $x$ 为根的子树就对应着区间 $[L,R]$；
...
最后一步的伸展操作并非为了保证复杂度正确，而是为了更新结点信息。因为伸展操作涉及到结点 $x$ 的左右子结点，所以之前需要将结点 $x$ 处的标记先下传一次。当然，仅对于区间翻转操作而言，子区间的翻转不会对祖先结点产生影响，所以省去这一步骤也是正确的。此处实现保留这两行，是为了说明一般的情形下的操作方法。

### 懒标记管理

首先，需要辅助函数 `lazy_reverse(x)` 和 `push_down(x)`。前者交换左右结点，并更新懒标记；后者将标记下传。
...
然后，只需要在向下经过结点时下传标记即可。模板题要求的操作比较简单，只有按照排名寻找的操作（即 `loc`）涉及向下访问结点。注意，需要在函数每次访问一个新的结点 **前** 下传标记。
...
因为向下访问结点时已经移除了经过的路径的所有懒标记，所以利用伸展操作上移结点时不再需要处理懒标记。但是，对于区间操作的那一个结点要谨慎处理：因为它同样位于伸展操作的路径上，但是刚刚操作完，可能存在尚未下传的标记，需要首先下传再做伸展操作，正如同上文所做的那样。
