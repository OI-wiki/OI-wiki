...
能够在这棵树上查找某个值的性质：左儿子的值 $<$ 根结点的值 $<$ 右儿子的值。
...
-   否则按照二叉查找树的性质向下找，找到空结点就插入即可（当然别忘了 $\text{Splay}$ 操作哦）。
...
-   如果 $x$ 比当前结点的权值小，向其左子树查找。
-   如果 $x$ 比当前结点的权值大，将答案加上左子树（ $size$ ）和当前结点（ $cnt$ ）的大小，向其右子树查找。
...
-   否则将 $k$ 减去左子树的和根的大小。如果此时 $k$ 的值小于等于 $0$ ，则返回根结点的权值，否则继续向右子树查找。
...
2.  如果 $x$ 没有儿子结点，那么直接将当前结点 $\text{clear}$ 并退出。
3.  如果 $x$ 只有一个儿子，那么先将当前结点 $\text{clear}$ 再把唯一的儿子作为根结点。
4.  否则将 $x$ 的前驱旋转到根并作为根结点，将 $x$ 的右子树接到根结点的右子树上，最后要将根的信息更新。
