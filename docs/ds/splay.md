...
简单合并两棵 $\text{Splay}$ 树，两棵树的根节点分别为 $x$ 和 $y$ 且 $x$ 为根的树当中的所有关键词都小于 $y$ 为根的树当中的任意关键词，如果 $x$ 和 $y$ 其中之一或两者都为空树，直接返回不为空的那一棵树的根节点或空，否则将 $x$ 为根的树中最右边的节点 $\operatorname{splay}$ 到根并把这个节点的右子树设置为 $y$ 并更新节点的信息，返回这个节点。
