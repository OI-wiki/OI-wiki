...
3.  ⼀棵 Splay 的根节点的 Father 指向它在辅助树中的父亲节点。但是它父亲节点的 ch 并没有指向这个点的。即父亲不不⼀定认⼉子，⽽⼉子能找到⽗亲。
4.  由于 LCT 的 Access 操作（后面会解释），使得 3. 中的⽗亲不认⼉子对答案⽆任何影响，同时，也使一些叶⼦节点单独构成一棵 Splay 辅助树成为可能
