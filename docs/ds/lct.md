...
2.  原树每个结点与辅助树的 Splay 结点一一对应。
3.  辅助树的各棵 Splay 之间并不是独立的。每棵 Splay 的根结点的父亲结点本应是空，但在 LCT 中每棵 Splay 的根结点的父亲结点指向原树中 **这条链** 的父亲结点（即链最顶端的点的父亲结点）。这类父亲链接与通常 Splay 的父亲链接区别在于儿子认父亲，而父亲不认儿子，对应原树的一条 **虚边**。因此，每个连通块恰好有一个点的父亲结点为空。
...
-   原树中的实链 : 在辅助树中结点都在一棵 Splay 中。
-   原树中的虚链 : 在辅助树中，子结点所在 Splay 的 Father 指向父结点，但是父结点的两个儿子都不指向子结点。
-   注意：原树的根不等于辅助树的根。
...
这里提供的 Access 还有一个返回值。这个返回值相当于最后一次虚实链变换时虚边父亲结点的编号。该值有两个含义：

-   连续两次 Access 操作时，第二次 Access 操作的返回值等于这两个结点的 LCA.
-   表示 $x$ 到根的链所在的 Splay 树的根。这个结点一定已经被旋转到了根结点，且父亲一定为空。
...
1.  **splay**
    ...
    -   定义势能函数 $\Phi = \sum_{x \in T} w(x)$，其中 $T$ 表示所有结点的集合。
...
2.  **访问虚边**
    ...
    -   **重虚边**：从结点 $v$ 到其父结点的虚边，其中 $size(v) > \frac{1}{2} size(parent(v))$。

    -   **轻虚边**：从结点 $v$ 到其父结点的虚边，其中 $size(v) \leq \frac{1}{2} size(parent(v))$。
...
综上所述，LCT 中 `Access` 操作的时间复杂度是 splay 和 虚边访问的复杂度之和，因此最后的均摊复杂度为 $O(\log n)$，即 n 个结点的 LCT，做 m 次 `Access` 操作的时间复杂度为 $O(n \log n + m \log n)$，从而基于 `Access` 操作的 `Cut`,`Link`,`Findroot` 等操作的均摊复杂度也为 $O(\log n)$。
...
在 `access` 操作中，在每次 splay 完后，都会改变刚刚 splay 完的结点的右儿子，即该结点与其原右儿子的连边和该结点和新右儿子的连边的虚实情况发生了变化，我们需要加上新变成虚边所连的子树的贡献，减去刚刚变成实边所连的子树的贡献。代码如下：
...
3.  其余部分同普通 LCT，在统计子树信息时一定将其作为根结点。
