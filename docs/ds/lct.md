...
1.  每⼀个 Splay 维护的是一条路径，并且在原树中所有结点深度严格递增，并且，中序遍历这棵 Splay 得到的点序列列的点深度严格递增。
2.  每个结点包含且仅包含于一棵 Splay 中。
3.  ⼀棵 Splay 的根结点的 Father 指向它在辅助树中的父亲结点。但是它父亲结点的 ch 并没有指向这个点的。即父亲不不⼀定认⼉子，⽽⼉子能找到⽗亲。
