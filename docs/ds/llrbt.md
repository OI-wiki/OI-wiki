...
1. 边是红色或黑色，方便起见用一个节点的颜色代指它的父亲边的颜色；
2. 从叶子到根的简单路径不存在两条相邻的红边；即红色的节点的所有儿子的颜色必须是黑色，从每个叶子到根的所有简单路径上没有两个连续的红色节点；
3. # 从任一节点到其子树中的每个叶子的所有简单路径上都包含相同数目的黑边/黑色节点。（黑高平衡）
4. 节点是红色或黑色；
5. 红色的节点的所有儿子的颜色必须是黑色，即从每个叶子到根的所有路径上不能有两个连续的红色节点；
6.  从任一节点到其子树中的每个叶子的所有简单路径上都包含相同数目的黑色节点。（黑高平衡）
    > > > > > > > parent of 2c779899 (<fix>(docs/ds/llrbt.md)：修正对普通红黑树的一些错误的解释）
