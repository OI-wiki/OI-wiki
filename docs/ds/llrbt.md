...
1.  结点是红色或黑色；
2.  NIL 结点（空叶子结点）为黑色；
3.  红色的结点的所有儿子的颜色必须是黑色，即从每个叶子到根的所有路径上不能有两个连续的红色结点；
4.  从任一结点到其子树中的每个叶子的所有简单路径上都包含相同数目的黑色结点。（黑高平衡）

这保证了从根结点到任意叶子的最长路径（红黑交替）不会超过最短路径（全黑）的二倍。从而保证了树的平衡性。

维护这些性质是比较复杂的，如果我们要插入一个结点，首先，它一定会被染色成红色，否则会破坏性质 4。即使这样，我们还是有可能会破坏性质 3。因此需要进行调整。而删除结点就更加麻烦，与插入类似，我们不能删除黑色结点，否则会破坏黑高的平衡。如何方便地解决这些问题呢？
...
在以下左偏红黑树示意图中，是边具有颜色而不是结点具有颜色。我们习惯用一个结点的颜色代指它的父亲边的颜色。

左偏红黑树对红黑树进行了进一步限制，一个黑色结点的左右儿子：
...
我们首先使用普通的 BST 插入方法，在树的底部插入一个红色的叶子结点，然后通过从下向上的调整，使得插入后的树仍然符合左偏红黑树的性质。下面描述调整的过程：
...
因此需要把它进行一次右旋。而对于右旋后的情况，我们应该对它进行 `color_flip`：即翻转该结点和它的两个儿子的颜色
...
删除操作基于这样的思想：我们不能删除黑色的结点，因为这样会破坏黑高。所以我们需要保证我们最后删除的结点是红色的。

##### 删除最小值结点

首先来试一下删除整棵树里的最小值。

怎么才能保证最后删除的结点是红色的呢？我们需要在向下递归的过程中保证一个性质：如果当前结点是 `h`，那么需要保证 `h` 是红色，或者 `h->lc` 是红色。

考虑这样做的正确性，如果我们能够通过各种旋转和反转颜色操作成功维护这个性质，那么当我们到达最小的结点 `h_min` 的时候，有 `h_min` 是红色，或者 `h_min` 的左子树——但是 `h_min` 根本没有左子树！所以这就保证了最小值结点一定是红的，既然它是红色的，我们就可以大胆的删除它，然后用与插入操作相同的调整思路对树进行调整。
...
我们首先考虑删除叶子：与删最小值类似，我们在删除任意值的过程中也要维护一个性质，不过这次比较特殊，因为我们不是只向左边走，而是可以向左右两个方向走，因此在删除过程中维护的性质是这样的：如果往左走，当前结点是 `h`，那么需要保证 `h` 是红色，或者 `h->lc` 是红色；如果往右走，当前结点是 `h`，那么需要保证 `h` 是红色，或者 `h->rc` 是红色。这样可以保证我们最后总会删掉一个红色结点。

下面考虑删除非叶子结点，我们只需要找到其右子树（如果有）里的最小结点，然后用右子树的最小结点的值代替该结点的值，最后删除右子树里的最小结点。
...
2-3 树和左偏红黑树实质是等价的。2-3 树中一个结点可以存储 1 个元素或 2 个元素，而红黑树的一个结点只能存储一个元素。如下图所示，2-3 树的 2 结点对应一个黑色结点，3 结点对应一个红色结点和一个黑色结点（可以将 bc 视作平行）。
