...
下图是一棵红黑树和与之对应的 2-3-4 树。将红黑树中的红色节点上移到父节点的左右两侧，就可以得到与之对应的2-3-4树。可以发现，红黑树的节点数等于2-3-4树的节点个数。
...
插入节点的父节点是黑色节点，可以直接插入，不需要进行调整。如下图插入9、22、28、40。

![2-3-4-tree-rbt](images/2-3-4-tree-rbt-insert-1.svg)

#### Case 2

插入节点的父节点是红色节点，祖父节点是黑色节点，可以分成LL、RR、LR、RL四种情况。其中LL和RR需要进行一次染色和单旋（左旋或右旋）。LR和RL则需要进行染色和双旋。

如下图插入11、13、16、19。其中插入13和16分别为RR型和LL型。以插入13举例，在 2-3-4 树中应将 13 插入12 的右侧，但违反了红黑树的性质，需要将父节点12染成黑色，祖父节点 10 染成红色并左旋10。调整后满足红黑树的性质。
