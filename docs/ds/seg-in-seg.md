...
我们考虑用树套树如何实现在二维平面上进行单点修改，区域查询。我们考虑外层的线段树，最底层的 $1$ 到 $n$ 个结点的子树，分别代表第 $1$ 到第 $n$ 行的线段树。那么这些底层的结点对应的父结点，就代表其两个子结点的子树所在的一片区域。

## 空间复杂度

通常情况下，我们不可能对于外层线段树的每一个结点都建立一颗子线段树，空间需求过大。树套树一般采取动态开点的策略。单次修改，我们会涉及到外层线段树的 $\log{n}$ 个结点，且对于每个结点的子树涉及 $\log{n}$ 个结点，所以单次修改产生的空间最多为 $\log^2{n}$ 。
