...
> 线段树是一种二叉搜索树，与区间树相似，它将一个区间划分成一些单元区间，每个单元区间对应线段树中的一个叶结点。使用线段树可以快速的查找某一个结点在若干条线段中出现的次数，时间复杂度为 $O(\log N+K)$ ，其中 $k$ 是匹配条件的区间数量。而未优化的空间复杂度为 $2N$ ，因此有时需要离散化让空间压缩。——via 百度 & 维基
...
我们有个大小为 $5$ 的数组 $a=\{10,11,12,13,14\}$ 要进行区间求和操作，现在我们要怎么把这个数组存到线段树中（也可以说是转化成线段树）呢？我们这样子做：设线段树的根结点编号为 $1$ ，用数组 $d$ 来保存我们的线段树， $d[i]$ 用来保存编号为 $i$ 的结点的值（这里结点的值就是这个结点所表示的区间总和），如图所示：![](./images/segt1.png)

图中 $d[1]$ 表示根结点，紫色方框是数组 $a$ ，红色方框是数组 $d$ ，红色方框中的括号中的黄色数字表示它所在的那个红色方框表示的线段树结点所表示的区间，如 $d[1]$ 所表示的区间就是 $1\sim 5\ (a[1]\sim a[5])$ ，即 $d[1]$ 所保存的值是 $a[1]+a[2]+ \cdots +a[5]$ ， $d[1]=60$ 表示的是 $a[1]+a[2]+ \cdots +a[5]=60$ 。

通过观察我们不难发现， $d[i]$ 的左儿子结点就是 $d[2\times i]$ ， $d[i]$ 的右结点就是 $d[2\times i+1]$ 。进一步观察，可以看出如果 $d[i]$ 表示的是区间 $[s,t]$ （即 $d[i]=a[s]+a[s+1]+ \cdots +a[t]$ ) 的话，那么 $d[i]$ 的左儿子结点表示的是区间 $[s, \frac{s+t}{2} ]$ ， $d[i]$ 的右儿子表示的是区间 $[ \frac{s+t}{2} +1,t]$ 。
...
分析：容易知道线段树的深度是 $\left\lceil\log{n}\right\rceil$ 的，则在堆式储存情况下叶子结点（包括无用的叶子结点）数量为 $2^{\left\lceil\log{n}\right\rceil}$ 个，又由于其为一棵完全二叉树，则其总结点个数 $2^{\left\lceil\log{n}\right\rceil+1}-1$ 。当然如果你懒得计算的话可以直接把数组长度设为 $4n$ ，因为 $\frac{2^{\left\lceil\log{n}\right\rceil+1}-1}{n}$ 的最大值在 $n=2^{x}+1(x\in N_{+})$ 时取到，此时结点数为 $2^{\left\lceil\log{n}\right\rceil+1}-1=2^{x+2}-1=4n-5$ 。

而如果采用动态开点的方法（动态开点即为不一次性开出全部结点的内存，而是在第一次使用到一个空结点时才开出其内存，这样可以减少空间复杂度的常数。动态开点的方法见[内存池](../../intro/common-tricks/#mempool))，由于不会存在堆式储存中空置结点的问题，故叶子结点数量为 $n$ ，非叶子结点数量为 $n-1$ ，只需要开 $2n-1$ 大小的数组即可。缺点是由于不使用堆式储存，无法知道其左右儿子的编号，故须新增两个域来储存其左右儿子的编号。
...
区间修改是个很有趣的东西……你想啊，如果你要修改区间 $[l,r]$ ，难道把所有包含在区间[l,r]中的结点都遍历一次、修改一次？那估计这时间复杂度估计会上天。这怎么办呢？我们这里要引用一个叫做 **「懒惰标记」** 的东西。

我们设一个数组 $b$ ， $b[i]$ 表示编号为 $i$ 的结点的懒惰标记值。啥是懒惰标记、懒惰标记值呢？这里我再举个例子：
...
在这个故事中我们不难看出，A 就是父亲结点，B 和 C 是 A 的儿子结点，而且 B 和 C 是叶子结点，分别对应一个数组中的值（就是之前讲的数组 $a$ ），我们假设结点 A 表示区间 $[1,2]$ （即 $a[1]+a[2]$ ），结点 B 表示区间 $[1,1]$ （即 $a[1]$ ），结点 C 表示区间 $[2,2]$ （即 $a[2]$ ），它们的初始值都为 $0$ （现在才刚开始呢，还没拿到红包，所以都没钱~）。
...
注：这里 D 表示当前结点的值（即所表示区间的区间和）。
为什么结点 A 的 D 是 $2\times (1000000000000001\bmod 2)$ 呢？原因很简单：结点 A 表示的区间是 $[1,2]$ ，一共包含 $2$ 个元素。我们是让 $[1,2]$ 这个区间的每个元素都加上 $1000000000000001\bmod 2$ ，所以结点 A 的值就加上了 $2\times (1000000000000001\bmod 2)$ 咯。

如果这时候我们要查询区间 $[1,1]$ （即结点 B 的值）怎么办呢？不是说了吗？如果 B 要用到的时候，A 就把它欠的还给 B！
...
void update(int l, int r, int c, int s, int t, int p) {
  // [l,r] 为修改区间,c 为被修改的元素的变化量,[s,t] 为当前结点包含的区间,p
  // 为当前结点的编号
  if (l <= s && t <= r) {
    d[p] += (t - s + 1) * c, b[p] += c;
    return;
  }  // 当前区间为修改区间的子集时直接修改当前结点的值,然后打标记,结束修改
  int m = (s + t) / 2;
  if (b[p] && s != t) {
    // 如果当前结点的懒标记非空,则更新当前结点两个子结点的值和懒标记值
...
-   建树时记录每个结点所对应的区间，就不需要每次计算当前结点的左右端点了，减小代码复杂度。
-   因为下标为 $a$ 的结点的左儿子下标为 $a\times 2$ ，右儿子下标为 $a\times 2+1$ ，所以可以：
...
-   因为递归到叶子结点（左端点等于右端点的结点）的时候叶子结点一定包含在查询的区间内，所以一定会在懒惰标记下放前就处理完了 return 掉了，所以不用担心会出现叶子结点懒惰标记下放导致数组越界，也不用懒惰标记下方每次还检查当前结点是否为叶子结点了。（代码中的 `s!=t` 可以去掉）减小代码复杂度。
-   最好别像上文那样把所有功能都写一起，比如下放懒惰标记可以写一个专门的函数，从儿子结点更新当前结点也可以写一个专门的函数，等等。
...
不同于传统线段树在这个结点里只保留 $[l,r]$ 的和，我们在这个结点里面额外保存 $（l,mid]$ 的后缀和数组和 $(mid,r]$ 的前缀和数组
...
此时我们发现线段树上两个结点的 lca 编号，就是两个结点二进制编号的 lcp
