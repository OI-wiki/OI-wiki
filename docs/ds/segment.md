...
分析: 容易知道线段树的深度是 $\left\lceil\log{n}\right\rceil$ 的,则在堆式储存情况下叶子结点(包括无用的叶子结点)数量为 $2^{\left\lceil\log{n}\right\rceil}$ 个,又由于其为一棵完全二叉树,则其总结点个数 $2^{\left\lceil\log{n}\right\rceil+1}-1$.

而如果采用动态开点的方法(动态开点即为不一次性开出全部节点的内存,而是在第一次使用到一个空节点时才开出其内存,这样可以减少空间复杂度的常数),由于不会存在堆式储存中空置节点的问题,故叶子结点为 $n$ 个,非叶子结点为 $n-1$ 个,只需要开 $2n-1$ 大小的数组即可.缺点是由于不使用堆式储存,无法知道其左右儿子的编号,故须新增两个域来储存其左右儿子的编号.
