...
以下的描述中，对于节点内部所保存的数据，会混用「元素」、「关键字」、「键」等表述，它们的含义相同。
...
如果 $m-1$ 是奇数，那么记 $m=2L$，待分裂节点总共有 $2L-1$ 个元素，每个节点允许的最小元素数量为 $\lceil m/2 \rceil -1=L-1$。分裂后的两个新节点一个有 $L-1$ 个元素，另外一个有 $L$ 个元素，都是合法的节点。如果 $m-1$ 是偶数，那么记 $m=2L-1$, 待分裂节点总共有 $2L-2$ 个元素，每个节点允许的最小元素数量为 $\lceil m/2 \rceil -1=L-1$。分裂后的两个新节点各有 $L-1$ 个元素，也均合法。
