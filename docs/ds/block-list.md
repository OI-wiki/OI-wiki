...
不难发现块状链表就是一个链表，每个结点指向一个数组。
我们把原来长度为 n 的数组分为 $\sqrt{n}$ 个结点，每个结点对应的数组大小为 $\sqrt{n}$ 。
所以我们这么定义结构体，代码见下。
...
分裂操作怎么做呢？先新建一个结点，再把被分裂的结点的后 $\sqrt{n}$ 个值 `copy` 到新结点，然后把被分裂的结点的后 $\sqrt{n}$ 个值删掉（ `size--` ），最后把新结点插入到被分裂结点的后面即可。
