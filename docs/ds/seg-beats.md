...
### [BZOJ4695 最假女选手](https://new.bzoj.org:88/p/P4695)
...
我们把区间 $[l,r]$ 中的备选答案 $A_i+B_i$ 分成四类：$A_i,B_i$ 均不是序列 $A,B$ 区间最大值、$A_i$ 是序列 $A$ 区间最大值但是 $B$ 不是序列 $B$ 区间最大值、$A_i$ 不是序列 $A$ 区间最大值但是 $B$ 是序列 $B$ 区间最大值、$A_i,B_i$ 均是序列 $A,B$ 区间最大值。我们不妨分别设为 $C_{0,0},C_{1,0},C_{0,1},C_{1,1}$ 。此外我们正常维护序列 $A,B$ 的区间最大值和次大值。下传区间加法标记和 $\min$ 标记时对 $A,B$ 最大值和次大值的处理与上述两个例题一致。对 $A$ 的 $\min$ 标记会影响到 $C_{1,1}$ 和 $C_{1,0}$ ，对 $B$ 的标记会影响到 $C_{1,1}$ 和 $C_{0,1}$ 。对 $A,B$ 的加法则会对 $C_{0,0},C_{1,0},C_{0,1},C_{1,1}$ 均产生影响。只需要注意 $C_{0,0},C_{1,0},C_{0,1}$ 不存在的边界情况即可（例如区间 $[i,i]$ 只有 $A,B$ 的最大值与 $C_{1,1}$ 存在）。
