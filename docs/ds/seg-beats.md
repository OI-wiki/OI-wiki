...
???+ note "[BZOJ4695 最假女选手](https://loj.ac/p/6565)"
    ...
    6.  `6 l r` 输出 $\min\limits_{i=l}^r a_i$。$n,m\le 5\times 10^5,~|a_i|\le 10^8$。所有类型 $1$ 操作有 $|x|\le 10^3$，其余操作满足 $|x|\le10^8$。
...
???+ note "[CTSN loves segment tree](https://www.luogu.com.cn/problem/U180387)"
    ...
    5.  `5 l r` 输出 $\max\limits_{i=l}^r (a_i+b_i)$。$n,m\le 3\times 10^5,~|a_i|,|b_i|,|x|\le 10^9$。
...
???+ note "[CPU 监控](https://www.luogu.com.cn/problem/P4314)"
    ...
    2.  对 $A$ 做区间加 $x$
    3.  询问 $A$ 的区间 $\max$
    4.  询问 $B$ 的区间 $\max$ 每次操作后，我们都进行一次更新，$\forall i\in [1,n],\ B_i=\max(B_i,A_i)$。$n,m\le 10^5$。
