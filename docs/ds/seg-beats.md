...
接下来需要考虑在 pushup 时如何维护 $C_{0,0},C_{1,0},C_{0,1},C_{1,1}$。我们可以考虑一下完成 $A,B$ 最大值的更新之后，讨论左右儿子的 $A,B$ 最大值是否与当前结点 $A,B$ 最大值相等。我们以左儿子为例进行讲解，右儿子类似处理：

-   当左儿子的 $A,B$ 最大值与当前结点的 $A,B$ 最大值均相等时，左儿子的 $C_{0,0},C_{1,0},C_{0,1},C_{1,1}$ 会分别对当前结点的 $C_{0,0},C_{1,0},C_{0,1},C_{1,1}$ 产生贡献。
-   当左儿子的 $A$ 最大值与当前结点 $A$ 最大值相等，但是 $B$ 最大值不相等时，左儿子的 $C_{1,0},C_{1,1}$ 会对该结点的 $C_{1,0}$ 产生贡献，$C_{0,0},C_{0,1}$ 会对该结点的 $C_{0,0}$ 产生贡献。
-   当左儿子的 $A$ 最大值与当前结点 $A$ 最大值不相等，但是 $B$ 最大值相等时，左儿子的 $C_{0,1},C_{1,1}$ 会对该结点的 $C_{0,1}$ 产生贡献，$C_{0,0},C_{1,0}$ 会对该结点的 $C_{0,0}$ 产生贡献。
-   当左儿子的 $A,B$ 最大值与当前结点的 $A,B$ 最大值均不相等时，左儿子的 $C_{0,0},C_{1,0},C_{0,1},C_{1,1}$ 仅会对该结点的 $C_{0,0}$ 产生贡献。
