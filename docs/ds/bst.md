...
```cpp
void print(int o)  //遍历以 o 为根结点的二叉搜索树
{
  if (!o) return;          //遇到空树，返回
  print(lc[o]);            //递归遍历左子树
  printf("%d\n", val[o]);  //输出根结点信息
...
定义 `insert(o,v)` 为在以 $o$ 为根结点的二叉搜索树中插入一个值为 $v$ 的新结点。

分类讨论如下：

若 $o$ 为空，直接返回一个值为 $v$ 的新结点。

若 $o$ 的权值大于 $v$ ，在 $o$ 的左子树中插入权值为 $v$ 的结点。

若 $o$ 的权值等于 $v$ ，该结点的附加域该值出现的次数自增 $1$ 。

若 $o$ 的权值小于 $v$ ，在 $o$ 的右子树中插入权值为 $v$ 的结点。
...
定义 `delete(o,v)` 为在以 $o$ 为根结点的二叉搜索树中删除一个值为 $v$ 的结点。

先在二叉搜索树中找到权值为 $v$ 的结点，分类讨论如下：

若该结点的附加 $cnt$ 为 $1$ ：

若 $o$ 为叶子结点，直接删除该结点即可。

若 $o$ 为链结点，即只有一个儿子的结点，返回这个儿子。

若 $o$ 有两个非空子结点，一般是用它左子树的最小值代替它，然后将它删除。
...
维护每个根结点的子树大小 $siz$ 。查找一个元素的排名，首先从根结点跳到这个元素，若向右跳，答案加上左儿子结点个数加当前结点重复的数个数，最后答案加上终点的左儿子子树大小 $+1$ 。
...
若其左子树的大小在区间 $[k-1,k+cnt-1]$ 中，则该元素为子树的根结点；
