...
在以 `root` 为根结点的二叉搜索树中搜索一个值为 `value` 的结点。
...
在以 `root` 为根结点的二叉搜索树中插入一个值为 `value` 的结点。

分类讨论如下：

-   若 `root` 为空，直接返回一个值为 `value` 的新结点。

-   若 `root` 的权值等于 `value`，该结点的附加域该值出现的次数自增 $1$。

-   若 `root` 的权值大于 `value`，在 `root` 的左子树中插入权值为 `value` 的结点。

-   若 `root` 的权值小于 `value`，在 `root` 的右子树中插入权值为 `value` 的结点。
...
???+ note "实现"
    ...
    TreeNode* insert(TreeNode* root, int value) {
      ...
      if (value < root->key) {
        ...
        root->count++;  // 结点值相等，增加重复数量
      }
      root->size = root->count + (root->left ? root->left->size : 0) +
                   (root->right ? root->right->size : 0);  // 更新结点的子树大小
      return root;
    }
    ```
...
在以 `root` 为根结点的二叉搜索树中删除一个值为 `value` 的结点。

先在二叉搜索树中搜索权值为 `value` 的结点，分类讨论如下：

-   若该结点的附加 `count` 大于 $1$，只需要减少 `count`。

-   若该结点的附加 `count` 为 $1$：

    -   若 `root` 为叶子结点，直接删除该结点即可。

    -   若 `root` 为链结点，即只有一个儿子的结点，返回这个儿子。

    -   若 `root` 有两个非空子结点，一般是用它左子树的最大值（左子树最右的结点）或右子树的最小值（右子树最左的结点）代替它，然后将它删除。

时间复杂度 $O(h)$。

???+ note "实现"
    方法使用 `root = remove(root, 1)` 表示删除根结点为 `root` 树中值为 1 的结点，并返回新的根结点。
      ...
      if (value < root->key) {
        ...
        if (root->count > 1) {
          ...
          if (root->left == nullptr) {
            ...
            // 当 successor->count > 1时，也应该删除该结点，否则
            // 后续的删除只会减少重复数量
            successor->count = 1;
...
查找一个元素的排名，首先从根结点跳到这个元素，若向右跳，答案加上左儿子结点个数加当前结点重复的数个数，最后答案加上终点的左儿子子树大小加一。
...
在一棵子树中，根结点的排名取决于其左子树的大小。

-   若其左子树的大小大于等于 $k$，则该元素在左子树中；

-   若其左子树的大小在区间 $[k-\textit{count},k-1]$（`count` 为当前结点的值的出现次数）中，则该元素为子树的根结点；
...
使用搜索树的目的之一是缩短插入、删除、修改和查找（插入、删除、修改都包括查找操作）结点的时间。

关于查找效率，如果一棵树的高度为 $h$，在最坏的情况，查找一个关键字需要对比 $h$ 次，查找时间复杂度（也为平均查找长度 ASL，Average Search Length）不超过 $O(h)$。一棵理想的二叉搜索树所有操作的时间可以缩短到 $O(\log n)$（n 是结点总数）。
...
关于一棵搜索树是否「**平衡**」，不同的平衡树中对「**平衡**」有着不同的定义。比如以 T 为根结点的二叉搜索树，左子树和右子树的高度相差很大，或者左子树的结点个数远大于右子树的结点个数，这棵树显然不具有平衡性。

对于二叉搜索树来说，常见的平衡性的定义是指：以 T 为根结点的树，每一个结点的左子树和右子树高度差最多为 1。

-   [Splay 树](splay.md) 中，对于任意结点的访问操作（搜索、插入还是删除），都会将被访问的结点移动到树的根结点位置。

-   [AVL 树](avl.md) 每个结点 N 维护以 N 为根结点的树的高度信息。AVL 树对平衡性的定义：如果 T 是一棵 AVL 树，当且仅当左右子树也是 AVL 树，且 $|height(T->left) - height(T->right)| \leq 1$。

-   [Size Balanced Tree](sbt.md) 每个结点 N 维护以 N 为根结点的树中结点个数 `size`。对平衡性的定义：任意结点的 `size` 不小于其兄弟结点（Sibling）的所有子结点（Nephew）的 `size`。
...
在这里先介绍右旋，右旋也称为「右单旋转」或「LL 平衡旋转」。对于结点 $A$ 的右旋操作是指：将 $A$ 的左孩子 $B$ 向右上旋转，代替 $A$ 成为根结点，将 $A$ 结点向右下旋转成为 $B$ 的右子树的根结点，$B$ 的原来的右子树变为 $A$ 的左子树。

![bst-rotate](images/bst-rotate.svg)

右旋操作只改变了三组结点关联，相当于对三组边进行循环置换一下，因此需要暂存一个结点再进行轮换更新。

对于右旋操作一般的更新顺序是：暂存 $B$ 结点（新的根结点），让 $A$ 的左孩子指向 $B$ 的右子树 $T2$，再让 $B$ 的右孩子指针指向 $A$，最后让 $A$ 的父结点指向暂存的 $B$。
...
对于这段示例代码，在调用时需要保存 `root` 的父结点 `pre`。方法返回指向新的根结点的指针，只需要将 `pre` 指向新的根结点即可。

#### 四种平衡性破坏的情况

虽然不同的二叉平衡树的定义有所区别，不同二叉平衡树区别只在于结点维护的信息不同，以及旋转调整后结点更新的信息不同。二叉平衡树平衡性被破坏的情况只有以下四种。进行平衡性调整的操作只包括左旋和右旋。以下先介绍四种情况，再对不同的二叉平衡树进行对比。
