...
??? note "证明"
    进行区间查询时访问到的每个结点，其父结点对应区间必然包含查询区间左端点或右端点．如果不然，父结点对应区间要么完全包含在查询区间内，要么与查询区间不交，都不应该继续访问其子结点（即当前结点）．而且，线段树相同深度的结点中，对应区间包含查询区间左右端点的，至多只有 $2$ 个．所以，区间查询时访问到的相同深度结点，至多只有 $4$ 个．线段树深度是 $\Theta(\log N)$ 的，区间查询所访问到的结点数目就也是 $O(\log N)$ 的．
