...
线段树的本质是把一个区间分成两个大小接近的小区间，然后合并两个小区间。

然而二分的本质也是把一个区间分成两个大小接近的小区间，然后找到唯一合法的区间。

于是，就可以把二分的过程「打印」到线段树的结构上，这样就可以每一次只查询当前节点的值即可。
...
单词查询一个节点是 $O(1)$，乘以二分的复杂度 $O(\log n)$，一共是 $O(\log n)$ 的复杂度。
...
???+ note "例题"
    ...
    2.  给定 $x$，查询最小的 $i$ 满足 $\sum \limits_{j = 1}^i a_j \ge x$。
...
-   第二个 $\log$ 来源于线段树的区间查询。用线段树二分的思路，很容易将其优化掉。
...
看一下根节点的左儿子——和只有 $38 < 56$，所以答案不会在左儿子，那只能是右儿子。于是，我们使要询问的 $x$ 从 $56$ 变成 $56 - 38 = 18$。
...
然后发现，由于这个节点上面有一个懒标记，这说明左儿子的信息是不准确的。因此，需要下传标记。
