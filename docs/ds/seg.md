...
除了访问子结点的方法有所差异外，存储方式的选取不会影响建树后线段树操作的实现．
...
前文介绍的递归实现的线段树，都只能自上而下地递归访问，需要支付相应的递归开销．本节介绍另一种自下而上维护线段树的方法．在国内竞赛资料中，这种实现因张昆玮介绍而广为流传，所以它也常称为 **zkw 线段树**．

![](images/seg-4.svg)

上图展示了非递归实现的存储方式．它仍然将线段树嵌入完美二叉树中，但改为自下而上建树．首先，将所有叶结点存储在深度为 $\lceil\log_2N\rceil + 1$ 的同一层，多余的叶结点存储单位元 $e$，可以看作是空区间对应的信息．然后，自下而上遍历非叶结点，将子结点的信息合并到当前结点．由于所有叶结点都排在同一层，这样建立的线段树与前两种方式结构略有差异，但同样可以维护区间信息．好处是可以根据叶结点编号快速算出祖先结点编号，操作无需递归，直接自下而上计算即可．空间上与堆式存储相同，需要长度为 $2^{\lceil\log_2N\rceil + 1}$ 的数组．

需要注意的是，这种实现最适合单点修改、区间查询的场景．一旦涉及区间修改和懒惰标记，标记下传仍需沿根到叶的路径自上而下进行，实现的复杂程度并不低于递归写法．

### 动态开点线段树

前文介绍的所有存储方式，都需要在建树时一次性地分配大小为 $\Theta(N)$ 的存储空间．这样做在区间长度 $N$ 较大（例如 $\sim 10^9$）时并不可行．为了节省空间，可以不一次性建好树，而是在最初只建立一个根节点代表整段区间．当需要访问某个子区间时，才建立代表这段区间的子结点．
