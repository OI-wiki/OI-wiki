...
??? note "证明"
    ...
    这相当于对所有满足 $x_1\le i<x_2$ 的差分的算术平均值．如果 $x_2$ 增加一，就相当于插入一项更大的差分；如果 $x_1$ 增加一，就相当于移除一项最小的差分．这两个操作都会使得平均值弱增．这就说明斜率 $\Delta f(x_1,x_2)$ 弱增，即 $f$ 是 $\mathbf Z$ 上的凸函数．
...
??? note "证明"
    ...
    所以，只要 $\operatorname{epi} f + \operatorname{epi} g$ 是闭集，就有 $\operatorname{epi} f + \operatorname{epi} g = \operatorname{epi}h$．一个使其满足的条件是，$f$ 和 $g$ 都是正常凸函数且 [下半连续](https://en.wikipedia.org/wiki/Semi-continuity)．对于算法竞赛的应用来说，这已经足够了，比如分段线性函数总是满足这些条件的．
...
??? note "解答"
    ...
    明晰了这些操作后，已经可以直接用平衡树维护所有斜率段了，但代码较复杂．注意到问题中斜率每次变化至多 $1$，故而所有斜率的绝对值都不超过 $n$．不直接维护斜率段，转而直接维护斜率拐点更为方便．
    
    设 $f_{i-1}(x)$ 的拐点集为 $\xi_{-k}\le\cdots\le\xi_{-1}\le\xi_{1}\le\cdots\le\xi_{\ell}$．那么，上面的两步操作分别对应：
    ...
    本题还要求输出一种最优方案．因为最后操作结束时，最优解就是堆顶，所以 $b'_n$ 的取值可以直接确定．如果已经知道了第 $i$ 个最优解 $b'_i$，要求解 $f_{i-1}(x)$ 满足 $x\le b'_i$ 的最优解，只需要注意到因为 $f_{i-1}(x)$ 是凸的，所以越接近它的全局最小值点，解就越优，故而只要记录 $f_{i-1}(x)$ 的全局最小值点，并将它与 $b'_i$ 取最小值，就可以得到最优的 $b'_{i-1}$．
...
??? note "解答"
    ...
    显然，对于本题，将正负斜率段分别维护较为方便．因为操作主要集中在零斜率段附近，因此考虑使用 [对顶堆](../../ds/binary-heap.md#对顶堆)，即分别用最大堆和最小堆维护负斜率段和正斜率段的拐点．拐点的整体平移操作用懒标记完成．第二步操作需要分别对两个堆插入一个 $a_i$；而且，插入完成后，最大堆的堆顶未必仍然小于等于最小堆的堆顶．此时，交换两堆顶，直到堆顶的大小关系得到满足即可．
...
??? note "解答"
    ...
    1.  将 $f_{i-1}(x)$ 与函数
        ...
        对应的分段线性函数 $\tilde h_i(x)$（显然是凹函数）做卷积上确界；
