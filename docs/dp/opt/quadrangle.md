...
利用决策单调性，有很多常见算法都可以将算法复杂度优化到 $O(n\log n)$。
...
具体地，算法需要顺次遍历决策点。当遍历到决策点 $k$ 时，队列中需要记录到目前为止每个可行的决策点 $j$ 和能够解决的问题区间左右端点 $l_j$ 和 $r_j$ 构成的 **三元组**。对于给定区间 $[l_j,r_j]$ 内的问题，$j$ 应该是到目前为止考虑过的决策点（即区间 $[1,k]$ 中的决策点）中最小最优的（本节中简称「最优决策」）。每时每刻，队列中存储的决策未必是连续的，但是尚未解决的问题 $[j,n]$ 应该是队列中存储的问题区间的不交并。

为了说明队列更新过程中，决策点 $j$ 是最优决策的问题 $i$ 总构成一段连续的区间，需要适当加强前文的结论：
...
??? note "证明"
    设 $M$ 是充分大的正实数。函数 $w_M(j,i) = w(j,i) + M[j > k]$ 仍然满足四边形不等式，其中，$[\cdot]$ 是 Iverson 括号。而且，对于任何问题 $i$，决策 $j > k$ 都不可能是最优的，即 $\operatorname{opt}_k(i) = \operatorname{opt}(i)$。对 $w_M(j,i)$ 应用定理 1 就得到本推论。
...
前两种算法已经可以解决很多决策单调性优化 DP 问题，但仍然存在其局限性。第一种分治算法无法处理 $w(j,i)$ 需要动态计算的情形（如下一节介绍的区间分拆问题），第二种单调队列算法无法处理 $w(i,j)$ 不支持随机访问的情形。因此，本节介绍一种能够同时克服这两处局限的算法。它是 Larmore 和 Schieber 在 1991 年提出的 LARSCH 算法[^larsch]的简化版本。后者可以在 $O(n)$ 时间内解决决策单调性 DP 问题，但实现较复杂，本文不做介绍。
...
对整个区间 $[1,n]$ 执行递归前，首先需要用决策 $j=1$ 更新问题 $i\in\{1,n\}$ 的取值。该算法在递归到 $l=r$ 时就终止。
...
???+ example "例题 2：[P4767 \[IOI2000\] 邮局 加强版](https://www.luogu.com.cn/problem/P4767)  [P6246 \[IOI2000\] 邮局 加强版 加强版](https://www.luogu.com.cn/problem/P6246)"
