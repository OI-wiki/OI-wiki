...
分治算法无法解决 $w(j,i)$ 需要动态计算的情形。如果 $w(j,i)$ 的计算需要依赖于 $f(j)$ 等函数值，那么，分治算法就没有办法在没有解决左半区间的问题前，就计算出区间中点处的最小最优决策 $\operatorname{opt}(n/2)$。但是，这种情形并不少见，例如下文讨论的区间分拆问题，就无法通过分治算法解决。
...
由于二分队列算法顺次考虑所有问题和决策点，它可以应用于分治算法无法解决的动态计算 $w(j,i)$ 情形。但是，因为二分队列算法的二分步骤依赖于对 $w(j,i)$ 的随机访问，它只能应用于 $w(j,i)$ 可以 $O(1)$ 访问或计算的情形。
...
前两种算法已经可以解决很多决策单调性优化 DP 问题，但仍然存在其局限性。第一种分治算法无法处理 $w(j,i)$ 需要动态计算的情形（如下一节介绍的区间分拆问题），第二种二分队列算法无法处理 $w(i,j)$ 不支持随机访问的情形。因此，本节介绍一种能够同时克服这两处局限的算法。它是 Larmore 和 Schieber 在 1991 年提出的 LARSCH 算法[^larsch]的简化版本。该算法的原始版本可以在 $O(n)$ 时间内解决决策单调性 DP 问题，但实现较复杂，本文不做介绍。
