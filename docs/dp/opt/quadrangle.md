...
前两种算法已经可以解决很多决策单调性优化 DP 问题，但仍然存在其局限性。第一种分治算法无法处理 $w(j,i)$ 需要动态计算的情形（如下一节介绍的区间分拆问题），第二种单调队列算法无法处理 $w(i,j)$ 不支持随机访问的情形。因此，本节介绍一种能够同时克服这两处局限的算法。它是 Larmore 和 Schieber 在 1991 年提出的 LARSCH 算法[^larsch]的简化版本。该算法的原始版本可以在 $O(n)$ 时间内解决决策单调性 DP 问题，但实现较复杂，本文不做介绍。
