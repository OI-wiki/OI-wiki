...
二分/ CDQ 配合斜率优化则是辅助维护性质不那么好（缺少一些单调性性质）的 DP 方程。
...
在算出 $f_i$ 的值后，我们会向凸包中加入 $f_i$ 代表的决策点。在「玩具装箱」问题中，这一点很好实现，因为决策点的横坐标是单调的，我们只需要把这个点插入到队尾即可。

而对于横坐标没有单调性的情况，我们有两种方法：

1. 使用平衡树维护动态凸包，这样比较麻烦。
2. CDQ 分治。

斜率优化是为了快速寻找 $f_i$ 转移的最优决策点。如果我们使用状态集合 $S$ 中的每个状态 $f_j, j\in S$ 去更新 $f_i$ 。只要最优决策点 $k$ 满足 $k\in S$ ，那么我们 $f_i$ 的决策就一定是最优的。

我们可以使用 CDQ 分治， `CDQ(l,r)` 代表求 $f_i,i\in [l,r]$ 。对于 `CDQ(1,n)` ，我们先调用函数 `CDQ(1,mid)` 算出 $f_i,i\in[1,mid]$ 。然后我们对 $[1,mid]$ 这个区间内的点建一个凸包，然后使用这个凸包去更新 $f_i,i\in [mid+1,n]$ 。
