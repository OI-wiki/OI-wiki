...
设 $h_{i,j,k}$ 表示处理到第 $i$ 张牌，共摸了 $j$ 张牌，走到了胡牌自动机上的 $k$ 号节点的方案数。转移枚举摸牌数 $0\leq t\leq 4-a_i$，其中 $a_i$ 为初始 $13$ 张牌中用掉的 $i$ 的张数，则 $g_i=\sum_{j=1}^N h_{n,i,j}$，其中 $N$ 为自动机点数。于是就有转移：
