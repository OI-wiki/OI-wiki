...
-   给定一组长度为 $n$ 的序列（可以是二进制串、数字串、状态序列等）；
-   对于每一个具体的序列，我们有办法用动态规划来判断它是否合法、计算它的权值或求出相关值；
-   但是，我们希望统计所有满足条件的序列的总方案数、总权值、期望值等等。

这时，枚举所有序列是不可行的。于是我们考虑将「判断一个序列是否合法」的过程（即内层 DP）抽象为一个 **确定有限状态 [自动机](../../string/automaton)（DFA）**：这个自动机的每个状态对应于内层 DP 的状态，自动机的转移则对应于内层 DP 的转移。
