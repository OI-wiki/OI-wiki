...
首先我们要说明一下什么是子序列。子序列指的是不改变一个序列的顺序，但从这个序列中去掉若干元素从而获得的一个新的序列。例如：$abc$ 的子序列可以是 $ab$，$ac$。子序列是不需要在原来的序列之中连续出现的。与子序列相对应的一个概念是子串。子串必须在原来的序列之中连续出现。所以 $abc$ 的子串就不包括 $ac$ 了。

如果一个序列既是 $A$ 的子序列，也是 $B$ 的子序列， 那么该序列称为 $A$ 和 $B$ 的公共子序列。 公共子序列可能有多个，例如字符串 $abcde$ 与字符串 $acde$， 它们的公共子序列有 $a$，$c$，$d$，$e$，$ac$，$ad$，$ae$，$cd$，$ce$，$de$，$ade$，$ace$，$cde$，$acde$。最长公共子序列问题即是求解两个字符串的所有公共子序列中的最长的那一个，最长公共子序列可能不唯一，我们的问题是求解最长公共子序列的长度。这个例子中最长公共子序列的长度是4。
...
另外，本题存在 $O\left(\dfrac{nm}{w}\right)$ 的算法[^ref1]。有兴趣的同学可以自行探索，这里不做赘述。
