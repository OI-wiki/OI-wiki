...
考虑每一次更新 $d$ 时维护 $d$ 每一个历史版本中这个位置上的值。

从最后一个元素的最后一次被更新时对应的上升子序列开始，我们每一次二分找到「最后一个可能转移到当前的数并且是当前的数的最优转移的数」。显然这可以二分实现，时间复杂度 $O(n \log n)$.

可以发现算法二中我们实际上是在压缩转移，所以算法的正确性是显然的。
