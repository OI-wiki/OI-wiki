...
首先，定义 $a_1 \dots a_n$ 为原始序列，$d_i$ 为所有的长度为 $i$ 的不下降子序列的末尾元素的最小值，$len$ 为子序列的长度。

初始化：$d_1=a_1,len=1$。

现在已知最长的不下降子序列长度为 1，那么我们让 $i$ 从 2 到 $n$ 循环，依次求出前 $i$ 个元素的最长不下降子序列的长度，循环的时候我们只需要维护好 $d$ 这个数组还有 $len$ 就可以了。**关键在于如何维护。**
...
由于是从前往后扫，所以说当元素大于等于 $d_{len}$ 时一定会有一个不下降子序列使得这个不下降子序列的末项后面可以再接这个元素。

但是，假如不接这个东西，偏偏往前接，既不符合定义，又不是最优解。

于是直接接就是了。

然后，步骤 2：

如果往前面插，理由就和步骤 1 一模一样；

如果往后面插，那么因为这个元素前面的元素也大于要插入的元素，所以又不符合 $d$ 数组的定义。

所以必须找到 **第一个** 大于它的元素，再用 $a_i$ 替换。

可是暴力查找仍然是 $O(n^2)$ 的，需要优化。

根据 $d$ 数组的定义：

$d_i$ 为所有的长度为 $i$ 的不下降子序列的末尾元素的最小值，

又由于本题要求不下降子序列，所以很显然，$d$ 一定是 **单调不减** 的。
