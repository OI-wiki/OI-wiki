...
假设两个字符串分别为 $I$ 与 $J$，同时创建一个二维数组 $c$ 储存不同状态下的最优解，即最长公共子序列长度。

比如说，$c[4][5]$ 储存了 $I$ 前 4 位字符与 $J$ 前 5 位字符状态下的最优解。

每个 $c[i][j]$ 只依赖于 $c[i - 1][j]$、$c[i][j - 1]$ 和 $c[i - 1][j - 1]$。

记录最优方案的时候可以不需要额外建表（优化空间），因为重新选择一遍（转移过程）也是 $O(1)$ 的。

可参考此 [交互网页](http://lcs-demo.sourceforge.net/) 更好的了解 最长公共子序列 的实现过程。
