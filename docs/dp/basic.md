...
由于我们是从前往后扫，所以说当元素大于等于 $d_{len}$ 时一定会有一个不下降子序列使得这个不下降子序列的末项后面可以再接这个元素。

在思考一下，假如你不接这个东西，偏偏往前接，既不符合定义，又不是最优解。
...
可是暴力查找仍然是 $O(n^2)$ 的，那该怎么优化呢？

根据 $d$ 数组的定义：

$d_i$ 为所有的长度为 $i$ 的不下降子序列的末尾元素的最小值，

又由于本题要求不下降子序列，所以说 $d$ 一定是 **单调不减** 的。
