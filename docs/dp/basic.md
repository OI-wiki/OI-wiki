...
子序列的定义可以参考 [子序列](../string/basic.md)。

如果一个序列既是 $A$ 的子序列，也是 $B$ 的子序列， 那么该序列称为 $A$ 和 $B$ 的公共子序列。 公共子序列可能有多个，例如字符串 $abcde$ 与字符串 $acde$， 它们的公共子序列有 $a$，$c$，$d$，$e$，$ac$，$ad$，$ae$，$cd$，$ce$，$de$，$ade$，$ace$，$cde$，$acde$。我们的问题是求解最长公共子序列的长度。所以这个例子中，答案是4。
