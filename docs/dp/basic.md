...
如果算法二还要输出方案，我们只需要再维护一个 $d'_x$ 表示目前所有长度为 $x$ 的最长不下降子序列中，最后一位最小的一个子序列的最后一个数的位置（如果有多个可以任取）。然后，对于每一个 $i$，二分查找出它的最长不下降子序列长度 $l$ 之后直接使状态 $i$ 的最优转移 $p_i$ 变为 $d'_{l - 1}$ 即可。最后找到一个最优状态逐个逐个跳最优转移就可以输出方案了。
...
=== "C++"
    ...
    mx = dp[0];
    for (int i = 0; i < n; ++i) {
      int tmp = std::upper_bound(dp, dp + n, a[i]) - dp;
      pre[i] = di[tmp - 1];
      dp[tmp] = a[i], di[tmp] = i;
    }
    ans = 0;
