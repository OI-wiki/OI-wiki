...
$$
f[l,r] = \mathbf{min}\ \{\ f[l,k] + f[k+1,r]\ |\ l \leq k < r\ \} + w(l,r)\ ,\qquad 1 \leq l \leq r \leq n
$$
...
> **引理 1**    若函数 $w(l, r)$ 满足四边形不等式，则状态 $f[l, r]$ 也满足四边形不等式。

考虑对区间长度使用数学归纳法。

定义 $f_k[l,r]=f[l,k]+f[k+1,r]+w(l,r)$ 表示当决策为 $k$ 时的状态值，任取 $l_1\leq l_2\leq r_1\leq r_2$，记 $u=\mathop{\arg\min}\limits_{l_1\leq k < r_2}{f_{k}[l_1,r_2]},\ v=\mathop{\arg\min}\limits_{l_2\leq k < r_1}{f_{k}[l_2,r_1]}$ 分别表示状态 $f[l_1,r_2]$ 和 $f[l_2,r_1]$ 的最优决策点。

1. 若 $u\leq v$，则 $l_1\leq u< r_1,\ l_2\leq v< r_2$，因此

    $$
    \begin{aligned}
    	f[l_1,r_1] \leq f_u[l_1,r_1] &= f[l_1,u] + f[u+1,r_1] + w(l_1,r_1) \\
    	f[l_2,r_2] \leq f_v[l_2,r_2] &= f[l_2,v] + f[v+1,r_2] + w(l_2,r_2) 
    \end{aligned}
    $$
    ...
    $$
    f[u+1,r_1] + f[v+1,r_2] \leq f[u+1,r_2] + f[v+1,r_1]
    $$
    ...
    $$
    \begin{aligned}
    	f[l_1,r_1] + f[l_2,r_2] & \leq f[l_1,u] + f[l_2,v] + f[u+1,r_2] + f[v+1,r_1] + w(l_1,r_2) + w(l_2,r_1) \\
    	& \leq f_{u}[l_1,r_2] + f_{v}[l_2,r_1] = f[l_1,r_2] + f[l_2,r_1]
    \end{aligned}
    $$
    

2. 若 $v< u$，则 $l_1\leq v<r_1,l\ _2\leq u<r_2$，因此

    $$
    \begin{aligned}
    f[l_1,r_1] \leq f_v[l_1,r_1] &= f[l_1,v] + f[v+1,r_1] + w(l_1,r_1) \\
    f[l_2,r_2] \leq f_u[l_2,r_2] &= f[l_2,u] + f[u+1,r_2] + w(l_2,r_2) 
    \end{aligned}
    $$
    ...
    $$
    f[l_1,v] + f[l_2,u] \leq f[l_1,u] + f[l_2,v]
    $$
    ...
    $$
    \begin{aligned}
    f[l_1,r_1] + f[l_2,r_2] & \leq f[l_1,u] + f[l_2,v] + f[v+1,r_1] + f[u+1,r_2] + w(l_1,r_2) + w(l_2,r_1) \\
    & \leq f_{u}[l_1,r_2] + f_{v}[l_2,r_1] = f[l_1,r_2] + f[l_2,r_1]
    \end{aligned}
    $$
    
综上所述，两种情况均有 $f[l_1,r_1] + f[l_2,r_2] \leq f[l_1,r_2] + f[l_2,r_1]$，即四边形不等式成立。       $\blacksquare$

> **定理 1**  若状态 $f$ 满足四边形不等式，记 $m[l,r]=\min\{\ k\ |\ f[l,r] = f_k[l,r] \ \}$ 表示最优决策点，则有
>
> $$
> m[l,r-1] \leq m[l, r] \leq m[l+1, r]
> $$

记 $u = m[l,r],\ k_1=m[l,r-1],\ k_2=m[l+1,r]$，分情况讨论：
...
1. 若 $k_1>u$，则 $u+1 \leq k_1+1 \leq r-1 \leq r$，因此根据四边形不等式有
    ...
    即 $f_u[l,r-1] \leq f_{k_1}[l,r-1]$，但这与 $k_1$ 是最小的最优决策点矛盾，因此 $k_1\leq u$。

2. 若 $u>k_2$，则 $l\leq l+1 \leq k_2\leq u$，根据四边形不等式可得

    $$
    f[l,k_2] + f[l+1,u] \leq f[l,u] + f[l+1, k_2]
    $$
    
    再根据 $k_2$ 是状态 $f[l+1, r]$ 的最优决策点可知
    
    $$
    f[l+1,k_2] + f[k_2+1, r] \leq f[l+1,u] + f[u+1, r]
    $$
    
    将以上两个不等式相加，得
    
    $$
    f[l,k_2]+f[k_2+1,r] \leq f[l,u] + f[u+1,r]
    $$
    
    即 $f_{k_2}[l,r] \leq f_u[l,r]$，但这与 $u$ 是最小的最优决策点矛盾，因此 $u \leq k_2$。        $\blacksquare$ 

因此，如果在计算状态 $f[l, r]$ 的同时将其最优决策点 $m[l, r]$ 记录下来，那么我们对决策点 $k$ 的总枚举量将降为

$$
\sum_{1\leq l<r\leq n} m[l+1,r] - m[l,r-1] = \sum_{i=1}^n m[i,n] - m[1,i]\leq n^2
$$
...
$$
f[r] = \mathbf{min}\ \{\ f[l]+w(l,r)\ |\ 1\leq l<r \ \}, \qquad 1 \leq r \leq n
$$

> **定理 2**    若函数 $w(l,r)$ 满足四边形不等式，记 $k[r]=\min\{\ l\ |\ f[r]=f_l[r] \ \}$ 表示最优决策点，则有
>
> $$
> k[r_1] \leq k[r_2] \quad \forall r_1 \leq r_2
> $$

记 $l_1=k[r_1],\ l_2=k[r_2]$，若 $l_1>l_2$，则 $l_2<l_1<r_1\leq r_2​$，根据四边形不等式有

$$
w(l_2,r_1) + w(l_1,r_2) \leq w(l_1,r_1) + w(l_2,r_2)
$$

又由于 $l_2$ 是最优决策点，因此 $f_{l_2}[r_2] \leq f_{l_1}[r_2]$，即

$$
f[l_2]+w(l_2,r_2) \leq f[l_1]+w(l_1,r_2)
$$

将以上两个不等式相加，可得

$$
f[l_2]+w(l_2,r_1) \leq f[l_1]+w(l_1,r_1)
$$

即 $ f_{l_2}[r_1] \leq f_{l_1}[r_1]​$，但这与 $l_1​$ 是最小最优决策点矛盾，因此必有 $l_1 \leq l_2​$。        $\blacksquare​$ 
...
$$
f[r] = \mathbf{min}\ \{\ w(l,r)\ |\ 1\leq l<r \ \}, \qquad 1 \leq r \leq n
$$

在这种情况下，我们定义过程 $\textsf{DP}(l, r, k_l, k_r)$ 表示求解 $f[l]\sim f[r]$ 的状态值，并且已知这些状态的最优决策点必定位于 $[k_l, k_r]$ 中，然后使用分治算法如下：
...
题目大意：给定一个长度为 $n\leq 500000 ​$ 的序列 $a_1, a_2, \cdots, a_n​$，要求对于每一个 $1 \leq r \leq n​$，找到最小的非负整数 $f[r]​$ 满足

$$
a_l \leq a_r + f[r] - \sqrt{|r-l|},\qquad \forall\, l = 1, 2, \cdots, n
$$

显然，经过不等式变形，我们可以得到待求整数 $f[r] = \mathbf{max}\{\ a_l+\sqrt{r-l}-a_r\ |\ l = 1,2,\dots, n\ \}​$。不妨先考虑 $l < r​$ 的情况（另外一种情况类似），此时我们可以得到状态转移方程：

$$
f[r] = \mathbf{min}\{\ -a_l-\sqrt{r-l}+a_r\ |\ 1 \leq l < r \}
$$

根据 $-\sqrt{x}$ 的凸性，我们很容易得出（后文将详细描述）函数 $w(l, r) = -a_l - \sqrt{r-l} + a_r$ 满足四边形不等式，因此套用上述的分治算法便可在 $O(n\log n)$ 的时间内解决此题了。

现在处理一般情况， 即转移函数的值是在动态规划的过程中按照一定的拓扑序逐步确定的。此时我们需要改变思维方式，由“确定一个状态的最优决策”转化为“确定一个决策是哪些状态的最优决策“。具体请移步 `单调栈优化DP`。
...
题目大意：有 $n​$ 个玩具需要装箱，要求每个箱子中的玩具编号必须是连续的。每个玩具有一个长度 $C_i​$，如果一个箱子中有多个玩具，那么每两个玩具之间要加入一个单位长度的分隔物。形式化地说，如果将编号为 $l\sim r​$ 的玩具装在一个箱子里，那么这个箱子的长度为 $r-l+\sum\limits_{k=l}^r C_k​$。现在需要制定一个装箱方案，使得所有容器的长度与 $K​$ 差值的平方之和最小。

设 $f[r]$ 表示将前 $r$ 个玩具装箱的最小代价，则枚举第 $r$ 个玩具与哪些玩具放在一个箱子中，可以得到状态转移方程为

$$
f[r] = \mathbf{min}\ \{\ f[l] + \left(r-l-1-K+\sum\limits_{k=l+1}^r C_k\right)^2\ |\ 1\leq l < r \ \}
$$
