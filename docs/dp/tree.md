...
树形 DP 中的换根 DP 问题又被称为二次扫描，通常不会指定根节点，并且根节点的变化会对一些值，例如子节点深度和、点权和等产生影响。
...
不妨令$u$为当前结点，$v$为当前节点的子节点。首先需要用$s_i$来表示以 $i$ 为根的子树中的结点个数，并且有$s_u=\sum s_v$。显然需要一次 DFS 来计算所有的$s_i$，这次的 DFS 就是预处理，我们得到了以某个结点为根时其子树中的结点总数。
