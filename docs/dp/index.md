...
对n个数求它的最长不下降子序列，也就是求最长的上升（一个数比一个数大）子序列有多长。与最长连续不下降子序列不同的是，不需要这个子序列是连续的了。
...
首先，定义$a_1 \dots a_n$为原始序列，d为当前的不下降子序列，len为子序列的长度，那么$d_{len}$就是长度为len的不下降子序列末尾元素.

初始化：$d_1=a_1,len=1$。

现在我们已知最长的不下降子序列长度为1，那么我们让i从2到n循环，依次求出前i个元素的最长不下降子序列的长度，循环的时候我们只需要维护好d这个数组还有len就可以了。**关键在于如何维护。**
