 ...
 **务必理解这个转移方程，因为大部分背包问题的转移方程都是在此基础上推导出来的。** 
...
??? note "例题：[「Luogu P1616」疯狂的采药](https://www.luogu.com.cn/problem/P1616)"
...
\begin{array}{ll}
1 & \textbf{for} \text{ 物品} \\
2 & \qquad \textbf{if} \text{ 是 0-1 背包} \\
3 & \qquad\qquad \text{套用 0-1 背包代码} \\
4 & \qquad \textbf{else if} \text{ 是完全背包} \\
5 & \qquad\qquad \text{套用完全背包代码} \\
6 & \qquad \textbf{else if} \text{ 是多重背包} \\
7 & \qquad\qquad \text{套用多重背包代码}
\end{array}
$$
...
```cpp
for (int k = 1; k <= n; k++) {
  for (int i = m; i >= m[i]; i--)    // 对经费进行一层枚举
    for (int j = t; j >= t[i]; j--)  // 对时间进行一层枚举
      dp[i][j] = max(dp[i][j], dp[i - m[i]][j - t[i]] + 1);
}
```
...
如果 $f_{i,j}=f_{i-1,j}$ 且 $f_{i,j}\neq f_{i-1,j-v}+w$ 说明我们此时不选择把物品放入背包更优，方案数由 $g\_{i-1,j}$ 转移过来；
