...
普通的 0/1 背包是要求最优解，求 0/1 背包第 $k$ 优解的算法就是在普通的背包 dp 方法上稍作改动，增加了一维来记录了当前状态下的前 k 优解。
具体来讲：设 $dp[i][j]$ 是一个大小为 $k$ 的数组， 记录了前 $i$ 个物品中，选择的物品总体积为 $j$时，前 $k$ 大的总价值依次是什么。转移时，普通背包最优解的求法是 $dp[i][j]=max(dp[i-1][j],dp[i-1][j-v[i]]+w[i])$ ，现在我们则是要合并 $dp[i-1][j]$，$dp[i-1][j-v[i]]+w[i]$ 这两个大小为 $k$ 的递减数组，并保留合并后前 $k$ 大的价值记在 $dp[i][j]$ 里，这一步利用双指针法，复杂度是 $O(k)$ 的。
